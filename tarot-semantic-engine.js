  ...
  function compileOOTKEvidence(ootkData) {
    ...
    for (var stage = 1; stage <= 5; stage += 1) {
      ...
      declaredPairs.forEach(function (pair, index) {
        ...
        if (pair.single) {
          stageDeps.push(factory.add('operation_single', '第' + stage + '次操作中央單張', [leftIndex], {
            topology:'unpaired_center_card', stage:stage, eventBinding:'QUERY_EVENT_STAGE_' + stage,
            metadata:{pairingPolicy:op.pairingPolicy || 'opposite_ends_including_significator'},
            eventJoin:'within_same_operation_only'
          }));
          return;
        }
