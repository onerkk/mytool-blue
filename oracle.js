...
var JY_READING_ORACLE = [
  ...
  "呈現方式：像老師父當面解籤，先直說此籤對本題較偏宜進、宜守、待條件或調整，再逐句引用本次原詩，每句用自然段落解釋字面意象、在全詩的作用及如何對應原問題。逐句後把整首串起來，指出真正的轉折與前提；分清籤意的鼓勵、轉機條件、行動方向與實際成事，不只挑一句吉語給保證。",
...
function _buildOraclePrompt(poem, qText) {
  ...
  lines.push('────────────────');
  lines=lines.concat(window.JY_READING_QUALITY?window.JY_READING_QUALITY.lines('oracle'):JY_READING_ORACLE);
  lines.push('【解籤方法】');
  lines.push('1. 開頭直接回答求籤者的問題，給出吉、凶、宜、忌、等待或行動的主要方向與把握度。');
  lines.push('2. 先讀四句籤詩的整體語勢、因果、轉折與收束，逐句辨識背景、條件、阻力、轉機或勸戒的功能，再連回實際問題。逐句引用本次完整原詩，用自然段落說清每句的意思、如何對應本題及前後句如何修正；最後合讀全詩，不用吉凶標籤取代詩意。');
  ...
  lines.push("先用白話給出整首籤詩對本題的主旨，再解釋各句如何承接、轉折與收束；重要的原句要交代它在本題指向什麼、為什麼如此對應。分析仍須涵蓋整首的轉折，不挑好聽的句子；不能把四句硬配四個月份。");
  ...
  lines.push('5. 使用繁體中文，語氣像老師父當面解籤：溫和、直接、有判斷。依「明確主旨 → 詩句脈絡與本題對應 → 轉機與成立條件 → 具體可行方向」自然組織，不逐欄複誦資料。');
  ...
  lines.push('');
  lines.push('【延伸選品】');
  lines.push((window.JY_READING_QUALITY&&window.JY_READING_QUALITY.version==="4.2.0"&&window.JY_READING_QUALITY.recommendationEnding?window.JY_READING_QUALITY.recommendationText('oracle'):JY_REC_ORACLE));
  lines.push('最後保留以下兩行：');
