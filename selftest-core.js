...
(function (root) {
  ...
  var SIGNATURES = {
    ...
    'ink-flow.js': {
      must: [
        '墨流 ink-flow v1.2',                                 // v86.11 輪詢偵測模式
        "PANEL_IDS = ['bzx-screen', 'mhx-screen', 'ln-screen', 'zw-input', 'zw-result']", // v86.11 已知容器
        'window.JY_INK'                                       // API 掛載
      ],
      mustNot: [
        '墨流 ink-flow v1.0',                                  // 舊版（首頁全域層）
        '墨流 ink-flow v1.1'                                   // 舊版（MutationObserver 抓不到 display 切換）
      ]
    },
    'lenormand.js': {
      must: [
        ...
        '它是金屬不是玻璃',                                   // v86.5 天鐵防呆
        '沒有】上下相鄰或對位關係',                            // v86.5 cartouche 對位幻覺根治
        '漏說＝破功；之後再回主盤核對',                        // v86.5 Sig 落收束區硬聲明
        '把該組兩張牌放進同一句裡對照',                        // v86.6 鏡像可驗證規格
        '嚴禁未經聲明直接斷成「另一個女人／另一個男人」',      // v86.6 人物牌歸屬規則
        '你的性別（可選——抽到淑女/紳士時歸屬會更準）',         // v86.7 性別選擇 UI
        '問卜者未聲明性別，此為暫定定位',                     // v86.7 GT 預設揭露
        '默默跳過＝挑牌，違規',                                // v86.8 同線不跳強負面牌
        '【主題牌定位（已由排盤系統精算'                        // v86.8 主題牌定位精算
      ],
      mustNot: [
        '礦物知識（晶系、色帶、淨度、共生礦這類），顯出挑貨有標準——不是自誇'  // 舊無錨點句式
      ]
    }
  };
