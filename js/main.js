...
/**
 * 靜月之光能量占卜儀 v2.0 - Wizard 整合層
 * 用途: 橋接 Wizard 流程與原有八字/梅花/塔羅系統
 */
let globalWizard = null;

const WizardBridge = {
    init: function() {
        console.log('[WizardBridge] 初始化橋接層...');
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', function() { WizardBridge.setup(); });
        } else {
            WizardBridge.setup();
        }
    },
    ...
    setup: function() {
        if (typeof WizardController !== 'undefined') {
            globalWizard = new WizardController({
                onStepChange: WizardBridge.handleStepChange.bind(WizardBridge),
                onComplete: WizardBridge.handleComplete.bind(WizardBridge)
            });
            console.log('[WizardBridge] Wizard 已初始化');
        }
        WizardBridge.bindEvents();
        WizardBridge.initCityData();
    },

    bindEvents: function() {
        document.addEventListener('change', function(e) {
            if (e.target.name === 'questionType') {
                WizardBridge.updateChoiceCardSelection(e.target);
            }
        });
        ...
        if (questionTextarea) {
            questionTextarea.addEventListener('input', function() {
                ...
                if (charHint) charHint.textContent = this.value.length + '/500 字';
            });
        }

        var birthCountry = document.getElementById('birth-country');
        if (birthCountry) {
            birthCountry.addEventListener('change', function(e) {
                WizardBridge.updateCityOptions(e.target.value);
            });
        }

        var birthCity = document.getElementById('birth-city');
        if (birthCity) {
            birthCity.addEventListener('change', function(e) {
                WizardBridge.updateCityCoordinates(e.target);
            });
        }

        var observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.target.dataset.wizardStep === '4' && mutation.target.style.display !== 'none') {
                    WizardBridge.updateConfirmationPage();
                }
            });
        });
        document.querySelectorAll('[data-wizard-step="4"]').forEach(function(el) {
            observer.observe(el, { attributes: true, attributeFilter: ['style'] });
        });

        var skipMeihua = document.getElementById('skip-meihua-wizard');
        if (skipMeihua) {
            skipMeihua.addEventListener('click', function() {
                if (globalWizard) globalWizard.handleNext();
            });
        }
        var skipTarot = document.getElementById('skip-tarot-wizard');
        if (skipTarot) {
            skipTarot.addEventListener('click', function() {
                if (globalWizard) globalWizard.handleNext();
            });
        }

        document.addEventListener('click', function(e) {
            if (e.target.id === 'step1-submit' || e.target.closest('#step1-submit')) {
                e.preventDefault();
                if (globalWizard) globalWizard.handleNext();
            }
        });

        var toggleWizardBtn = document.getElementById('toggle-wizard-mode');
        if (toggleWizardBtn) {
            toggleWizardBtn.addEventListener('click', function() {
                var isWizardMode = document.body.classList.toggle('wizard-mode');
                if (isWizardMode) {
                    document.querySelectorAll('[data-wizard-step]').forEach(function(el) {
                        el.style.display = el.dataset.wizardStep === '0' ? 'block' : 'none';
                    });
                    document.querySelectorAll('.section').forEach(function(el) {
                        if (!el.closest('[data-wizard-step]')) el.style.display = 'none';
                    });
                } else {
                    location.reload();
                }
    ...
    handleStepChange: function(step, stepName) {
        console.log('[WizardBridge] 步驟切換:', step, stepName);
        switch (step) {
            case 4: WizardBridge.updateConfirmationPage(); break;
            case 5: WizardBridge.startCalculation(); break;
            case 6: WizardBridge.loadMeihuaSystem(); break;
            case 7: WizardBridge.loadTarotSystem(); break;
            case 8: WizardBridge.loadResultSystem(); break;
        }
    },

    handleComplete: function() {
        console.log('[WizardBridge] 流程完成');
    },

    updateChoiceCardSelection: function(radio) {
        document.querySelectorAll('.choice-card').forEach(function(card) {
            card.classList.remove('choice-card--selected');
        });
        var selectedCard = radio.closest('.choice-card');
    ...
    initCityData: function() {
        window.CITY_DATA = {
            'TW': [
                { id: 'taipei', name: '台北市', lng: 121.5654, lat: 25.0330 },
                { id: 'taichung', name: '台中市', lng: 120.6736, lat: 24.1477 },
                { id: 'tainan', name: '台南市', lng: 120.2133, lat: 22.9998 },
                { id: 'kaohsiung', name: '高雄市', lng: 120.3014, lat: 22.6273 },
                { id: 'keelung', name: '基隆市', lng: 121.7419, lat: 25.1276 },
                { id: 'hsinchu', name: '新竹市', lng: 120.9647, lat: 24.8138 },
                { id: 'chiayi', name: '嘉義市', lng: 120.4473, lat: 23.4801 },
                { id: 'taoyuan', name: '桃園市', lng: 121.3009, lat: 24.9936 },
                { id: 'yilan', name: '宜蘭縣', lng: 121.7539, lat: 24.7021 },
                { id: 'hualien', name: '花蓮縣', lng: 121.6015, lat: 23.9871 },
                { id: 'taitung', name: '台東縣', lng: 121.1444, lat: 22.7972 },
                { id: 'penghu', name: '澎湖縣', lng: 119.5793, lat: 23.5711 },
                { id: 'kinmen', name: '金門縣', lng: 118.3186, lat: 24.4493 },
                { id: 'lienchiang', name: '連江縣', lng: 119.5397, lat: 26.1605 }
            ],
            'CN': [
                { id: 'beijing', name: '北京', lng: 116.4074, lat: 39.9042 },
                { id: 'shanghai', name: '上海', lng: 121.4737, lat: 31.2304 },
                { id: 'guangzhou', name: '廣州', lng: 113.2644, lat: 23.1291 },
                { id: 'shenzhen', name: '深圳', lng: 114.0579, lat: 22.5431 },
                { id: 'chengdu', name: '成都', lng: 104.0668, lat: 30.5728 },
                { id: 'chongqing', name: '重慶', lng: 106.5516, lat: 29.5630 },
                { id: 'wuhan', name: '武漢', lng: 114.3055, lat: 30.5928 },
                { id: 'xian', name: '西安', lng: 108.9398, lat: 34.3416 }
            ],
            'HK': [
                { id: 'central', name: '中環', lng: 114.1577, lat: 22.2819 },
                { id: 'kowloon', name: '九龍', lng: 114.1722, lat: 22.3193 },
                { id: 'newterritories', name: '新界', lng: 114.1095, lat: 22.4454 }
            ],
            'MO': [ { id: 'macau', name: '澳門', lng: 113.5439, lat: 22.1987 } ],
            'other': [ { id: 'other', name: '其他地區', lng: 0, lat: 0 } ]
        };
    },

    updateCityOptions: function(country) {
        var birthCity = document.getElementById('birth-city');
        if (!birthCity) return;
        birthCity.innerHTML = '<option value="">載入中...</option>';
        birthCity.disabled = true;
        var self = this;
        setTimeout(function() {
            if (window.CITY_DATA && window.CITY_DATA[country]) {
                var cities = window.CITY_DATA[country];
                birthCity.innerHTML = '<option value="">請選擇城市</option>';
                cities.forEach(function(city) {
                    ...
                    birthCity.appendChild(option);
                });
                birthCity.disabled = false;
            } else {
                birthCity.innerHTML = '<option value="">無可用城市</option>';
            }
        }, 300);
    },

    updateCityCoordinates: function(select) {
        var selectedOption = select.options[select.selectedIndex];
        var coordinatesDiv = document.getElementById('city-coordinates');
        var lngSpan = document.getElementById('longitude');
        var latSpan = document.getElementById('latitude');
        var locationText = document.getElementById('solar-location-text');
        if (selectedOption.dataset.lng && selectedOption.dataset.lat) {
            if (lngSpan) lngSpan.textContent = selectedOption.dataset.lng;
            if (latSpan) latSpan.textContent = selectedOption.dataset.lat;
            if (coordinatesDiv) coordinatesDiv.style.display = 'block';
            if (locationText) {
                var countrySelect = document.getElementById('birth-country');
                var countryNames = { TW: '台灣', CN: '中國大陸', HK: '香港', MO: '澳門', other: '其他' };
                var countryName = countryNames[countrySelect && countrySelect.value] || '';
                locationText.textContent = countryName + ' ' + selectedOption.textContent;
            }
            if (globalWizard) {
                globalWizard.updateFormData('longitude', selectedOption.dataset.lng);
                globalWizard.updateFormData('latitude', selectedOption.dataset.lat);
            }
        } else {
    ...
    updateConfirmationPage: function() {
        if (!globalWizard) return;
        var formData = globalWizard.getFormData();
        WizardBridge.updateElement('confirm-name', formData.name || '-');
        WizardBridge.updateElement('confirm-gender', formData.gender === 'male' ? '男' : formData.gender === 'female' ? '女' : '-');
        WizardBridge.updateElement('confirm-birth-date', formData.birthDate || '-');
        WizardBridge.updateElement('confirm-birth-time', formData.birthTime || '未填寫(將使用12:00)');
        var countryNames = { TW: '台灣', CN: '中國大陸', HK: '香港', MO: '澳門', other: '其他' };
        var countryName = countryNames[formData.birthCountry] || '-';
        var citySelect = document.getElementById('birth-city');
        var cityName = (citySelect && citySelect.options[citySelect.selectedIndex]) ? citySelect.options[citySelect.selectedIndex].textContent : '-';
        WizardBridge.updateElement('confirm-birth-location', countryName + ' ' + cityName);
        WizardBridge.updateElement('confirm-solar-time', formData.useTrueSolarTime ? '已啟用' : '未啟用');
        if (window.QUESTION_TYPES && formData.questionType) {
            var typeData = window.QUESTION_TYPES[formData.questionType];
            WizardBridge.updateElement('confirm-question-type', typeData ? typeData.icon + ' ' + typeData.title : '-');
        }
        WizardBridge.updateElement('confirm-question', formData.question || '-');
    },
    ...
    startCalculation: function() {
        console.log('[WizardBridge] 開始計算八字...');
        var formData = globalWizard ? globalWizard.getFormData() : {};
        ...
        var self = this;
        setTimeout(function() { WizardBridge.triggerBaziCalculation(formData); }, 1000);
    },

    triggerBaziCalculation: function(formData) {
        console.log('[WizardBridge] 觸發八字計算:', formData);
        var oldNameInput = document.querySelector('#input-section #name');
        if (oldNameInput) oldNameInput.value = formData.name || '';
        var oldGenderRadio = document.querySelector('#input-section input[name="gender"][value="' + (formData.gender || '') + '"]');
        if (oldGenderRadio) oldGenderRadio.checked = true;
        var oldBirthDate = document.querySelector('#input-section #birth-date');
        if (oldBirthDate) oldBirthDate.value = formData.birthDate || '';
        var oldBirthTime = document.querySelector('#input-section #birth-time');
        if (oldBirthTime) oldBirthTime.value = formData.birthTime || '';
        var calculateBtn = document.getElementById('calculate-bazi');
        if (calculateBtn && typeof calculateBtn.click === 'function') calculateBtn.click();
        if (typeof window.calculateBaziChart === 'function') window.calculateBaziChart(formData);
    },

    loadMeihuaSystem: function() {
        console.log('[WizardBridge] 載入梅花系統...');
        var placeholder = document.getElementById('meihua-content-placeholder');
        var originalSection = document.getElementById('meihua-section');
        if (placeholder && originalSection && placeholder.children.length === 0) {
            var content = originalSection.querySelector('.meihua-container');
            if (content) {
                placeholder.appendChild(content.cloneNode(true));
                WizardBridge.rebindMeihuaEvents();
            }
        }
    },

    rebindMeihuaEvents: function() {
        if (typeof window.initMeihuaSystem === 'function') window.initMeihuaSystem();
    },

    loadTarotSystem: function() {
        console.log('[WizardBridge] 載入塔羅系統...');
        var placeholder = document.getElementById('tarot-content-placeholder');
        var originalSection = document.getElementById('tarot-section');
        if (placeholder && originalSection && placeholder.children.length === 0) {
            var content = originalSection.querySelector('.tarot-container');
            if (content) {
                placeholder.appendChild(content.cloneNode(true));
                WizardBridge.rebindTarotEvents();
            }
        }
    },

    rebindTarotEvents: function() {
        if (typeof window.initTarotSystem === 'function') window.initTarotSystem();
    },

    loadResultSystem: function() {
        console.log('[WizardBridge] 載入結果系統...');
        var placeholder = document.getElementById('result-content-placeholder');
        var originalSection = document.getElementById('result-section');
        if (placeholder && originalSection && placeholder.children.length === 0) {
            var content = originalSection.querySelector('.bazi-container');
            if (content) {
                placeholder.appendChild(content.cloneNode(true));
                WizardBridge.rebindResultEvents();
            }
        }
        WizardBridge.triggerFusionEngine();
    },

    rebindResultEvents: function() {
        if (typeof window.initResultSystem === 'function') window.initResultSystem();
    },

    triggerFusionEngine: function() {
        console.log('[WizardBridge] 觸發融合引擎...');
        if (!globalWizard) return;
        var formData = globalWizard.getFormData();
        if (typeof window.FusionEngine !== 'undefined' && typeof window.FusionEngine.analyze === 'function') {
            var result = window.FusionEngine.analyze({
                question: formData.question,
                questionType: formData.questionType,
                bazi: window.baziResult || {},
                meihua: window.meihuaResult || {},
                tarot: window.tarotResult || {},
                ziwei: window.ziweiResult || {},
                name: window.nameResult || {}
            });
            console.log('[WizardBridge] 融合引擎結果:', result);
            WizardBridge.displayFusionResult(result);
        }
    },

    displayFusionResult: function(result) {
        var answerEl = document.getElementById('direct-answer');
        if (answerEl && result.directAnswer) answerEl.textContent = result.directAnswer;
        var probabilityEl = document.getElementById('overall-probability');
        var meterFill = document.getElementById('meter-fill');
        if (result.probability !== undefined) {
            if (probabilityEl) probabilityEl.textContent = Math.round(result.probability) + '%';
            if (meterFill) meterFill.style.width = result.probability + '%';
        }
        if (result.factors && result.factors.length > 0) {
            var factorsList = document.getElementById('answer-factors-list');
            if (factorsList) {
                factorsList.innerHTML = result.factors.map(function(f) { return '<li>' + f + '</li>'; }).join('');
                var factorsSection = document.getElementById('answer-factors');
                if (factorsSection) factorsSection.style.display = 'block';
            }
        }
        if (result.suggestions && result.suggestions.length > 0) {
            var suggestionsList = document.getElementById('answer-suggestions-list');
            if (suggestionsList) {
                suggestionsList.innerHTML = result.suggestions.map(function(s) { return '<li>' + s + '</li>'; }).join('');
                var suggestionsSection = document.getElementById('answer-suggestions');
