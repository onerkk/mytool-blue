...
(async()=>{
 ...
 await test('Eight new 3D sets have finite transforms and release every geometry, material and timer',async()=>{
  ...
  for(const kind of Object.keys(choreo.CAST)){
   ...
   e.ctx.__T={...Three,WebGLRenderer:Renderer,TextureLoader:Loader};e.ctx.__choreo=choreo;
   ...
   vm.runInContext('const T=__T;const {cardPose,actorPose,cameraPose,instrumentPose,CAST,clamp,ease}=__choreo;'+scenery+stage+';window.__mount=mountStage;',e.ctx);
