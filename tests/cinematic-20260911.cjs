...
(async()=>{
 ...
 await test('All seven scenes release their Three.js geometries, textures, listeners and frame loop; late loads cannot resurrect them',async()=>{
  ...
  for(const kind of ['tarot','lenormand','bazi','compat','ziwei','meihua','oracle']){
   ...
   e.ctx.__T={...Three,WebGLRenderer:Renderer,TextureLoader:Loader};e.ctx.__choreo={cardPose,actorPose,cameraPose,instrumentPose,CAST,clamp:(v,a=0,b=1)=>Math.max(a,Math.min(b,v)),ease:v=>{v=Math.max(0,Math.min(1,v));return v*v*(3-2*v);}};
   ...
   vm.runInContext('const T=__T;const {cardPose,actorPose,cameraPose,instrumentPose,CAST,clamp,ease}=__choreo;'+src+';window.__mount=mountStage;',e.ctx);
