...
if(require.main===module)(async()=>{
 ...
 await test('Recorded sound starts with an interaction; hidden and cancelled rituals stop their own scope',async()=>{
  const e=fixture(),calls=[];e.ctx.JYFoley={prepare:keys=>calls.push(['prepare',...keys]),play:(key,o)=>calls.push(['play',key,o.scope]),stop:scope=>calls.push(['stop',scope]),sync(){},unlock:()=>Promise.resolve(true)};
  load(e,'ritual-ateliers');const h=e.ctx.JYRitual.play('meihua'),d=e.doc.querySelector('dialog');e.clock.advance(1000);assert.equal(calls.filter(c=>c[0]==='play').length,0);
  assert.equal(d.querySelector('.jr-sound').getAttribute('data-foley-preview'),'paper');d.querySelector('.jr-next').click();assert(calls.some(c=>c[0]==='play'&&c[1]==='paper'&&c[2]==='ritual-meihua'));
  e.doc.hidden=true;e.docEvents.dispatch('visibilitychange');assert(calls.some(c=>c[0]==='stop'&&c[1]==='ritual-meihua'));h.cancel();assert.equal(await h.finished,false);assert.equal(calls.at(-1)[0],'stop');assert.equal(e.clock.timers.size,0);
 });
