...
test('prompt invites the AI to use its own tarot knowledge while keeping method data',()=>{const p=prompt('five_card');assert(p.includes('運用你自身完整的塔羅知識'));assert(p.includes('提供閱讀上下文，不替 AI 預判答案'));assert(p.includes('方法資料是閱讀上下文，不是預先寫好的答案'));assert(p.includes('Rider-Waite 圖像象徵'));});
...
test('prompt preserves Book T, true adjacency and a concise recommendation ending',()=>{const p=prompt('relationship','現任是正緣嗎？');assert(p.includes('Golden Dawn《Book T／Liber T》'));assert(p.includes('真正有序相鄰線'));assert(p.includes('延伸選品'));assert(p.includes('以一至兩句將本盤的實際提醒連到日常配戴情境'));assert(!p.includes('交稿前核對'));});
