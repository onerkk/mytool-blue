...
test('foundation upgraded to method-data schema',()=>{assert.equal(F.VERSION,'104.0.0');assert.equal(F.SCHEMA,'jy.tarot.foundation/6');assert.deepEqual(F.validateMethodRegistry(),{ok:true,errors:[]});});
...
test('Mathers methods remain ordered sequence methods',()=>{const p=prompt('mathers_horseshoe','現任是正緣嗎？');assert(p.includes('有序牌列成員'));assert(p.includes('序號表示順序與配對'));assert(p.includes('A組26張由右至左'));assert(p.includes('C組17張同法'));assert(p.includes('E組11張同法'));assert(!p.includes('先在此牌位權限內成義'));});
test('prompt preserves Book T, true adjacency and a concise recommendation ending',()=>{const p=prompt('relationship','現任是正緣嗎？');assert(p.includes('Golden Dawn《Book T／Liber T》'));assert(p.includes('真正有序相鄰線'));assert(p.includes('本題延伸手鍊建議'));assert(p.includes('不先選商品再反推需求'));assert(!p.includes('粉晶手排／10mm／手圍16cm'));assert(!p.includes('【可推薦庫存品項】'));assert(p.includes('【白話優先】'));assert(!p.includes('交稿前核對'));});
