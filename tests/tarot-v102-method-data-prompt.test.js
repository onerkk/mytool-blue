...
test('foundation upgraded to method-data schema',()=>{assert.equal(F.VERSION,'103.1.0');assert.equal(F.SCHEMA,'jy.tarot.foundation/6');assert.deepEqual(F.validateMethodRegistry(),{ok:true,errors:[]});});
...
test('prompt preserves Book T, true adjacency and a concise recommendation ending',()=>{const p=prompt('relationship','現任是正緣嗎？');assert(p.includes('Golden Dawn《Book T／Liber T》'));assert(p.includes('真正有序相鄰線'));assert(p.includes('延伸選品'));assert(p.includes('不先選商品再反推需求'));assert(!p.includes('粉晶手排／10mm／手圍16cm'));assert(!p.includes('【可推薦庫存品項】'));assert(p.includes('【白話優先】'));assert(!p.includes('交稿前核對'));});
