...
test('Real AI payload and final exported prompt agree on every card orientation and method',()=>{
 ...
 const p=ctx.JY_buildExportPrompt('tarot');assert(p.includes('Rider–Waite–Smith'));ctx.drawnCards.forEach(c=>assert(p.includes(c.n+'【'+(c.isUp?'正位':'逆位')+'】')));assert(!p.includes('一般牌陣正向展示'));assert(!p.includes('Book T原典核心義'));assert(p.includes('替代解讀'));assert(/(?:驗證|調整|觀察)(?:訊號|指標)/.test(p));
