...
test('Lenormand: methods cannot substitute tarot meanings or each other’s geometry',()=>{
 ...
 for(const text of Object.values(texts)){assert(text.includes('不使用塔羅')||text.includes('雷諾曼不套用塔羅'));assert(text.includes('本題延伸手鍊建議'));assert(text.includes('理由須引用本次一項有效盤面發現'));assert(text.includes('不捏造商品庫存'));}
});
...
test('Bazi: each lens and each role scenario injects its distinct guide into the actual export',()=>{
 ...
 for(const lens of Object.keys(b.BaziSuiteCore.lenses)){const p=b.BaziSuiteCore.buildSinglePrompt(lens,a,{},'如何改善？');for(const s of b.JY_BAZI_PROMPT_ROOT.lensGuideLines(lens))assert(p.includes(s),lens);assert(p.includes('理由須引用本次一項有效盤面發現'));assert(p.includes('不捏造商品庫存'));}
