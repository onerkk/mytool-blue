...
(async()=>{
 ...
 await p.setViewportSize({width:390,height:844});await p.screenshot({path:out+'/yarrow-ready.png'});const beforeStems=await played(p,'stems');await room.locator('[data-action=toss]').click();await p.waitForTimeout(1400);await p.screenshot({path:out+'/yarrow-action.png'});await p.waitForFunction(()=>!JYGuaRoom.snapshot('yijing').busy);assert((await played(p,'stems'))>=beforeStems+2);assert((await played(p,'bamboo'))>=2);assert.equal(await room.locator('.gw-scene').getAttribute('data-render'),'webgl');
