...
function _fixJsonStringNewlines(s) {
  // 只修 JSON 字串值裡的 literal 換行，不動結構性換行
...
function parseJSON(raw) {
  ...
  if (first !== -1 && last > first) {
    ...
    // 修 trailing commas + 字串值內的 literal 換行
...
function buildTarotUserMessage(p) {
  ...
  // 塔羅解牌方法論
  lines.push('【怎麼看牌】');
  lines.push('每張牌在它的位置上針對問題說什麼——不是通用牌義，是「這張牌在這個位置、面對這個問題」的具體訊息。正位是能量順流，逆位是受阻、內化、或過度。元素看能量偏向（火=行動衝勁、水=情緒感受、風=思考溝通、土=現實物質），缺少的元素就是這件事缺少的面向。大牌多=命運力量介入不完全受控，宮廷牌多=有具體的人在影響局面。GD元素組合看宮廷牌的深層性質。整體逆位比例高=內在阻抗大。牌與牌之間的呼應、矛盾、元素互動比單張牌義重要——相鄰位置的牌在對話。數字學看整副牌的數字加總隱含的主題。');
  lines.push('');
