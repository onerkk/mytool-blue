...
const PRICE_TAROT = 5;   // NT$5 塔羅快讀 (Sonnet)
const PRICE_FULL = 50;   // NT$50 七維度深度 (Sonnet)
const PRICE_OOTK = 45;   // NT$45 開鑰之法 (Sonnet)
const PRICE_FOLLOWUP = 5; // NT$5 追問 (Sonnet)
...
function buildUserMessage(p, questionPlan, autoPassPlan) {
  ...
  // ═══ 核心架構：結構化數據為主（dims），白話文為輔（rawReadings） ═══
  // dims = 精簡結構化欄位（格局/用神/四化/體用/行星/Dasha 等），AI 必須逐一分析
  // rawReadings = 前端產生的白話文判讀，作為背景補充參考
  // crossObs / caseMatrix / timeline / conflicts / semanticResonance = 跨系統交叉摘要

  // ★ v21：主線裁決——從已有的跨系統數據合成 dominant narrative，讓 AI 一開始就知道主次
  try {
    const _csm = (p.structured && p.structured.crossSystem && p.structured.crossSystem.caseMatrix) || {};
    const _cross = p.crossSummary || p.consensus || {};
    const _sr = p.semanticResonance || [];
    const _narrative = [];
    const _modifiers = [];

    // 從 topEssence 抽主線（多系統同指的 tag）
    if (_csm.topEssence && _csm.topEssence.length) {
      const top = _csm.topEssence.filter(t => t.systems && t.systems.length >= 2).slice(0, 2);
      if (top.length) _narrative.push(top.map(t => t.tag + '(' + t.systems.length + '套)').join('＋'));
    }
    // 從 semanticResonance 補強（多系統共振的訊號 = 高可信度）
    if (_sr.length >= 1 && !_narrative.length) {
      _narrative.push(_sr[0]);
    }
    // 從 main_symbols 補（crossSummary 層級）
    if (_cross.main_symbols && _cross.main_symbols.length && !_narrative.length) {
      _narrative.push(_cross.main_symbols.slice(0, 2).join('＋'));
    }

    // 修飾項：阻礙 + 矛盾
    if (_csm.topObstacles && _csm.topObstacles.length) {
      _modifiers.push('阻礙：' + _csm.topObstacles.slice(0, 2).map(t => t.tag).join('、'));
    }
    if (_csm.contradictions && _csm.contradictions.length) {
      _modifiers.push('矛盾：' + _csm.contradictions.slice(0, 2).map(c => c.positive + ' vs ' + c.negative).join('、'));
    }
    if (_cross.variable_symbols && _cross.variable_symbols.length) {
      _modifiers.push('變數：' + (Array.isArray(_cross.variable_symbols) ? _cross.variable_symbols.slice(0, 2).join('、') : _cross.variable_symbols));
    }

    if (_narrative.length) {
      lines.push('【主線裁決】');
      lines.push('主敘事：' + _narrative.join('；'));
      if (_modifiers.length) lines.push('修飾項：' + _modifiers.join('；'));
      lines.push('↑ 你的五層分析必須圍繞主敘事展開。修飾項是配角——它們說明主線在什麼條件下會偏移，但不能喧賓奪主。');
      lines.push('');
    }
  } catch(_e) {}
  ...
  // ═══ 第五層：西洋星盤 ═══
  {
    ...
    if (nd.planets || txt.length >= 20) {
      ...
      lines.push('→ 用以上數據逐一分析：①問題相關宮位的行星配置 ②最緊密相位的意涵 ③行運在推他往哪走 ④次限/太陽弧有沒有事件觸發信號 ⑤定位星收束點=全盤能量核心');
      if (txt.length >= 20) {
        let cleanTxt = txt.replace(/\n*【(?:系統摘要|原始判讀重述|摘要重述|長度補齊|補齊片段|欄位重點重列|來源資料重列|系統載荷重列|結構欄位展開|來源資料 JSON|系統載荷 JSON)】[\s\S]*?(?=\n【|$)/g, '');
        cleanTxt = cleanTxt.replace(/【系統角色】[\s\S]*?(?=\n【|$)/g, '').replace(/^【原始判讀】\n?/, '').trim();
        if (cleanTxt.length >= 20) { lines.push('（白話文補充）'); lines.push(cleanTxt); }
...
const DIRECT_PROMPT = `你是靜月。

你手上有七套命理系統。但你的工作不是把七套各分析一遍——你的工作是看懂面前這個人。

先從他的問題讀他：他怎麼問、他的年齡和處境、他真正焦慮的是什麼、他沒問出口的又是什麼。然後帶著那個理解去看盤。七套系統是你的驗證工具——用不同角度證明你看對了、補充他自己不知道的盲區、給出他接下來具體該怎麼做。

不要從八字開始。從這個人開始。

═══ 七套系統的讀法（你帶著對他的理解來用這些）═══

七套系統不是逐一報告——是交叉碰撞，從五個維度拆解他的問題。每一層至少用兩套系統的交叉結果。
...
二、七套系統都在你手上，但不必每套都給同等篇幅——對這個人這個問題最有說服力的系統多講，弱相關的精簡帶過就好。整個五層跑完後，最終判斷必須有多系統交叉支撐。
...
三、如果數據裡有【主線裁決】，你的五層分析必須服務這條主線。每一層的 conclusion 要推進或挑戰主線——不能五層各說各的、什麼都想講。修飾項只在它改變主線結論的條件下才展開講。
...
輸出 JSON（不加 markdown）：
{
  ...
  "closing": "最後一句話。",
  "crystalRec": "從【水晶商品清單】選一顆最切合此人此刻狀態的（沒清單就省略整個欄位）。",
  "crystalReason": "用兩三句話解釋：你的盤裡哪個具體問題→這顆水晶怎麼對應解決。不要寫『補X行能量』這種泛泛的話，要接回你在 depth 層看到的那個最關鍵的卡點或缺口。語氣像醫師開處方解釋給病人聽（沒清單就省略）。"
}
五層每層都有 conclusion + reading。conclusion 逼自己先想清楚結論再展開。reading 按編號子步驟逐一分析，不能跳過。timing 和 depth 篇幅最長。

重要：在 depth 層的 reading 最後，自然帶出一句「隨身帶一顆XX」的建議，融進行動建議裡——不要另起標題叫水晶推薦。如果沒有水晶商品清單就完全不提水晶。
繁體中文。`;
...
const TAROT_PROMPT = `你是靜月。

他的問題就是最大的牌。他怎麼問、用什麼詞、問的場景、語氣裡的焦慮或期待——在你翻牌之前就已經告訴你一半答案了。你先把這個人看透：他真正怕的是什麼、真正想要的是什麼、沒說出口但你聽得出來的那一層是什麼。

然後你才看牌。牌是用來驗證你看到的、補充他自己不知道的、告訴他接下來該怎麼走。
...
象徵交集（多張牌指向同一件事就是答案）：
・多張牌同向 → 判斷果斷
・牌跟牌打架 → 變數——講清楚什麼條件下往哪邊
・某張牌獨立出現的細節 → 盲區，挖出來
・同一元素反覆出現 → 核心議題

最後你要用交集結果重組成一個映射到他真實處境的故事：前因→現況→走向→變數→他現在該做什麼。

═══ 分析要求 ═══

一、問「對方」就答「對方」。問「他是不是真心」你不能只分析問卜者自己。從牌面推斷對方的狀態、心態、行為模式，再回來講問卜者。
二、每個子問題正面回答。不迴避。
三、每個判斷交代來源（白話）。
四、所有變數講清楚條件——不是「有變數」就結束。
五、「期望翻轉」：先點名他期望的結果，再用牌面告訴他實際情況。
六、每張牌的 role 欄位告訴你它在裁決上的功能（core＝主導結論、outcome＝結果方向、obstacle＝阻力來源、background＝背景補充）。role 是 core 或 outcome 的牌必須主導你的 directAnswer 和 crossReading。
...
輸出 JSON（不加 markdown）：
{
  ...
  "atmosphere": { "conclusion": "10字以內的整體氣氛判斷", "reading": "你從這組牌的元素、大牌比例、倒放比例、宮廷牌看到了什麼整體氣氛——只講影響你判斷的觀察。" },
  "cardReadings": [
    {
      "cardIndex": 0,
      "position": "位置名稱",
      "conclusion": "10字以內",
      "reading": "這張牌在這個位置、跟旁邊的牌組合起來、映射到他的處境後，你看到了什麼。role 是 core 或 outcome 的牌多寫，background 的牌可以精簡。"
    }
  ],
  "crossReading": { "conclusion": "10字以內的交集判斷", "reading": "牌跟牌之間碰撞出來的新發現——單看任何一張牌看不到的洞察。象徵交集、象徵衝突、期望翻轉都在這裡。" },
  "story": "用以上所有分析重組出來的完整故事——不是逐張重複，是新的整合。前因→現況→走向→所有變數→具體建議。",
  "closing": "最後一句話。",
  "crystalRec": "從【水晶商品清單】選一顆最切合牌面訊息的（沒清單就省略整個欄位）。",
  "crystalReason": "牌面指出的核心狀態→這顆水晶怎麼對應。接回 crossReading 或 story 裡最關鍵的點（沒清單就省略）。"
}
cardReadings 策略：3-5 張全寫。6-7 張全寫但 background 牌精簡。8 張以上只寫 role 是 core/outcome/obstacle 的關鍵牌（5-7 張），其餘牌的訊息融進 crossReading 和 story。你分析時看全部的牌，展示上聚焦關鍵牌。
cardReadings 是你的分析素材，crossReading 和 story 才是最終產品。有強訊號的牌多寫，弱的幾句就好——你自己判斷。

重要：在 story 的結尾建議段落裡，如果有水晶商品清單，自然帶出一句「身上帶一顆XX」的建議——融在行動建議裡，不要另起標題叫水晶推薦。如果沒有水晶商品清單就完全不提水晶。
繁體中文。`;
...
const OOTK_PROMPT = `你是靜月。這個人剛完成金色黎明開鑰之法五層占卜。

五層牌陣會從表面潛到靈魂底層。但在你潛下去之前，先看清楚站在你面前的這個人——他的問題怎麼問的、他的處境、他沒說出口的那一層。你先讀懂他，再帶著那個理解一層一層潛下去。每一層看到的東西，要映射回這個人的真實生活，不是抽象的能量描述。
...
═══ 核心分析邏輯：五層收斂鏈——每層只定一個調，五層串成一條因果線 ═══

【第一步：每層定調——只講關鍵發現，不逐牌復述】

每一層先用 conclusion 定調，再用 reading 講這一層最重要的發現。不要把活躍堆每張牌都描述一遍——只講影響判斷的關鍵牌。計數路徑重點在「終點牌＝結論」和「途中最大轉折」，配對重點在「#1 最靠近的那對＝最直接影響」。

Op.1 定調：這件事的本質是什麼元素問題（火＝意志/行動、水＝情感/關係、風＝思維/溝通、土＝物質/現實）
Op.2 定調：這件事打到生活的哪個戰場（哪個宮位＝哪個面向）
Op.3 定調：什麼能量模式在主導這件事（星座特質怎麼左右走向）
Op.4 定調：最尖銳的焦點和引爆點在哪（旬主星聚焦到什麼具體能量）
Op.5 定調：靈魂層面的功課是什麼（跟 Op.1 表層的落差就是真正的答案）

【第二步：五層交叉——這才是整個解讀最值錢的部分】

crossAnalysis 不是五層摘要拼接，是五層因果鏈：
因為本質是 X（Op.1）→ 打到這個生活面向（Op.2）→ 用這種模式運作（Op.3）→ 焦點在這裡（Op.4）→ 靈魂功課是這個（Op.5）→ 所以真正的答案是……

交叉規則：
・同一張牌在多層出現 → 核心答案，判斷果斷
・多層同向 → 定論
・層間矛盾 → 變數（什麼條件下往好、什麼條件下往壞）
・Op.1 表層 vs Op.5 最深處的翻轉 → 表面看起來是 A，潛到底其實是 B
・前因→現況→走向→所有變數→具體建議
...
輸出 JSON（不加 markdown）：
{
  "directAnswer": "直接判斷，一到兩句話。",
  "operations": {
    "op1": { "conclusion": "10字以內——這件事的本質元素", "reading": "這一層你看到了什麼、為什麼這樣判斷。只講影響結論的關鍵牌和訊號。" },
    "op2": { "conclusion": "10字以內——打到哪個生活戰場", "reading": "這一層你看到了什麼、為什麼這樣判斷。跟Op.1呼應還是矛盾。" },
    "op3": { "conclusion": "10字以內——什麼模式在主導", "reading": "這一層你看到了什麼、為什麼這樣判斷。跟前面的層有什麼交叉。" },
    "op4": { "conclusion": "10字以內——最尖銳的焦點", "reading": "這一層你看到了什麼、為什麼這樣判斷。焦點在哪。" },
    "op5": { "conclusion": "10字以內——靈魂功課", "reading": "最深處看到的跟Op.1表層的落差——這個翻轉是什麼意思。" }
  },
  "crossAnalysis": "整個解讀的核心產品。五層因果鏈 + 期望翻轉 + 前因→現況→走向→變數→建議。",
  "summary": "好壞都講。具體建議至少三個，到行為層面。",
  "closing": "最後一句話。",
  "crystalRec": "從【水晶商品清單】選一顆最切合五層交集核心的（沒清單就省略整個欄位）。",
  "crystalReason": "五層潛到底看到的核心卡點→這顆水晶怎麼對應。接回 crossAnalysis 的發現（沒清單就省略）。"
}
五層 operations 是你的分析素材，crossAnalysis 才是你的最終產品。有強訊號的層多寫，弱的層幾句就好——你自己判斷。省下的空間全部灌進 crossAnalysis 和 summary。不要逐牌復述活躍堆——只講改變你判斷的關鍵牌。

重要：在 summary 的具體建議裡，如果有水晶商品清單，自然帶出一句「身上帶一顆XX」——融在行動建議裡，不要另起標題叫水晶推薦。如果沒有水晶商品清單就完全不提水晶。
繁體中文。`;
...
function buildTarotUserMessage(p) {
  ...
  // ── 牌面逐張（原封不動全部給 AI）──
  lines.push('【牌面（逐張）】');
  lines.push('（「順」＝正放、「逆」＝倒放，回答時用白話如「卡住」「反著來」「順的」，不要說「正位」「逆位」）');
  lines.push('');
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (body.action === 'check') {
        ...
        if (cp.mode === 'ootk') {
          const ootkKey = `ootk:${today}:${userKey}`;
        ...
        if (cp.mode === 'tarot_only') {
          const tarotKey = `tarot:${today}:${userKey}`;
        ...
        if (cp.mode === 'tarot_followup' || cp.mode === 'full_followup') {
          const fuKey = `tarot_fu:${today}:${userKey}`;
        ...
        // 七維度：每日 1 次免費
        const fullKey = `full:${today}:${userKey}`;
      ...
      const aiProcess = (async () => {
        try {
          ...
          if (isFullFollowUpMode) {
            ...
            // ★ v20：Sonnet 自律性好，不需過多 buffer 避免灌水
            const _cardCount = (payload.tarotData && payload.tarotData.cards) ? payload.tarotData.cards.length : 0;
            const _tarotMaxTokens = isFollowUp ? 1500 : (_cardCount >= 10 ? 4000 : (_cardCount >= 6 ? 3200 : 2500));
            aiResult = await callAI(env, fuPrompt, tarotMessage, _tarotMaxTokens, 0.68, 'claude-sonnet-4-20250514', () => sendSSE('ping', ''));
          } else {
            ...
            if (fullMessage.length > MAX_MSG_CHARS) {
              ...
              // 第四輪：最後才砍 dims（★ v21：按題型決定砍的順序）
              if (fullMessage.length > MAX_MSG_CHARS && payload.dims) {
                // dims.bazi + dims.ziwei + dims.meihua 永遠不砍（格局/用神/四化/體用是核心）
                // 題型決定砍序：只砍對該題型最不重要的，核心系統保護不砍
                const _topic = (questionPlan && questionPlan.topic) || 'general';
                let _trimOrder;
                if (_topic === 'love' || _topic === 'relationship') {
                  // 感情：塔羅(問事直判)+西占(5/7/8宮) 不能砍 → 只砍吠陀、星盤
                  _trimOrder = ['vedic', 'natal'];
                } else if (_topic === 'career' || _topic === 'wealth') {
                  // 事業/財運：吠陀Dasha+八字大運 不能砍 → 只砍塔羅、星盤
                  _trimOrder = ['tarot', 'natal'];
                } else if (_topic === 'health') {
                  // 健康：吠陀力量指數+紫微疾厄 不能砍 → 只砍塔羅、星盤
                  _trimOrder = ['tarot', 'natal'];
                } else {
                  // general / family：預設順序，三個都可砍
                  _trimOrder = ['tarot', 'vedic', 'natal'];
                }
                for (let _ti = 0; _ti < _trimOrder.length; _ti++) {
                  if (fullMessage.length <= MAX_MSG_CHARS) break;
                  const _dk = _trimOrder[_ti];
                  if (payload.dims[_dk]) {
                    delete payload.dims[_dk];
                    fullMessage = buildUserMessage(payload, questionPlan, autoPassPlan);
                  }
                }
                // ★ v20：dims.meihua 不再砍——體用/動爻/變卦是問事直判核心，砍掉等於丟結論
                analysisNotes += 'trim_order=' + _trimOrder.join('>') + '_topic=' + _topic + '; secondary_trim_to_' + fullMessage.length + '; ';
              }
            }
            
            await sendSSE('progress', { step: 'analyzing', message: '七套系統交叉比對中…' });
            // ★ v17：五層交叉分析，5000 tokens
            // ★ v20：七維度五層交叉實需~5000，留 buffer 不灌水
            aiResult = await callAI(env, DIRECT_PROMPT, fullMessage, 5500, 0.68, 'claude-sonnet-4-20250514', () => sendSSE('ping', ''));
          }
