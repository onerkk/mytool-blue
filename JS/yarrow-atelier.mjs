...
export function createYarrow(host){
 ...
 function attach(next){detach();host=next;paused=false;host.appendChild(renderer.domElement);host.dataset.render=lost?'fallback':'webgl';host.addEventListener('pointerdown',down);host.addEventListener('pointermove',move);host.addEventListener('pointerup',up);host.addEventListener('pointercancel',up);ro.observe(host);io.observe(host);size();}
 attach(host);
 return {kind:'yarrow',attach,update(next){state=next;progress.forEach((a,i)=>{a.material.emissiveIntensity=i<(next.values?.length||0)?.8:0;});wake();},animate(){motion={start:performance.now(),from:stalks.map(a=>pose(a.position.x,a.position.y,a.position.z,a.rotation.y,a.rotation.x))};phase=-1;wake();},finish(){motion=null;phase=-1;wake();},pause(){paused=true;cancelAnimationFrame(raf);raf=0;},dispose(){if(dead)return;dead=true;cancelAnimationFrame(raf);detach();document.removeEventListener('visibilitychange',visibility);renderer.domElement.removeEventListener('webglcontextlost',contextLost);resources.forEach(r=>r.dispose?.());environment.dispose();renderer.dispose();renderer.forceContextLoss();renderer.domElement.remove();},get mode(){return host.dataset.render;}};
}
