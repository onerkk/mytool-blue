...
function _render() {
  ...
  if (_lnPhase === 'input') {
    ...
    for (var a=0;a<AI_LIST.length;a++) {
      var ai = AI_LIST[a];
      // v3.13：Gemini 用目前頁面直接進官方 HTTPS，避免 Android/PWA 建立白色空白視窗後返回；其餘 AI 維持新分頁。
      var aiTarget = ai.id === 'gemini' ? '_self' : '_blank';
      h += '<a class="ln-ai-sc" href="'+ai.url+'" target="'+aiTarget+'" rel="external noopener noreferrer" onpointerdown="_lnPrimeAICopy(event,this)" onclick="_lnPrimeAICopy(event,this)" aria-label="複製提示詞並開啟 '+ai.name+' 網頁版">';
      h += '<img src="ai-icons/ai-'+ai.id+'.png" alt="'+ai.name+'">';
      h += '<span>'+ai.name+'</span></a>';
    }
    h += '</div><div class="ln-ai-foot">Gemini 以官方 HTTPS 同頁開啟，避免白畫面閃退；進入後直接貼上送出</div></div>';
