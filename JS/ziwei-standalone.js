/*! ziwei-standalone.js — 靜月之光 紫微斗數獨立流程  [v80.54]
...
(function () {
  ...
  try{var _g2=document.createElement('style');_g2.setAttribute('data-jy-gilt2','ziwei');_g2.textContent='.zw-in-sec{background:linear-gradient(180deg,rgba(24,20,14,.78),rgba(14,12,9,.86));border:1px solid rgba(201,168,76,.2);border-radius:18px;box-shadow:0 18px 40px rgba(0,0,0,.45),inset 0 1px 0 rgba(245,231,184,.14);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)}.zw-in-title{position:relative;padding-left:12px;letter-spacing:.08em;color:#e8d28a}.zw-in-title::before{content:"";position:absolute;left:0;top:50%;transform:translateY(-50%);width:3px;height:1.05em;border-radius:2px;background:rgba(156,130,222,.9);box-shadow:0 0 8px rgba(156,130,222,.9)}.zw-in-q,.zwx-field,.zw-in-sec input,.zw-in-sec select,.zw-in-sec textarea{background:rgba(8,7,5,.62);border:1px solid rgba(201,168,76,.26);border-radius:12px;color:#f2e9d6;transition:border-color .2s,box-shadow .2s}.zw-in-q:focus,.zwx-field:focus,.zw-in-sec input:focus,.zw-in-sec select:focus,.zw-in-sec textarea:focus{border-color:#e8d28a;box-shadow:0 0 0 3px rgba(201,168,76,.16);outline:none}.zw-in-pill,.zwx-sbtn,.zwx-cell{background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.22);color:#d8c79a;border-radius:12px;transition:all .18s}.zw-in-pill.active,.zw-in-pill.on,.zwx-sbtn.active,.zwx-cell.active,.zwx-cell.on{background:linear-gradient(135deg,#e8d28a,#c9a84c);color:#171208;border-color:transparent;box-shadow:0 6px 18px rgba(201,168,76,.28);font-weight:700}.zw-in-go{background:linear-gradient(110deg,#8a6d2f,#e8d28a 28%,#c9a84c 52%,#f5e7b8 74%,#8a6d2f);background-size:220% 100%;animation:jyGiltFlow 5.5s linear infinite;color:#171208;border:none;border-radius:14px;font-weight:800;letter-spacing:.14em;box-shadow:0 10px 26px rgba(201,168,76,.32),inset 0 1px 0 rgba(255,255,255,.35)}.zw-in-go:active{transform:translateY(1px)}.zw-in-back{color:rgba(232,210,138,.75)}.zw-in-back:hover{color:#f5e7b8}.zwx-sheet{background:rgba(16,13,10,.93);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);border-top:1px solid rgba(201,168,76,.3);box-shadow:0 -18px 50px rgba(0,0,0,.6)}.zwx-grip{background:linear-gradient(90deg,#8a6d2f,#e8d28a,#8a6d2f);opacity:.85;border-radius:99px}.zw-ai{background:linear-gradient(180deg,rgba(24,20,14,.78),rgba(14,12,9,.86));border:1px solid rgba(201,168,76,.2);border-radius:18px;box-shadow:0 18px 40px rgba(0,0,0,.45),inset 0 1px 0 rgba(245,231,184,.14);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)}.zwx-cal-d{border-radius:10px}.zwx-cal-d.active{background:linear-gradient(135deg,#e8d28a,#c9a84c);color:#171208;border-color:transparent;box-shadow:0 6px 18px rgba(201,168,76,.28);font-weight:700}.zw-board{border-color:rgba(201,168,76,.28)}@media (prefers-reduced-motion:reduce){.zw-in-go{animation:none}}@supports not (backdrop-filter:blur(1px)){[data-jy-view-ziwei]{}}.zw-in-go:focus-visible{outline:2px solid #e8d28a;outline-offset:2px}';document.head.appendChild(_g2);}catch(e){}
  }
...
'③三方四正一起看：判任何一宮，必須同時看本宮＋對宮(遷移/夫官財)＋三合兩宮，絕不可只憑本宮一顆星下結論。空宮要明講「借對宮某某星」。\n' +
...
'⑦應期要具體：用大限主星、流年四化、流年命宮落點推出「哪一年、最可能哪幾個月」，並說出為什麼是這個時間。禁止只說「快了」「近期」「未來幾年」。每件重要事件要一併標出：時間範圍＋吉凶屬性(吉/凶/平)＋影響程度(高/中/低)。\n' +
...
'1. 先定命格高低與主軸：用引擎給的「命盤格局」＋命宮主星廟旺＋三方吉煞，先一句話定調這是什麼層次、走什麼路線的命（不是把每顆星意義背一遍）。\n' +
...
'・綜合題（這一生怎麼走、整體格局、未來幾年總運）→ 才做大限定位＋流年應期＋可驗證人事的全幅鋪陳。\n' +
'\n' +
...
'・收尾 2–3 句：依「化忌或煞星所在宮位的領域弱點」推薦一種適合隨身的水晶/能量石補那塊（例：化忌在福德→安神靜心類；煞重在財帛→聚財穩定類；化忌在夫妻→暖心助緣類），語氣平實不推銷，可輕附蝦皮去處，並以連結收尾：https://shopee.tw/a50h95648d?tab=shop ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。只講一種、只在最後（品牌實務輔助，非斗數原理）。主石只准寫出「一種」石頭的名字——嚴禁「或」「這類」「例如A、B」式並列第二種；石頭名也只能是通用礦石名，不可使用命盤外的商品資訊。【嚴禁】引用八字、日主、天干、用神、五行喜忌等本盤沒有的資料來湊——本盤只有紫微，沒有八字；講出「乙木」「用神丁火」這類就是編造，一律不准。';
