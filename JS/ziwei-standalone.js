/*! ziwei-standalone.js — 靜月之光 紫微斗數獨立流程  [v80.52]
...
(function () {
  ...
  function palaceStarParts(p) {
    ...
    (p.stars || []).forEach(function (s) {
      ...
      var label = s.name + (tag ? '·' + tag.slice(-1) : '') + hua;
  ...
  function serializeChart(zw, form) {
    ...
    try {
      if (zw.feiGongHua && zw.feiGongHua.length) {
        ...
        L.push('(讀法：A宮「忌」入B宮＝因A領域的執著或破耗，牽動／傷到B領域；「祿」入＝把A的資源送進B；標(自化)＝該宮自己漏掉或反復。)');
      }
    } catch (e) {}
...
'②看盤不看心：依星曜、宮位、廟旺、四化的客觀資料推，不要憑空臆測他的情緒或動機。\n' +
...
'⑤廟旺利陷定星力：同一顆星廟旺與落陷天差地別；落陷主星＋煞，別當吉論。\n' +
'⑥體用分清：命盤是「體」(一生底色)，大限是「十年的用」，流年是「當年的用」。論運勢要把大限宮位＋流年四化疊到本命宮位上看，不能只報本命。\n' +
...
'・收尾 2–3 句：依「化忌或煞星所在宮位的領域弱點」推薦一種適合隨身的水晶/能量石補那塊（例：化忌在福德→安神靜心類；煞重在財帛→聚財穩定類；化忌在夫妻→暖心助緣類），語氣平實不推銷，可輕附蝦皮去處，並以連結收尾：https://tw.shp.ee/9UHEJTp4 ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。只講一種、只在最後（品牌實務輔助，非斗數原理）。【嚴禁】引用八字、日主、天干、用神、五行喜忌等本盤沒有的資料來湊——本盤只有紫微，沒有八字；講出「乙木」「用神丁火」這類就是編造，一律不准。';
  ...
  function renderChartGrid(zw) {
    ...
    palaces.forEach(function(p){
      ...
      (p.stars || []).forEach(function(s){
        ...
        starHtml += '<span>' + esc(s.name) + (br?'<span style="opacity:.5;font-size:.5rem">' + br.slice(-1) + '</span>':'') + hua + '</span>';
      });
      // 輔吉/煞（精簡，最多各3）
