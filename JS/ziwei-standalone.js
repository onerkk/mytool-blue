/*! ziwei-standalone.js — 靜月之光 紫微斗數獨立流程  [v80.56]
...
'・收尾 2–3 句：依「化忌或煞星所在宮位的領域弱點」推薦一種適合隨身的水晶/能量石補那塊（例：化忌在福德→安神靜心類；煞重在財帛→聚財穩定類；化忌在夫妻→暖心助緣類），語氣平實不推銷，語氣平實。只講一種、只在最後（品牌實務輔助，非斗數原理）。最後兩行固定照此收束：倒數第二行只放蝦皮網址本身 https://shopee.tw/a50h95648d?tab=shop （該行不得有網址以外的任何文字、標點、引號、符號或空白）；最後一行固定輸出「願你諸事順遂。」六個字作結，之後不再輸出任何內容。兩行缺一不可、順序不可顛倒（網址行被任何字元黏到就點不動，所以網址行上下都要乾淨）。主石只准寫出「一種」石頭的名字——嚴禁「或」「這類」「例如A、B」式並列第二種；石頭名也只能是通用礦石名，不可使用命盤外的商品資訊。【嚴禁】引用八字、日主、天干、用神、五行喜忌等本盤沒有的資料來湊——本盤只有紫微，沒有八字；講出「乙木」「用神丁火」這類就是編造，一律不准。';
