/*! ziwei-standalone.js — 靜月之光 紫微斗數獨立流程  [v80.62]
...
(function () {
  ...
  function serializeChart(zw, form) {
    ...
    try {
      ...
      L.push('');
      L.push('【三方四正索引（引擎依本盤地支動態計算）】(題目主宮必須照此取對宮與兩個三合宮；其他宮位只能作另有理由的補充機制，不能冒充三方四正)');
    ...
    if (zw.daXian && zw.daXian.length) {
      ...
      L.push('');
      L.push('【運限計算政策】大限採虛歲；現行大限優先採引擎 isCurrent，若引擎未標才以「系統公曆年份－出生年＋1」回推。資料未提供精確大限切換日期，故只能按引擎年齡區間判讀，不得自創交界日。流年以下方公曆年份參數列示；未提供農曆年／立春切換時間與流月盤，年度邊界附近須降級，且不得細化月份。');
  ...
  var ZW_HEAD_FALLBACK =
    '你是資深紫微斗數證據整合者。第一句直接回答完整原問句；若答案取決於特定他人或盤外事實，先說命盤不能判定該部分，再回答命主可判的條件與行動。只依本次資料，使用引擎動態提供的三方四正、主星組合、資料所列廟旺、生年四化、本命／大限／流年為主判；宮干飛化、自化與來因宮只作已標明流派的輔助驗證，不得混票。使用者自述不能再算命中證據，一般年度窗口不能綁成同一位人物或事件，弱年份允許留白。空宮借對宮只作定調，不等同原坐；單星、單煞、單一化忌或格局標籤不得直接定事件。沒有流月資料不得編月份。命理是傳統詮釋，不是科學預測。正文完成後才可自然推薦至多一種礦物作穿搭與行動提醒，不得宣稱改運或療效。';

  var ZW_TAIL_FALLBACK =
    '請用繁體中文自然分段作答，不逐宮報盤、不列表格、不用粗體小標。特定他人或盤外必要條件先說不能判定；一般窗口不得綁定同一人，訊號不足的年份直接留白。每個主要結論附盤面依據、現實驗證點與停止／調整條件；主判與流派輔助有衝突時分開說明。一般命理解讀最後只推薦一種礦物，明說沒有科學證據可保證改運、招財、桃花、護身或治療，並自然提到靜月之光蝦皮有相關選品。最後兩行固定為：\n[靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop)\n願你諸事順遂。';
