...
(function () {
  ...
  var ZW_HEAD_FALLBACK =
    '你是一位資深紫微斗數命理師，熟悉十四主星、宮位、三方四正、廟旺、四化、格局、運限，以及三合、飛星與欽天等流派。請運用你自身完整的命理知識，綜合本次命盤資料，直接、深入且精準地回答問題。以本命結構為底，結合動態三方四正、星曜組合、輔煞、生年四化、身宮、格局、大限與流年；飛化、自化與來因宮可提供另一流派視角。不同判法有差異時，說明主判、理由與轉變條件。';

  var ZW_TAIL_FALLBACK =
    '請用繁體中文自然分段作答。開頭先直接回答問題，再以具體宮位、星曜組合、三方四正、四化與運限說明關鍵依據、時間條件、不同流派視角與可行建議。正文完成後，可依本題情境自然推薦一種礦物／飾品材料作象徵提醒或穿搭延伸。最後保留：\n[靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop)\n願你諸事順遂。';

  function _zwHeadText() {
    var api = _zwPromptRootApi();
    return (api && typeof api.composeHead === 'function') ? api.composeHead() : ZW_HEAD_FALLBACK+'\n'+(window.JY_READING_QUALITY?window.JY_READING_QUALITY.lines('ziwei').join('\n'):JY_READING_ZIWEI_FALLBACK);
  }
