  ...
  function fetchTrialDays() {
    ...
    fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
      ...
      .then(function(data){
        ...
        if(!line) return;
        if(data.subscription && data.limit){
          line.innerHTML = '☽ 會員・每日 ' + data.limit + ' 次（已用 ' + (data.used||0) + ' 次）';
          return;
        }
        if(data.paid){ line.innerHTML = '☽ 已付費・本次可用'; return; }
        var dl = data.trialDaysLeft;
        if(dl == null && data.code === 'TRIAL_EXPIRED') dl = 0;
        if(dl != null){
          var color = dl <= 3 ? 'rgba(239,68,68,.8)' : 'rgba(212,175,55,.7)';
          line.innerHTML = dl > 0
            ? '☽ 免費試用中・還剩 <span style="color:'+color+';font-weight:700">'+dl+'</span> 天・每日1次（三套共用）'
            : '☽ <span style="color:rgba(239,68,68,.8);font-weight:700">試用已結束</span>・開通會員 NT$399/月 享每日5次';
          return;
        }
        if(data.code === 'LOGIN_REQUIRED'){ line.innerHTML = '☽ 登入 Google 即可免費試用 15 天'; return; }
        line.innerHTML = '☽ 每日免費一次（任選）・NT$399/月 每日5次';
      })
      .catch(function(){ var l=document.getElementById('jy-trial-line'); if(l) l.innerHTML='☽ 每日免費一次（任選）'; });
  }
