  ...
  function fetchTrialDays() {
    ...
    fetch(url, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
      ...
      .then(function(data){
        ...
        var fl = data.freeLeft;
        if(fl == null && data.code === 'TRIAL_EXPIRED') fl = 0;
        if(fl != null){
          line.innerHTML = fl > 0
            ? '☽ 免費體驗中・剩餘 <span style="color:rgba(212,175,55,.7);font-weight:700">'+fl+'</span> 次'
            : '☽ <span style="color:rgba(239,68,68,.8);font-weight:700">免費體驗已用完</span>・NT$29 單次 或 NT$599/月 每日3次';
          return;
        }
        if(data.code === 'LOGIN_REQUIRED'){ line.innerHTML = '☽ 登入 Google 即可免費體驗 3 次'; return; }
        line.innerHTML = '☽ 首 3 次免費體驗・NT$599/月 每日 3 次';
      })
      .catch(function(){ var l=document.getElementById('jy-trial-line'); if(l) l.innerHTML='☽ 首 3 次免費體驗'; });
  }
