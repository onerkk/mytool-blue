...
(function (root) {
  ...
  var themes = {
    ...
    compat: {name:'八字與紫微斗數合盤',world:'celestial-observatory',room:'雙星之境',en:'WHEN TWO WORLDS MEET',title:'相遇，從理解彼此開始。',intro:'兩個人帶著不同的時間與故事而來。先看見彼此，再談如何靠近。',focus:'為彼此，留一束光。',guide:'輕觸兩個座標。你們各自完整，也能一起尋找相處的方法。',action:'兩束光，照見相遇的地方',ready:'一起看見，關係裡的可能。',outro:'四柱與十二宮，分別展開兩人的需要、牽動與時間節奏。',finish:'展開兩人的合盤',type:'seals',seals:['甲方','乙方']},
  ...
  function play(kind,options){
    ...
    if(mode==='cards'){
      ...
      interaction='<div class="jr-seals">'+cfg.seals.map(function(label,i){return kind==='bazi'?timeSeal(label,i):kind==='compat'&&root.JYRelationshipUI?root.JYRelationshipUI.seal(label,i):'<button type="button" class="jr-seal" data-seal-index="'+i+'" aria-pressed="false" aria-label="點亮'+label+'座標"><span>'+label+'</span><small>'+String(i+1).padStart(2,'0')+'</small></button>';}).join('')+'</div>';
    }else{
