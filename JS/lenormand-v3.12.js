// ═══════════════════════════════════════
// 靜月之光 — 雷諾曼牌 Lenormand v3.12
// v3.12(2026/6/19・index v86_35)：針對實機仍顯示舊版的根因重做，不沿用上一版失效做法。
//   ①載入根治：改用全新實體檔名 lenormand-v3.12.js，不只改 query string；即使 CDN／主機忽略查詢參數，
//     也不可能再命中舊 lenormand.js。window.__LENORMAND_BUILD__ 可直接確認實際載入版本。
//   ②五張線根治：DOM 本身拆成兩個獨立列容器，上列只產生 3 張、下列只產生 2 張；
//     不再依 nth-child、flex-wrap 或瀏覽器自動換行，所以結構上無法排成 4+1。
//   ③Gemini 根治：AI 捷徑先在同一點擊事件內做同步 copy，再直接導覽；不等待 Promise、不 window.open、
//     不 render、不切 phase。Android 以 Chrome intent 強制開 Gemini 網頁版，並保留 HTTPS fallback，避免 App Link 閃退返回。
...
(function () {
'use strict';
window.__LENORMAND_BUILD__ = 'v3.12-20260619';
console.log('[Lenormand] 靜月之光 雷諾曼牌 v3.12 loaded — DOM 固定 3+2／Gemini 強制網頁版／獨立檔名破快取');
...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  // ── v3.10 載入收斂：所有「依題型/牌陣才載入」的條件集中在這一個 _ctx，全函式唯一真相源。 ──
  //   根因（前版問題4）：特殊題型各自 inline regex、應期無條件硬載、GT 各自判斷——條件散在多處＝
  //   不一致、補一個漏一個（典型 whack-a-mole）。改走單一 _ctx 後，每個區塊只讀它，增刪區塊只動這裡。
  //   根因（前版問題3）：與當題無關的大塊（應期、5張以上才用的距離法）對短盤一律壓上＝稀釋關鍵指令、
  //   把它們推進 U 型曲線中段。Lost-in-the-Middle/ManyIFEval/RECAST：脈絡越雜、指令越多，遵循越差。
  var _q = String(question || '');
  var _ctx = {
    q: _q,
    count: (drawn || []).length,
    isTiming: /什麼時候|幾時|多久|何時|哪一年|哪個月|幾月|時間點|來得及|等多久|還要等|快了嗎|應期|多快|多晚|哪天|今年.*嗎|近期/.test(_q),
    isChoice: /還是|或是|或者|哪個|哪一個|要不要|該不該|二選一|二擇一|選哪|to be/i.test(_q),
    isInner:  /想法|想什麼|在想|心裡|意圖|打算|隱瞞|瞞|可信|可不可信|信不信|喜不喜歡|喜歡我|在乎|在意|暗中|背著|偷|誠實|真心|是不是真|有沒有人/.test(_q)
  };

  lines.push('你是精通 Petit Lenormand（小雷諾曼）的正統雷諾曼占卜師。以下是問卜者抽到的牌面與完整資料，請依雷諾曼組合義、線讀、九宮格或大牌陣規則解讀。');
  lines.push('');
  lines.push('★鐵律（先記這三條，其餘都是工具）：①第一句直接回答問卜者問的那一題。②只引用本盤抽到的牌，盤外牌名一個都不准出現。③壞牌照講不包裝（棺材＝結束、鐮刀＝切斷、山＝阻礙、老鼠＝消耗）。');
  lines.push('');
  ...
  // 問題
  lines.push('【問卜者性別】' + (declaredGender === 'male' ? '男——人物牌歸屬適用「性別已知」：紳士(28)若被抽出且未另指他人，優先讀成問卜者本人；淑女(29)＝對象或重要女性。' : declaredGender === 'female' ? '女——人物牌歸屬適用「性別已知」：淑女(29)若被抽出且未另指他人，優先讀成問卜者本人；紳士(28)＝對象或重要男性。' : '未聲明——人物牌歸屬適用「性別未知」分支（見下方人物牌歸屬規則）。'));
  lines.push('');
  lines.push('【問卜者的問題】');
  ...
  // ════ 特殊題型（依 _ctx 才載入，避免無關規則稀釋）════
  var _spLines = [];
  if (_ctx.isChoice) {
    _spLines.push('・問「A 還是 B」或「多選一」（價格/功效/外觀、工作壓力/睡眠/情緒…）：本盤是隨機抽牌，牌面沒有替這些選項貼標籤，嚴禁硬把某張牌指定成「A」或某個選項。正確做法：①若主軸牌性質明顯吻合某選項（如核心是雲＝心思混亂→偏「情緒」；核心是太陽＝亮眼曝光→偏「外觀」），就說它偏向那個並附牌；②若沒有任何牌對得上給的選項，就老實說「牌面不是落在這幾個選項，而是指向○○」，給方向不硬挑。A/B 題同理：讀整體基調指出「哪一種性質的選擇」較吻合（如主軸是止損就選風險低、能止血的那邊），不替 A、B 編故事。');
  }
  if (_ctx.isInner) {
    _spLines.push('・問「對方的想法/意圖/是否隱瞞/是否可信/有沒有人暗中…」這類第三方內心：有抽到人物牌（紳士28/淑女29）就以它代表對方、讀其相鄰牌與距離；沒有人物牌時，用情緒/狀態牌呈現對方在這題上的傾向（心＝有感情、雲＝對方自己也不清楚、蛇＝有算計或不老實、鳥＝在談論/猶豫）。只講牌面顯示的傾向與態度，不可編造對方具體的內心獨白、對話或牌面沒有的細節；訊號不足就說不足。');
  }
  // 人物牌歸屬：任何盤都可能出現人物牌（大牌陣必然有），一律保留
  _spLines.push('・人物牌歸屬（抽出紳士28或淑女29時，凡涉及人物先做這個判斷）：①問卜者已聲明性別（見上方【問卜者性別】）或已選 28/29 當指示牌＝性別已知——抽出的那張若就是指示牌本身＝問卜者本人；另一張人物牌＝對象或他人。②問卜者沒有以 28/29 自指（性別未知）——抽出的人物牌歸屬未定：正文必須先講明「這張可能是你本人、也可能是你身邊的某位女性／男性，牌面沒有給定」，之後的解讀兩種可能都要照顧到；嚴禁未經聲明直接斷成「另一個女人／另一個男人」。');
  if (_spLines.length) {
    lines.push('【特殊題型讀法（按問題形狀調整，仍以組合義為本）】');
    for (var _spi = 0; _spi < _spLines.length; _spi++) lines.push(_spLines[_spi]);
    lines.push('');
  }

  // ════ 應期（時間）——僅時間題載入四法，非時間題只留防呆一行，免得逼身分題硬湊時間 ════
  if (_ctx.isTiming) {
    ...
    lines.push('【應期】這題問的不是時間，不要硬湊應期或時間範圍。除非牌面明確指向快慢，否則一句話帶過即可（要給就用距離或速度牌，禁止寫出沒有牌可溯源的數字）。');
  }
  lines.push('');
  ...
  if (spreadId === 'three') {
    ...
    lines.push('鏡像對照：再把第1張↔第3張（兩端外牌）當一組讀，看起點與終點怎麼呼應——這是三張線的核心技法，不可略過；正文至少要有一句把這兩張放進同一句裡對照。');
    ...
    lines.push('鏡像對照（雷諾曼五張線核心技法，兩組都要讀）：卡2↔卡4（內層鏡像——緊貼焦點的兩股力量互相補充/對照）、卡1↔卡5（外層鏡像——最遠的起點 vs 終點/背景對照）。可驗證規格：每組至少要在正文出現一句、把該組兩張牌放進同一句裡對照（白話講、不點術語）；某組真的讀不出對照就明說「這兩股力量在本盤沒有明顯呼應」，不可默默跳過。');
    ...
    lines.push('Significator（代表牌）：' + _sigLb + ((_lnSignif && _lnSignif !== 28 && _lnSignif !== 29) ? '——主題指示牌：以它代表「這件事」而非人；方位、距離、圍繞讀法與人物指示牌相同' : (!declaredGender && _lnSignif !== 28 && _lnSignif !== 29 ? '（問卜者未聲明性別，此為暫定定位——正文開頭要提醒「本盤以' + _sigLb + '代表你；若性別不符，建議聲明性別後重抽會更準」）' : '')));
    if (_lnSignif && _lnSignif !== 28 && _lnSignif !== 29) {
      lines.push('⚠ 上面這張是問卜者自選的「定位錨點」，不必然等於這題的標準主題牌。回答的主軸一律以【主題牌】與下方【主題牌定位】為準；若自選代表牌的方向跟主題牌不一致，以主題牌為主、代表牌只當輔助視角，可白話點出差別（例：自選的牌偏「改變/搬遷」，但真正在回答這題的是工作那組牌）。只有該題主題牌一張都沒抽到，才回到代表牌當主軸。');
    }
    lines.push('');
  ...
  lines.push('3. 像跟朋友講話，不像寫報告。不要粗體標題分類。全程繁體中文（台灣用語），嚴禁混入任何簡體字。同一結論只講一次——同一組牌、同一個論點不可換句話重講，後面每段都要推進新資訊。');
  ...
  if (isGT && drawn.length === 36) {
    ...
    lines.push('⚠ 末排 4 張 cartouche 是置中的獨立收束區：它與第 4 排之間【沒有】上下相鄰或對位關係（置中欄位對不準，文獻上 cartouche 也是獨立單元）——cartouche 牌只在 cartouche 內部彼此左右相鄰；嚴禁宣稱「某主盤牌壓在 cartouche 牌上方」或反向。cartouche 與主盤的關聯只能用主題牌核對與整體基調。');
    ...
    if (_si >= 0) {
      if (_si >= 32) {
        lines.push('Significator ' + _sigLb + ' 落在末排 cartouche（第' + (_si-31) + '張），屬收束/命運性背景區——正文第一段內必須向客人明講「這題落在收束／命運性的區域，代表局面有一部分不在你手上」（用白話講，不用術語），漏說＝破功；之後再回主盤核對，不硬塞回去。');
      } else {
    ...
    // v3.9：移除「問題正則挑主題牌」的自然語言解讀層——治本，不再打補丁。
    //   舊機制(v3.2~v3.8)用關鍵字正則比對問題、決定預算哪些主題牌格位；但自然語言無窮、正則永遠補不完
    //   （實測連續漏接「天職」「好感/正緣」「愛上/發展空間」＝典型 whack-a-mole），且額外注入與當題無關的牌
    //   反成干擾源（文獻：過量/不相關脈絡降低推理、模型難自行濾除；自然語言解讀層隨條件數指數退化）。
    //   正解＝確定性結構化真相（上方「實際版面」已無條件給全 36 張 2D 座標）＋由模型做語意挑選（判主題、定位）。
    //   實證：本批多盤正則未命中，AI 仍照版面把所有主題牌位置讀對＝版面才是真正的防呆，正則是冗餘脆弱層。
    lines.push('【主題牌定位——自行對照上方版面判斷】依問卜者的問題判斷主題（感情/工作/財/健康/升遷/溝通/旅行/運勢…），對照前面【主題牌】列的對應牌，到上方「實際版面」找出本盤實際抽到的相關主題牌；感情/關係題另須找出兩張人物牌(28紳士、29淑女)做雙代表牌連線。一律照上方 2D 版面的實際格位判斷它們的八方緊鄰、以及離指示牌的遠近與左右（左過去右未來、上檯面下根基），勿憑編號順序或牌義相近硬湊；版面沒相鄰就是沒相鄰。');
    lines.push('');
  }
  ...
  // v3.10：把問題＋最硬鐵律複述在最末端（query-at-end／近因效應）。
  //   依據：Lost-in-the-Middle（TACL 2024，U 型：開頭/結尾最強、中段最弱）。
  //   ⚠ Anthropic「query-at-end +30%」原文限定 20K+ token 多文件檢索；本提示詞約 3–4K、屬指令密集，
  //   不在那個區間——此處走的是「精實提示詞＋頭尾複述」的位置效應，不假借長文件的數字。
  lines.push('');
  lines.push('══════════════════════════════════════════');
  lines.push('現在開始解讀・最後確認（這是你真正要回答的）');
  lines.push('══════════════════════════════════════════');
  if (question && question.trim()) {
    lines.push('問卜者的問題：' + question.trim());
  } else {
    lines.push('問卜者未指定具體問題，請依牌面做通用解讀，涵蓋目前最需要注意的面向。');
  }
  lines.push('答題前最後確認（最常被忽略的三條）：');
  lines.push('1. 第一句直接回答這個問題，不鋪陳、不複述問題本身。');
  lines.push('2. 只答這一題，沒問到的領域（財／感情／工作…）不主動展開。');
  lines.push('3. 壞牌照講（棺材＝結束、鐮刀＝切斷、山＝阻礙、老鼠＝消耗），不包裝成勵志或行銷。');
  lines.push('現在用白話、像有經驗的占卜師當面跟客人聊，開始解讀：');
...
function _getWrap() {
  if (!_lnWrap) {
    ...
    css.textContent = [
      ...
      '#ln-screen .ln-five-layout{display:flex!important;flex-direction:column!important;align-items:center!important;justify-content:flex-start!important;gap:.62rem!important;width:100%!important;max-width:100%!important;margin:.7rem auto .85rem!important}',
      '#ln-screen .ln-five-row{display:grid!important;align-items:start!important;justify-content:center!important;gap:.5rem!important;width:max-content!important;max-width:100%!important;margin:0 auto!important}',
      '#ln-screen .ln-five-row-top{grid-template-columns:repeat(3,68px)!important}',
      '#ln-screen .ln-five-row-bottom{grid-template-columns:repeat(2,68px)!important}',
      '#ln-screen .ln-five-row .ln-card{width:68px!important;min-width:68px!important;max-width:68px!important;margin:0!important;justify-self:center!important}',
      '@media(max-width:350px){#ln-screen .ln-five-row-top{grid-template-columns:repeat(3,62px)!important}#ln-screen .ln-five-row-bottom{grid-template-columns:repeat(2,62px)!important}#ln-screen .ln-five-row .ln-card{width:62px!important;min-width:62px!important;max-width:62px!important}}',
...
function _lnCardMarkup(c, index) {
  var imgSrc = IMG_MAP[c.id] || '';
  var out = '<div class="ln-card" style="animation-delay:' + (index * 0.05) + 's">';
  if (c._presetSig) out += '<div style="font-size:.6rem;color:#e8d28a;letter-spacing:.12em;margin-bottom:2px">★ 指示牌</div>';
  if (imgSrc) out += '<img class="ln-card-img" src="' + imgSrc + '" alt="' + c.name + '">';
  out += '<div class="ln-card-name">' + c.id + '. ' + c.name + '</div>';
  out += '<div class="ln-card-en">' + c.en + '</div></div>';
  return out;
}

function _lnGeminiHref() {
  var web = 'https://gemini.google.com/app?hl=zh-TW';
  if (/Android/i.test(navigator.userAgent || '')) {
    return 'intent://gemini.google.com/app?hl=zh-TW#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=' + encodeURIComponent(web) + ';end';
  }
  return web;
}
...
function _render() {
  ...
  if (_lnPhase === 'input') {
    ...
    if (_lnResolved === 'five') {
      // v3.12：不是用 CSS 猜換行，而是 DOM 就只有「上列3張」與「下列2張」兩個容器。
      h += '<div class="ln-five-layout" role="group" aria-label="五張線：上排三張，下排兩張">';
      h += '<div class="ln-five-row ln-five-row-top" role="row">';
      for (var j=0; j<3 && j<_lnDrawn.length; j++) h += _lnCardMarkup(_lnDrawn[j], j);
      h += '</div><div class="ln-five-row ln-five-row-bottom" role="row">';
      for (var k=3; k<5 && k<_lnDrawn.length; k++) h += _lnCardMarkup(_lnDrawn[k], k);
      h += '</div></div></div>';
    } else {
      h += (_lnResolved === 'nine') ? '<div class="ln-grid-3x3">' : '<div class="ln-cards-row">';
      for (var m=0; m<_lnDrawn.length; m++) h += _lnCardMarkup(_lnDrawn[m], m);
      h += '</div></div>';
    }

    // AI card
    h += '<div class="ln-ai-card"><div class="ln-ai-title">🌙 AI 深度解讀</div>';
    h += '<div class="ln-ai-desc">點擊 AI 圖示會先複製提示詞，再直接開啟該 AI 網頁版。</div>';
    h += '<button class="ln-ai-copy-btn" onclick="_lnCopy()">✦ 一鍵複製占卜提示詞 ✦</button>';
    h += '<div class="ln-ai-grid">';
    for (var a=0;a<AI_LIST.length;a++) {
      var ai = AI_LIST[a];
      var aiHref = ai.id === 'gemini' ? _lnGeminiHref() : ai.url;
      var aiTarget = ai.id === 'gemini' ? '_self' : '_blank';
      // Gemini 在 Android 使用 Chrome intent 強制網頁版；其餘 AI 維持原生新分頁。
      h += '<a class="ln-ai-sc" href="'+aiHref+'" target="'+aiTarget+'" rel="noopener noreferrer" onclick="return _lnOpenAI(\''+ai.id+'\',this)" aria-label="複製提示詞並開啟 '+ai.name+' 網頁版">';
      h += '<img src="ai-icons/ai-'+ai.id+'.png" alt="'+ai.name+'">';
      h += '<span>'+ai.name+'</span></a>';
    }
    h += '</div><div class="ln-ai-foot">Gemini 強制使用網頁版；本站不重繪、不延遲開窗</div></div>';
...
window._lnOpenAI = function(id, link) {
  if (!_lastPrompt || !link) return false;

  // v3.12：導覽前先在同一個 click call stack 內同步複製；絕不等待 Promise 才開頁。
  // 同步 copy 是主路徑，Clipboard API 只做非阻塞強化，不參與導覽時序。
  ...
  // 防止任何舊版 inline handler 或全域函式把點擊改成 render／window.open。
  // 這裡只校正 href/target，之後交還 <a> 的瀏覽器原生導覽。
  if (id === 'gemini') {
    link.href = _lnGeminiHref();
    link.target = '_self';
  }
  return true;
};
