...
(function () {
  ...
  var TPL = {
    ...
    tarot: {
      ...
      head: [
        ...
        '',
        '【輸出】',
        '使用繁體中文。第一句直接回答原問句，接著用白話給可行方向，最後簡短說明支持主判的關鍵牌組與轉變條件，避免逐張抄義。'
      ].join('\n'),
  ...
  function buildPrompt(tool, suppliedPayload) {
    ...
    return [
      ...
      (window.JY_READING_QUALITY?'':'【白話優先】先用兩三句日常語言回答最在意的問題，接著說明現在能做什麼，再用一小段補上支持主判的關鍵依據。正文以白話為主，少用術語；首次使用術語時立刻翻成生活意思。分析要仔細，但不要把逐項推演、每宮每星或整套技法清單都寫給讀者；使用者要求詳細技法時再展開。每個實質子題仍須回應，不用縮短篇幅掩蓋資料不足。建議要具體、做得到，可附一句實際能說出口的話，以及何時依哪些現實訊號調整。語氣溫和直接，不責怪命主、不貼宿命標籤，不只說保持正向。'),
      ...
      '【判讀校核】分析時核對「本盤具體牌位／牌組 → 相互修飾或牽制 → 對原問題的含義 → 成立條件」。用全盤比較最有力的替代解讀；區分描述現況、預期發展和建議行動，不把建議牌當成事情已發生。每個子題均回應，不能確認的部分明說依據不足。對讀者只保留主判的簡短依據，不逐張或逐項展示推演。',
