...
(function () {
  ...
  var FRAG_CRYSTAL =
    ...
    '寫法：你是「靜月之光」解讀者，對客人（不是對店主）順口多聊一句，不是廣告。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句你懂這顆石的知識點顯出挑貨有標準。蝦皮連結放在全文最後：https://tw.shp.ee/9UHEJTp4 ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（句號黏上去連結會點不動）。\n' +
