...
(function () {
  ...
  var DOMAIN_HINT = {
    ...
    wealth:  '財運/財務——核心看金幣＋數字階段（金幣三穩步、五破財、十圓滿）。缺金幣＝想法不踏實。✗ 別把聖杯多讀成人脈財。是非題給「會不會進帳/虧損」的明確方向；被問到具體金額而牌面無對應＝老實說「方向是X，但這個數字牌面沒給」。⚠ 純機率開獎題（統一發票／樂透／刮刮樂／賭博）：塔羅只給你的狀態與傾向（會不會過度期待、有沒有進帳訊號），不是隨機開獎的保證——講「牌面傾向X＋為什麼」，別斷成「必中／必不中／訊號不成立」；金額門檻（如過萬）牌面對不上就老實說沒給、只給方向（此為現代實務 forecast≠prediction 框架，非古典原典）。',
  ...
  var DOMAIN_HINT_MATHERS = {
    wealth:  '財運/財務——金幣牌一律用「→」後的 Mathers 原典義讀，不可套「金幣三＝穩步、五＝破財、十＝圓滿」這類 RWS 現代義（本盤金幣五正位在 Mathers 是「戀人／愛」、金幣十正位是「家／家庭」，與現代相反）。缺金幣＝想法不踏實。✗ 別把聖杯多讀成人脈財。是非題給「會不會進帳／虧損」的明確方向；被問具體金額而牌面無對應＝老實說「方向是X，但這個數字牌面沒給」。⚠ 純機率開獎題（統一發票／樂透／刮刮樂／賭博）：塔羅只給你的狀態與傾向（會不會過度期待、有沒有進帳訊號），不是隨機開獎的保證——講「牌面傾向X＋為什麼」，別斷成「必中／必不中／訊號不成立」；金額門檻（如過萬）牌面對不上就老實說沒給、只給方向（此為現代實務 forecast≠prediction 框架，非古典原典）。',
  ...
  var FRAG_CRYSTAL =
    '\n【收尾・能量石（自然帶出，禁推銷腔）】\n' +
    '整份解讀寫完後，用最後 2-3 句輕收一個尾。能量石的選擇必須綁本盤資料區的花色張數（火／水／風／土四個，大牌不算元素）：張數最多的＝過盛候選、最少的＝匱乏候選，挑「對這次結論影響最大」的那一個元素，對應一種可隨身的能量石。⚠ 若四個花色張數很接近（最多與最少相差 ≤1，例如 5/4/4/4）就不准硬扣某元素過盛或不足——老實說「這盤元素大致均衡」，改挑一種最呼應「這次問題＋你給的結論」的石（結論要你篩人、靠判斷＝清明定心的白水晶或紫水晶；結論是情感受傷要復原＝粉晶或月光石），理由直接綁問題與結論，不要假裝成元素失衡。\n' +
