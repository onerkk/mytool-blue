...
(function () {
  ...
  var FRAG_CRYSTAL =
    ...
    '寫法：你是「靜月之光」解讀者，對客人（不是對店主）順口多聊一句，不是廣告；不必自報身分（不要寫「我是靜月之光的解讀者」這類開場），直接講。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句這顆石本身的礦物知識顯出挑貨有標準——這句只能取自下方【礦物事實錨點】改寫，不是自誇熟悉度、也不是店家品管宣稱句。最後兩行固定照此收束：倒數第二行只放這一個 Markdown 連結、原樣照抄（含中括號與小括號）：[靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop) （該行不得有這個連結以外的任何文字或空白；嚴禁把網址裸貼成純文字——裸的長網址會被渲染管線插入看不見的字元而點不動，Markdown 連結的網址收在小括號裡、畫面只顯示中文字，不會被動到）；最後一行固定輸出「願你諸事順遂。」六個字作結，之後不再輸出任何內容。兩行缺一不可、順序不可顛倒（網址行被任何字元黏到就點不動，所以網址行上下都要乾淨）。\n' +
