/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v80.60]
...
(function () {
  ...
  var SPREAD_METHODS = {
    ...
    five_card: "本次牌陣：五牌陣（5 張）\n【排法出處】現代實務牌陣，無單一古典文獻；用 RWS 位置義、正逆位、圖像敘事與 Golden Dawn 元素互動做嚴格分析。\n讀法：現況→原因→阻礙→建議→結果。結果定走向；建議位的牌必須轉成至少一條由該牌推出的具體行動並自然點名該牌，不可只把它當支持/反對的證據用掉。5 個位置都要用到，但輸出串成故事。",
  ...
  // v84_audit6(2026/6/10)：二選一 A/B 對應注入——問題能機械切出兩個選項就直接定義 A/B（實測 AI 曾被迫自己
  //   發明映射、位置牌被挪作主題證據）；切不出則強制第一句先明講對應。多選項題已在 tarot_upgrade 改派五牌陣。
  function _eitherOrABNote(q) {
    try {
      q = String(q || '');
      var parts = q.split(/還是|或者/);
      if (parts.length === 2) {
        var lhs = parts[0].replace(/[？?。！!，,、\s]+$/, '');
        var segs = lhs.split(/[、，,]/).filter(function(s){ return s.trim().length > 0; });
        var A = segs[segs.length - 1].trim()
          .replace(/^(我們|我|你|他|她)?(到底|究竟)?(該|要|想|會|應該|打算)?/, '');
        if (A.indexOf('是') >= 0) A = A.split('是').pop();
        A = A.trim();
        var B = parts[1].replace(/[？?。！!，,、\s]+/g, '').replace(/(嗎|呢)$/, '').trim();
        if (A && B && A.length >= 1 && A.length <= 12 && B.length <= 12 && segs.length <= 2) {
          return '\n【A/B 對應（依問題語序，最優先）】A＝「' + A + '」、B＝「' + B + '」。第一句就用這個對應直接回答，位置牌一律讀在 A/B 各自的位置上。';
        }
      }
    } catch (e) {}
    return '\n【A/B 對應】本題選項無法機械切分：你必須在第一句先明講你把 A、B 各對應到問題中的哪個選項，再依位置比較；問題若含 A/B 之外的其他選項，用整體牌面基調回應它們，不可假裝牌陣有替它們留位置。';
  }
  function getSpreadMethod(q) {
    try {
      ...
      if (id && SPREAD_METHODS[id]) {
        ...
        if (id === 'either_or') _m += _eitherOrABNote(q);
        return _m;
      }
    } catch(e){}
  ...
  var DOMAIN_HINT = {
    love:    '感情/關係——核心看聖杯（情感連結）與權杖（行動慾）。缺聖杯＝可能是「想要」不是「愛」；缺權杖＝對方沒主動能量。✗ 別把金幣多讀成財運、權杖多讀成工作。涉及第三人先查有沒有宮廷牌再推畫像。是非題定傾向前，先把「支持」與「反對」的牌各清點一次再權衡：不可把自足／審慎類牌（如金幣九＝對方自足、不急、有辨識力，甚至不一定在找對象）說成「對方對你有意思」；宮廷牌的不利特質（如權杖國王「通常已婚」＝對方可能已有對象）要照讀、不可只挑討喜特質；凶牌（塔、金幣五逆等）密集、聖杯又薄時，傾向就往保守收，不可靠單一吉訊（如一張聖杯六逆）硬撐成樂觀結論。',
  ...
  var FRAG_SOURCELOCK_TAROT =
    '\n【學理鎖定】只用三種知識：①本盤實際數據 ②可查文獻作為歷史根據：RWS 採 Pamela Colman Smith 圖像與 Golden Dawn／Book T 的元素與占星對應；牌義關鍵字（正位與逆位皆同）採現代 RWS 通行義——基於 PCS 圖像的業界慣例，與 Waite《Pictorial Key》1910 字面占義常不同（如權杖二 Waite 原書是「財富榮華或病痛悲傷」二擇、非「規劃遠見」），故不宣稱逐字原典；需要逐字 Waite 原典請改用純-Waite 模式 ③已明示為現代實務的輸出框架。禁止：把現代詮釋當古典原典、現代網紅塔羅、把心理學框架硬套（「逆位＝陰影面需療癒」）、靈性雞湯、無牌面支撐的道德勸說。原典沒明說處用「實務上我會這樣看」，不說「正統必然」。\n';
  // ★ 塔羅・純-Waite 模式：一律只用 Waite 原書正/逆義，逆位照原文（與現代否定式常相反），不疊 GD
  var FRAG_SOURCELOCK_TAROT_WAITE =
    '\n【學理鎖定・純-Waite 模式】本次牌義一律只用 A.E. Waite《The Pictorial Key to the Tarot》(1910) 原典占卜義：每張牌「→」後就是 Waite 原書「Divinatory Meanings／Reversed」的忠實中譯，正位用正位義、逆位用逆位義，直接照讀，不可改用現代義。⚠ 逆位照 Waite 原文，常與現代否定式邏輯相反——例：命運之輪逆＝增長／豐盛／過剩（非「衰退停滯」）；聖杯六逆＝未來／更新／即將到來（非「困在過去」）；權杖八逆＝嫉妒／內訌／口角（非單純「延遲」）——一律以 Waite 原文為準。不疊 Golden Dawn 元素尊嚴／旬（Decan）／卡巴拉，那是別的系統、Waite 占卜篇未用。時間只能從牌義本身的時序線索推（Waite 明言塔羅宜判「處於哪個階段」而非精確日期），不可用花色速度或 Decan 硬安月份。RWS 圖像（PCS 場景）可作敘事輔助；但逆位牌引用 RWS／PCS 圖像時，意義仍以 Waite 逆位義為準，不可被正位場景圖像（如錢幣五雪中行乞）帶回正位義。建議／應對位若落到凶牌（塔、寶劍類、惡魔等），仍只用該牌的 Waite 原義轉成「該防範、該避開、該預備什麼」的告誡（例：塔＝提防突如其來的崩壞與欺瞞、勿把事情建在不實期待上），不可改用現代象徵（如塔讀成「拆掉舊我／打破幻象」）去湊正向建議——那是現代義，純-Waite 不允許。宮廷牌除推人物類型外，先讀其 Waite 占卜義本身；感情題中 Waite 標的關係狀態是關鍵訊號（如權杖國王「通常已婚」），出現在走向／對方位時必須讀出（可能已婚或已有對象＝對能否發展的現實保留），不可只取「成熟主動」類型畫像而略過 Waite 明文的關係線索。禁止：現代網紅詮釋、心理學框架、靈性雞湯、無牌面支撐的道德勸說。\n';
  ...
  function buildRecencyTarot() {
    ...
    return '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
      ...
      + mathersClause
      + ' □ 正位吉牌與逆位凶牌一視同仁，不可只挑符合你預設結論的牌講、把相反訊號的牌跳過不讀——這不只指滿盤逆位中少數的正位牌，也包括全盤多數正位時、落在「原因／資源／下一步」等位的正位有力牌（例：權杖皇后正位在原因位＝你自身魅力與領導力是現狀成因，不可因結論偏謹慎就略過），與逆位凶牌落在好結果位；位置制牌陣每個位置的牌都要明確點到名、無一遺漏 □ 能量石只作品牌收尾且不冒充原典\n';
  }
  ...
  var FRAG_CRYSTAL =
    ...
    '整份解讀寫完後輕收一個尾，挑一種可隨身能量石。選石綁資料區花色張數（火／水／風／土，大牌不算）：先讀「花色分布」數字確認各幾張（憑數字不憑印象）。判定分兩種盤：\n' +
    '・極小盤（四花色牌總數≤3，如三牌陣）：樣本太小，「過盛」不判——只看缺席花色與崩解訊號：有崩解訊號可走天鐵／龍宮舍利收尾；否則在缺席花色中挑對這次結論影響最大的補；都不適用就依「這次問題＋結論」挑。\n・小盤（四花色牌總數 4～7，如五牌陣）：百分比無意義，直接用張數判——某花色 0 張＝匱乏、某花色佔花色牌總數一半以上＝過盛；缺與盛並存時挑對這次結論影響最大的那個元素配石；兩者都沒有＝均衡。\n' +
    '・大盤（四花色牌總數≥8）：按「佔四花色的比例」判——某花色約佔四成以上＝過盛、約一成以下＝匱乏，挑對這次結論影響最大的那個元素配石。\n' +
    '判為均衡時（大盤如 5/4/4/4、4/4/3/3）就老實說均衡——門檻沒到就是均衡，不可用「偏少／略缺／偏弱」這類說法繞過門檻改走補元素路線——改挑最呼應「這次問題＋結論」的石（要篩人靠判斷＝白水晶／紫水晶；情感受傷要復原＝粉晶／月光石）。\n' +
    '元素對照（缺的補、過盛的衡，挑一種）：火不足→紅瑪瑙/太陽石/紅碧玉、火過盛躁進→黑曜石/黑碧璽；水不足→粉晶/草莓晶/月光石、水過盛情緒淹沒→拉長石/海藍寶；風不足→白水晶/海藍寶/藍紋瑪瑙、風過盛心緒亂→紫水晶/紫龍晶；土不足→黃水晶/茶晶/虎眼石、土過盛僵固→綠幽靈/葡萄石。牌面崩解訊號密集時（以本盤每張牌實際標的牌義為準，如毀滅／破產／重大損失／悲苦類；不可拿牌名硬套——例如本盤若標「惡魔＝好的命定」就不算崩解訊號）→天鐵（辟邪定志）或龍宮舍利（護身安神）。\n' +
    '寫法：你是「靜月之光」解讀者，對客人（不是對店主）順口多聊一句，不是廣告；不必自報身分（不要寫「我是靜月之光的解讀者」這類開場），直接講。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句這顆石本身的礦物知識顯出挑貨有標準——這句只能取自下方【礦物事實錨點】改寫，不是自誇熟悉度、也不是店家品管宣稱句。蝦皮連結獨立成行、放在全文最後一行：https://shopee.tw/a50h95648d?tab=shop ——該行只能有網址本身，網址前後不得有任何文字、標點、引號、符號或空白，網址後也不得再輸出任何內容（黏上任何字元連結會點不動）。\n' +
    ...
    '禁止：每盤都推同一顆石、理由與本盤花色無關、照抄固定模板句，以及「優惠／特價／限時／下單／搶購／快買」等字眼。牌面沉重時語氣穩重，把石頭定位成「陪你穩住」。\n';
  ...
  var FRAG_CRYSTAL_ZIWEI =
    ...
    '寫法：你是「靜月之光」解讀者，對客人順口多聊一句，不是廣告；不必自報身分，直接講。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句這顆石本身的礦物知識——這句只能取自下方【礦物事實錨點】改寫，不得自行編造。能量石是品牌實務輔助，不屬紫微原典。蝦皮連結獨立成行、放在全文最後一行：https://shopee.tw/a50h95648d?tab=shop ——該行只能有網址本身，網址前後不得有任何文字、標點、引號、符號或空白，網址後也不得再輸出任何內容。\n' +
    '\n【礦物事實錨點——礦物知識句只能取自此表改寫，不得自行外加成因、成分或產地】\n' +
    '石英家族（白水晶/紫水晶/黃水晶/茶晶/粉晶/草莓晶）＝二氧化矽、三方晶系、硬度7：紫水晶含鐵經天然輻照致色；黃水晶鐵致色；茶晶含鋁受天然輻射呈煙色；粉晶多霧狀半透、全透極少；草莓晶＝內含纖鐵礦/赤鐵礦片狀包體。玉髓類（紅瑪瑙/藍紋瑪瑙/紅碧玉）＝隱晶質石英：瑪瑙看天然色帶層次；紅碧玉不透明、鐵氧化物致色。長石族：月光石＝正長石與鈉長石交層生暈彩，看暈彩乾淨度與內含物；拉長石＝斜長石、三斜晶系、看變彩面積；太陽石＝內含赤鐵礦/銅片生砂金閃光。海藍寶＝綠柱石族、六方晶系、鐵致色。黑曜石＝火山玻璃、非晶質、貝殼狀斷口。黑碧璽＝電氣石族、三方晶系、柱面縱紋。紫龍晶＝紫色纖維狀絲絹光澤、產俄羅斯查拉河流域。虎眼石＝石英交代石棉假象、絲絹貓眼光。綠幽靈＝白水晶內含綠泥石包體。葡萄石＝斜方晶系、常呈葡萄狀集合體。天鐵＝鎳鐵隕石（鐵鎳金屬、等軸晶系）：表面常見氣印、切磨酸蝕後現魏德曼花紋；它是金屬不是玻璃——含氣泡的天然玻璃是似曜岩（俗稱玻璃隕石），兩者是不同東西、嚴禁混述。龍宮舍利＝市場名稱、成因與成分說法不一：礦物句只談外觀挑選（珠體圓整、皮殼天然完整、結構緻密），不得宣稱地質成因。\n' +
  ...
  function buildFocusLock(q, tool) {
    ...
    f.domains.slice(0, 3).forEach(function (d) { var h = (_hintMath && DOMAIN_HINT_MATHERS[d]) ? DOMAIN_HINT_MATHERS[d] : DOMAIN_HINT[d]; if (h) L.push('・' + h); });
    // ★ v85.1 根治：多領域同時注入時，兩段規則對同一花色的讀法可能互斥
    //   （例：感情題規則「金幣讀現實條件不讀財運」 vs 財運規則「金幣五破財/十圓滿」）。
    //   不裁衝突會讓 AI 自行選邊；明定主軸優先序，衝突就地解決。
    if (f.domains.length > 1) {
      L.push('・【多領域優先序】本題偵測到多個領域，以上面第一段領域為主軸，其餘領域提示只作輔助；兩段規則對「同一花色怎麼讀」有衝突時，一律以主軸領域為準（例：感情為主軸時，金幣依感情題規則讀現實條件、投入與身分牽扯，不讀財運進帳與破財）。');
    }
