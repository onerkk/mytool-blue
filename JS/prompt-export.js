...
(function () {
  ...
  var FRAG_CRYSTAL =
    ...
    '整份解讀寫完後輕收一個尾，挑一種可隨身能量石。選石綁資料區花色張數（火／水／風／土，大牌不算）：先讀「花色分布」數字確認各幾張（憑數字不憑印象）。按「佔四花色的比例」判——某花色約佔四成以上＝過盛、約一成以下或 0～1 張且總數≥10＝匱乏，挑對這次結論影響最大的那個元素配石。若最多不到四成、最少又超過一成（如 5/4/4/4、4/4/3/3）＝元素大致均衡，就老實說均衡、改挑最呼應「這次問題＋結論」的石（要篩人靠判斷＝白水晶／紫水晶；情感受傷要復原＝粉晶／月光石）。\n' +
    ...
    '寫法：你是「靜月之光」解讀者，對客人（不是對店主）順口多聊一句，不是廣告；不必自報身分（不要寫「我是靜月之光的解讀者」這類開場），直接講。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句這顆石本身的礦物知識（晶系、色帶、淨度、共生礦這類）顯出挑貨有標準——不是自誇熟悉度、也不是店家品管宣稱句。蝦皮連結放在全文最後：https://shopee.tw/a50h95648d?tab=shop ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（句號黏上去連結會點不動）。\n' +
