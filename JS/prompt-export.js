/*! prompt-export.js — 靜月之光 Golden Dawn Book T 提示詞匯出引擎 [v99.2]
 *  v99.2（未知人物共指修正＋開鑰連續程序閘門＋自然選品收尾 2026/7/18）：
 *    1) 查詢圖逐子問句建 event，附屬人物屬性以條件共指承接；未量測子題不再污染可回答子題。
 *    2) 塔羅與開鑰統一讀同一正庫存候選，開鑰不再走舊 crystalCatalog。
 *    3) 商品收尾改為承接原問句與已成立主判的獨立「延伸選品」段落，保留空行與自然文案，禁止功效保證。
...
(function () {
      ...
      if (!contract && typeof E.compileReadingSpec === 'function') {
        if (tool === 'ootk') {
          contract = E.compileReadingSpec({
            ...
            referenceDate: obj.readingDate || obj.referenceDate || new Date().toISOString()
          });
        } else {
          ...
          contract = E.compileReadingSpec({
            ...
            methodPlan: td.methodPlan || obj.methodPlan || null,
            cards: td.cards || [],
            sourceProfile: 'gd_book_t',
            waitePure: false,
            knownCounterpart: (typeof E.inferExplicitCounterpartBinding === 'function' ? E.inferExplicitCounterpartBinding(q) : false),
            referenceDate: td.readingDate || td.referenceDate || obj.readingDate || new Date().toISOString()
          });
        }
      }
  ...
  function buildRootQuestionLock(question, tool) {
    ...
    return [
      ...
      BAR,
      '原問句：' + question,
      '應用程式已把原句編譯為型別化查詢圖，資料區會列出主體、事件主體、尺度、比較子、固定門檻、模態、期限、否定與排除等必要原子。不得重新分類、合併必要原子、改寫門檻或把完整原句再次當成單一原子。',
      '先核對 completedQueryGraph 的 round-trip、逐原子 deletion-sensitivity 與 no-added-premise；若契約標示 invalid 或方法 coverage 不完整，停止完整裁決並明示缺口，不得自由改題。',
  ...
  function buildRecencyTarot() { return "\n────────────────────────────\n交稿前語義稽核\n────────────────────────────\n□ 已逐字保留原句比較雙方、關係、門檻、模態與期限 □ 已分開精確值、範圍、相對排序、門檻跨越、趨勢與穩定度 □ 第一個裁決沒有重新定義成功或只回答較弱子命題 □ synthesis_only 單位只綜合 dependsOn 命題、未線性化依賴圖 □ 牌義來源釋義沒有被直接複製成事件結論 □ 情境例子已標成可能表現、沒有冒充事實 □ 每個位置與合法互動均已處理 □ 未由牌張數、牌號、宮廷牌數或聚合角色位換算數量 □ 全盤已完成反證競爭與語義飽和 □ 時間、人物與數字均可溯源 □ 只引用本盤合法牌名 □ 延伸選品只從資料區候選逐字選一項 □ 推薦文案已自然承接原問句與已成立主判，未宣稱功效或保證結果 □ 正文、文案、品項與賣場連結之間保留空行\n"; }
  var FRAG_RECENCY_OOTK = "\n────────────────────────────\n交稿前語義稽核\n────────────────────────────\n□ 已逐項回答原問句的資訊需求並建立證據矩陣 □ 所有實際生成的操作均已依Book T階段功能處理，程序停止後未補寫後續操作 □ 已採用資料區的適配、重試與中止結果 □ 每次操作均由落點、完整計數故事、兩側配對與牌力形成階段命題 □ 計數值、步數、落堆張數沒有被換算成現實數量 □ 代表牌反覆出現沒有被當成訊號 □ 不同操作沒有拼成假連線 □ 僅在五次操作完整且未停止時形成完整發展序列；不完整程序只裁決已生成階段 □ 現代附加觀察只作次證、沒有凌駕Book T程序 □ 時間只在資料可靠且問題需要時輸出 □ 所有不同有效命題已呈現、同義者已合併 □ 篇幅由有效命題決定 □ 只引用本盤實際牌與落點 □ 延伸選品只從資料區候選逐字選一項 □ 推薦文案已自然承接原問句與已成立主判，未宣稱功效或保證結果 □ 正文、文案、品項與賣場連結之間保留空行\n";

  // 塔羅／開鑰專用正庫存收尾：先完成占卜裁決，再以原問句與已成立結論自然承接一個品項。
  // 商品只作日常選品延伸，不是牌義證據，也不得宣稱功效或改變結果。
  var FRAG_TAROT_INVENTORY =
    "\n【延伸選品收尾——自然承接原問句，獨立於占卜裁決】\n" +
    "占卜正文、行動與停止條件全部寫完後，空一行，再輸出獨立的『延伸選品』段落。商品不得反向影響牌義、程序有效性或主判。\n" +
    "只能從資料區【可推薦庫存品項】逐字選一個正庫存品項；沒有候選時寫『推薦品項：無可用庫存品項』。不得自行改品名、尺寸或手圍，也不得推薦第二項。\n" +
    "文案必須融入使用者原問句：先用一句自然承接『你這次問的是什麼』，再接一個本盤已成立的核心提醒或行動重點，最後把選定商品寫成這個主題的日常配戴延伸。不得重複整篇結論，不得把產品說成牌面證據。\n" +
    "若查詢圖／方法／開鑰程序無法形成有效完整裁決，文案只能承接原問句的關注主題，並明說本次選品不代表牌面已作出結論；不得偽造主判。\n" +
    "若原問句涉及醫療、法律、犯罪、投資或人身安全，必須先完成現實證據與專業處理提醒；選品文案只能描述日常配戴或設計調性的延伸，不能暗示商品能診斷、治療、保證勝訴、獲利或確保安全。\n" +
    "禁止輸出價格、礦物功效、療效、招桃花／招財／改運／辟邪等保證、礦物知識、優惠話術、下單催促、第二個品項或與原問句無關的罐頭文案。商品含手圍時只能照品名列出，不得宣稱尺寸適合使用者，除非原問句或資料區明示其手圍。\n" +
    "固定排版如下，段落間各保留一個空行：\n\n延伸選品\n\n<一至兩句自然文案：承接原問句＋本盤已成立的核心提醒＋選品作為日常延伸；不得宣稱商品改變結果。>\n\n推薦品項：<品項全名>\n\n[前往靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop)\n" +
