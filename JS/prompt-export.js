/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v71.0]
 *  v71.0 改動（外科手術接全集，head 與風格一字未動，全部走 composition + 資料層）：
 *    A. 資料層治本：塔羅/開鑰資料區框架由「供參考、自行交叉驗證」改為「已精算、直接採用、勿重算」
 *       —— 尤其開鑰 counting 自算極易出錯，這是準確度最大槓桿。
 *    B. 禁幻覺：兩工具資料區都加「本次合法牌名清單」（替代複製模式失去的後端機械審計）。
 *    C. 全集高槓桿規則注入：FRAG_SOURCELOCK（學理鎖定）、FRAG_UNCERTAINTY（嚴格不確定判準）、
 *       FRAG_RECENCY_*（交稿前 recency 檢查，防後半段破功）。
 *    D. 問題路由強化：DOMAIN_HINT 加「✗ 不要主看」；新增 OOTK_ROUTING（Sig 應落堆/Op2 宮/Op4 旬主星）。
 *    E. buildFocusLock 加 window.JY_QUERENT 年齡/性別鉤子（宮廷牌年齡對照基準；無資料也不會壞）。
...
(function () {
  ...
  function formatOOTKData(result) {
    ...
    var L = [];
    L.push('── 以下五次操作資料（含 counting／pairing／dignities 計算）已由排盤系統精算完成，請【直接採用】。計數路徑、配對、元素尊嚴都已算好——不要自己重數、重排或重算位置（Book T counting 自己算極易出錯）；你的工作是依這些既定結果做解讀 ──');
    L.push('');
