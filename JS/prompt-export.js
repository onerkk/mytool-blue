...
(function () {
  ...
  var FRAG_CRYSTAL =
    '\n【收尾・能量石（自然帶出，禁推銷腔）】\n' +
    '整份解讀寫完後，用最後 2-3 句輕收一個尾。能量石的選擇必須綁本盤資料區的花色張數（火／水／風／土四個，大牌不算元素）：張數最多的＝過盛候選、最少的＝匱乏候選，挑「對這次結論影響最大」的那一個元素，對應一種可隨身的能量石。⚠ 動筆前先讀資料區「花色分布」數字，確認火／水／風／土各幾張、最多與最少各是哪個花色（憑數字不憑印象，絕不可把非最多的花色說成最多）。⚠ 過盛／匱乏要按「佔四花色總數的比例」判，不可用固定差值（牌陣張數從3到21不一，相差1在小牌陣顯著、在21張陣不顯著）：某花色約佔四花色總數四成以上＝過盛候選；約一成以下、或僅0～1張而總數已≥10＝匱乏候選；若最多的不到四成、最少的又超過一成（如 5/4/4/4、5/4/4/3、4/4/3/3）＝這盤元素大致均衡、不准硬扣某元素過盛或不足——老實說「這盤元素大致均衡」，改挑一種最呼應「這次問題＋你給的結論」的石（結論要你篩人、靠判斷＝清明定心的白水晶或紫水晶；結論是情感受傷要復原＝粉晶或月光石），理由直接綁問題與結論，不要假裝成元素失衡。\n' +
    ...
    '→ 每句都不同、都有一個「為什麼我挑這個」的知識點、連結自然帶入。\n⚠ 鐵則：石頭只挑一種、只講一次；理由必須對得上本盤實際花色張數（過盛或匱乏的那個元素），或——若四花色大致均衡——對得上「這次問題＋結論」；不可不管張數一律推同一顆石（例如每盤都推紫水晶）。禁止照抄上面任何範例句。\n';
