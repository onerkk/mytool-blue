...
(function () {
  ...
  var FRAG_RECENCY_TAROT =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 牌陣張數與位置讀對 □ 每個位置都已用於回答問題但未逐格報告 □ 只引用本盤合法牌名 □ 壞消息沒有包裝 □ 第一段已直接回答問題 □ 有說明原因、阻礙、時間、24小時行動、可驗證信號 □ 人物/年齡/外貌只在牌面足夠時才推 □ 不把現代牌陣說成古典原典 □ 不重複同一結論 □ 每排收尾牌與居中牌都已明確讀到並用上（Mathers 21張：第7／14／21張為各排收尾、第11張居中須單獨讀，不可與鄰牌混讀一句帶過；horseshoe 各組居中張同理） □ 正位吉牌與逆位凶牌一視同仁，不可只挑符合你預設結論的牌講、把相反訊號的牌跳過不讀——這不只指滿盤逆位中少數的正位牌，也包括全盤多數正位時、落在「原因／資源／下一步」等位的正位有力牌（例：權杖皇后正位在原因位＝你自身魅力與領導力是現狀成因，不可因結論偏謹慎就略過），與逆位凶牌落在好結果位；位置制牌陣（凱爾特十字、小阿卡那專題、關係牌陣）每個位置的牌都要明確點到名、無一遺漏 □ 能量石只作品牌收尾且不冒充原典\n';
