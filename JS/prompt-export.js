/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v80.12]
 *  v80.12（塔羅牌陣正統分層補齊 + 自動分流提示詞加固）：
 *    1) 補齊一張牌、七張馬蹄、大阿卡那22路徑、Waite 42、Waite 35、Mathers 66 的提示詞讀法。
 *    2) 明確聲明「自動判斷牌陣」是現代實務分流，不屬古典原典。
 *    3) 大型文獻牌陣依 Waite / Mathers 原文方向與用途標示，避免混用。
...
(function () {
  ...
  var SPREAD_METHODS = {
    ...
    one_card: "本次牌陣：一張牌（1 張）\n【排法出處】現代快問/今日指引牌陣，無單一古典文獻；只用 RWS 圖像、Waite 牌義與正逆位做單點判斷。\n讀法：只回答一個核心訊號，不能擴寫成通盤。是非題可給傾向，但若牌面不足要明說不足。",
    horseshoe: "本次牌陣：七張馬蹄形（7 張）\n【排法出處】近現代 GD/RWS 實務常用牌陣，非 Mathers 1888 第一法。\n讀法：過去→現在→隱藏影響→建議→他人態度→阻礙→結果。此牌陣適合中等複雜問題；結果位定方向，建議位給 24 小時內能做的事。不可把它說成 Mathers 原典。",
    major_arcana_22: "本次牌陣：大阿卡那二十二路徑（22 張，只用大牌）\n【排法出處】Hermetic Qabalah 22條路徑與22張大阿卡那的系統應用；不是 Waite/Mathers 固定牌陣。\n讀法：只看命運骨架、靈魂課題與大轉折，不拿來判很瑣碎的日常細節。每條路徑都要用到，但正文只深講與問題最直接的路徑，其他作背景。",
    waite_42: "本次牌陣：Waite 四十二張替代法（42 張）\n【排法出處】A.E. Waite《Pictorial Key to the Tarot》Part III §8：An Alternative Method of Reading the Tarot Cards。古典文獻牌陣。\n讀法照 Waite：六行七張，由右至左讀。適用於沒有明確問題、想看生命/命運大趨勢，或已指定某段時間。先快速掃整體趨勢，再回頭細讀；不要硬把每一行改成現代宮位。",
    waite_35: "本次牌陣：Waite 三十五張補充法（35 張）\n【排法出處】A.E. Waite《Pictorial Key to the Tarot》Part III §9：The Method of Reading by Means of Thirty-five Cards。古典文獻牌陣。\n讀法照 Waite：用42張後剩下未用牌再分六行，行長 7/6/5/4/2/11；第一行環境，第二行本人/主體，第三行外部事件，第四行意外，第五行慰藉/緩和，第六行只用來釐清前面謎點。全體由左至右讀。此法是追問/釋疑，不是第一手通盤。",
    mathers_66: "本次牌陣：Mathers 第三法六十六張（66 張）\n【排法出處】S.L. MacGregor Mathers《The Tarot》(1888) Methods of Divination 第三法。古典文獻牌陣。\n讀法照 Mathers：代表牌置中，66張成拱形/三角；1–11與34–44看過去，23–33與56–66看現在，12–22與45–55看未來。先按過去/現在/未來三段讀，再與代表牌、對稱牌合讀。原文最後可從棄用11張抽2張作 surprise 結論；若資料區沒有提供 surprise 牌，不可硬編。",
  };
