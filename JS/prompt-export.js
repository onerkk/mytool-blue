...
(function () {
  ...
  function buildRecencyTarot() { return "\n────────────────────────────\n交稿前語義稽核\n────────────────────────────\n□ 已逐字保留原句比較雙方、關係、門檻、模態與期限 □ 已分開精確值、範圍、相對排序、門檻跨越、趨勢與穩定度 □ 第一個裁決沒有重新定義成功或只回答較弱子命題 □ synthesis_only 單位只綜合 dependsOn 命題、未線性化依賴圖 □ 牌義來源釋義沒有被直接複製成事件結論 □ 情境例子已標成可能表現、沒有冒充事實 □ 每個位置與合法互動均已處理 □ 未由牌張數、牌號、宮廷牌數或聚合角色位換算數量 □ 全盤已完成反證競爭與語義飽和 □ 時間、人物與數字均可溯源 □ 只引用本盤合法牌名 □ 庫存推薦只從資料區候選逐字選一項，且僅輸出品項\n"; }
  var FRAG_RECENCY_OOTK = "\n────────────────────────────\n交稿前語義稽核\n────────────────────────────\n□ 已逐項回答原問句的資訊需求並建立證據矩陣 □ 五次操作均已依Book T階段功能處理 □ 已採用資料區的適配、重試與中止結果 □ 每次操作均由落點、完整計數故事、兩側配對與牌力形成階段命題 □ 計數值、步數、落堆張數沒有被換算成現實數量 □ 代表牌反覆出現沒有被當成訊號 □ 不同操作沒有拼成假連線 □ 第一次至第五次已形成發展序列並裁決完整命題 □ 現代附加觀察只作次證、沒有凌駕Book T程序 □ 時間只在資料可靠且問題需要時輸出 □ 所有不同有效命題已呈現、同義者已合併 □ 篇幅由有效命題決定 □ 只引用本盤實際牌與落點 □ 庫存推薦只從資料區候選逐字選一項，且僅輸出品項\n";
  ...
  var FRAG_TAROT_INVENTORY = "\n【庫存推薦附加層——全文最後兩行】\n占卜裁決完成後，只能從資料區【可推薦庫存品項】逐字選一個品項。正文最後固定輸出兩行：第一行：推薦品項：<品項全名>；第二行：[靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop)。不得輸出價格、礦物功效、礦物知識、推銷文、第二個品項或推薦理由；資料區沒有候選品項時，第一行輸出：推薦品項：無可用庫存品項。第二行仍須原樣保留賣場連結。這兩行不得反向影響牌義與裁決，第二行之後不得再有任何內容。\n";
