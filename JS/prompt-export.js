...
(function () {
  ...
  var FRAG_CRYSTAL =
    ...
    '寫法：你是「靜月之光」解讀者，對客人（不是對店主）順口多聊一句，不是廣告；不必自報身分（不要寫「我是靜月之光的解讀者」這類開場），直接講。只挑一種、只講一次（嚴禁「或」「這類」並列第二種）、貼著這次結論說清「為什麼對你是它」，並帶一句這顆石本身的礦物知識（晶系、色帶、淨度、共生礦這類）顯出挑貨有標準——不是自誇熟悉度、也不是店家品管宣稱句。蝦皮連結放在全文最後：https://tw.shp.ee/9UHEJTp4 ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（句號黏上去連結會點不動）。\n' +
