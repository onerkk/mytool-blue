/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v80.67]
 *  v80.67（依歐那指示，七套提示詞一律移除「結尾徵兆」與「限時即刻行動」兩項公式化收尾）：
 *    動到：梅花 head 輸出要求、開鑰 head（人設一句＋邊界現代擴充清單＋輸出要求兩項）、
 *    梅花與開鑰兩條 recency 勾選項；雷諾曼(lenormand.js)刪回答規則 6/7、應期落地改為 6；
 *    靈籤(oracle.js)規則 7 只留行動建議與時間誠實、完整性清單對應去除。各 tail 的
 *    「可被現實檢核的解讀」形容詞與一般行動方向皆保留；八字/紫微/塔羅 head 本無此兩項，未動。⚠ 本檔 ?v= 升 v92_trim。
 *  v80.66（七套人設口吻自然化——以「八字命理師」那套為範本拉齊）：
 *    塔羅／開鑰／紫微／梅花 四套 head 人設改寫成「鮮明但精簡的口吻人設」：
 *    具體角色＋一句口吻對比示例＋壓縮後核心護欄（先給答案／技法留心裡不上課／
 *    有依據沒就說不足／壞消息直說不嚇人不雞湯不推），不加裝飾性背景。
 *    依 persona-prompting 研究：人設對「語氣風格」最有效，但越具體越好、無關細節越多越傷準度，
 *    故只放領域相關的口吻特徵＋短示例。雷諾曼(lenormand.js)、靈籤(oracle.js)同步改；
 *    八字(bazi-standalone.js)人設本已是此風格、當範本保留未動。⚠ 本檔 ?v= 升 v91_voice。
 *  v80.65（逆位框架鬆綁——治本，非打補丁）：
 *    學理鎖定原寫死「逆位＝削弱/受阻/內化」單一公式，會跟資料區逐張逆位關鍵字打架——惡魔逆/聖杯五逆等其實是『釋放/反轉』非削弱，AI 曾因此把惡魔逆(資料義=解脫)讀回正位的『被綁住』。改為逆位義一律以資料區該牌逆位關鍵字為準（減弱/受阻/內化/釋放/反轉依牌而定），不機械套單一公式；保留『現代慣例、與 Waite 1888 字面常不同、不宣稱逐字』那層。
...
(function () {
  ...
  var FRAG_RECENCY_MEIHUA =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 第一段已直接回答問題 □ 本卦／互卦／變卦／動爻／體用都已檢查 □ 沒有混入塔羅、開鑰、七維或姓名學 □ 沒把沒有提供的外應硬編成事實 □ 有說明阻礙、時間 □ 壞消息沒有包裝 □ 最後有研究娛樂提醒\n';
  ...
  var FRAG_SOURCELOCK_TAROT =
    '\n【學理鎖定】只用三種知識：①本盤實際數據 ②可查文獻作為歷史根據：RWS 採 Waite《Pictorial Key》牌義與 Pamela Colman Smith 圖像、Golden Dawn／Book T 的元素與占星對應；逆位採現代通行義，且以資料區每張牌的逆位關鍵字為準——逆位可能是減弱、受阻、內化、釋放或反轉，依該牌而定，不要機械套用單一「削弱」公式去蓋過資料給的逆位義（例：惡魔逆＝掙脫／看穿幻象、聖杯五逆＝走出悲傷，都是釋放不是削弱）；此採現代慣例、與 Waite 1888-式字面逆位義常不同，故不宣稱逐字原典③已明示為現代實務的輸出框架。禁止：把現代詮釋當古典原典、現代網紅塔羅、把心理學框架硬套（「逆位＝陰影面需療癒」）、靈性雞湯、無牌面支撐的道德勸說。原典沒明說處用「實務上我會這樣看」，不說「正統必然」。\n';
