...
(function () {
  ...
  var FRAG_RECENCY_OOTK =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 五層內部檢查都已完成 □ 正文沒有輸出 Op/Sig/Counting/Pairing 等術語 □ 沒把主牌每層必出現當重複訊號 □ 沒硬湊跨層重複牌 □ 沒用正逆位取代開鑰主判斷 □ 計數走過的牌有串成歷程（不只報終點） □ 倒數那層的配對與計數有實際用上 □ 五層關鍵牌花色集中或分散有判斷 □ 配對有讀元素強弱（加持／削弱） □ 真實場域有具體落地、不是用「例如」帶過 □ 資料厚的層有深講沒跳過 □ 有具體時間錨 □ 有說明真實場域、阻力、行動、可驗證信號 □ 現代實務擴充有誠實標示，不冒充 Crowley 原典 □ 只引用本盤實際出現的牌與落點 □ 能量石只作品牌收尾、理由綁這次牌面、沒照抄範例原句\n';
