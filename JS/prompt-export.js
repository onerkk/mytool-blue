/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v80.7]
 *  v80.7（暗戀否定問句防反向過度斷言）：
 *    1) 問「都沒人暗戀我嗎／是不是沒人喜歡我」時，禁止把「好感跡象」反向寫成「不是都沒人／有人暗戀」。
 *    2) 第一段固定切分：能證實的只有注意/好感跡象；明確暗戀若不足就直接說不足。
 *    3) 交稿檢查新增否定問句保守輸出，避免把跡象升格成已證實事實。
 *  v80.6（暗戀/喜歡題防過度斷言修正）：
 *    1) 新增「暗戀/喜歡/有人注意我」硬限制：注意、好感、情緒反應 ≠ 明確暗戀。
 *    2) 單一宮廷牌、社交牌、情緒牌不得單獨斷成有人暗戀；需聖杯/宮廷/吸引牌互證且未被阻礙/結果位壓住。
 *    3) 公司/第三人題若阻礙位出現社交、群體、規範、防備或逆位結果，必須保守表述為「好感跡象/情緒性注意」，不說成確定暗戀。
 *  v80.5（第三方感情狀態防過度斷言修正）：
 *    1) 新增「單身/有男友女友/有對象」硬限制：必須區分能證實與不能證實。
 *    2) 戀人逆位不得單獨推成已有伴侶；防備/封閉牌只能表示不透明，不等於有對象。
 *    3) 問第三方關係狀態時，避免把「牽扯可能」講成「不是乾淨單身」。
...
(function () {
  ...
  // ── v80.5：第三方感情狀態題防過度斷言 ──
  // 目標：問「某人是否單身／有男友女友／有對象」時，避免把戀人逆位、隱者、權杖七等
  //       只能代表不透明/防備/失衡的牌，過度寫成「不是乾淨單身」或「一定有對象」。
  function isThirdPartyRelationshipStatus(q) {
    var s = String(q || '');
    return /(單身|有沒有男友|有男友|男朋友|女朋友|有對象|已有對象|有伴侶|已婚|結婚|交往中|是不是有人|是否有對象)/.test(s)
      && !/(我.*單身|自己.*單身|我有沒有|我是否)/.test(s);
  }

  var FRAG_THIRD_PARTY_REL_STATUS =
    '・第三方感情狀態硬限制：本題若問某人是否單身／是否有男友女友／是否有對象，第一句必須區分「牌面能證實」與「牌面不能證實」。沒有明確伴侶、承諾、同居、婚姻、穩定關係或第三人牌面互證時，不得斷「一定有對象」「不是乾淨單身」「一定單身」。戀人逆位只能讀成關係選擇不清、感情狀態失衡、價值衝突或牽扯可能，不可單獨證實已有伴侶；權杖七、隱者、月亮、寶劍類防備/封閉/不透明訊號，也只能說不好確認，不等於有男友/女友。最穩妥表述是：「本盤不能證實完全單身，也不能證實已有穩定對象；只能看出感情狀態不透明／有防備／不易確認。」';


  // ── v80.6：暗戀/喜歡題防過度斷言 ──
  // 目標：問「有沒有人暗戀/喜歡/注意我」時，避免把單一宮廷牌、社交牌或情緒牌
  //       過度寫成「有一位異性暗戀你」。暗戀必須由感情/吸引/人物/行動互證支持。
  function isCrushQuestion(q) {
    var s = String(q || '');
    return /(暗戀|喜歡我|愛慕|偷偷喜歡|對我有好感|對我有意思|注意我|在意我|有人喜歡|有人對我|異性.*喜歡|異性.*暗戀|桃花.*我)/.test(s);
  }

  var FRAG_CRUSH_LIMIT =
    '・暗戀/好感題硬限制：本題若問「有沒有人暗戀我／某人是否喜歡我／公司是否有人對我有意思」，第一句必須區分「注意/好感跡象」與「明確暗戀」。只有宮廷牌、聖杯牌、戀人/太陽/月亮等感情或吸引訊號互相支撐，且沒有被阻礙位或結果位明顯壓住時，才能說「明確暗戀」。若只有單一宮廷牌、社交牌或情緒牌，不得斷成已經暗戀；只能說「有注意、好感跡象或情緒性關注，但未達明確暗戀」。第三人或公司場域題，若阻礙位出現社交/群體/規範/防備牌，或結果位逆位、退縮、失控、缺乏行動力，必須保守判斷，不可硬說對方會主動。若無宮廷牌或人物牌，不可硬推年齡、外貌、身分或指定一個人。若問題用否定句問「都沒人暗戀我嗎／是不是沒人喜歡我／完全沒人對我有意思嗎」，第一句禁止寫成「不是都沒人」「有人暗戀你」這種把跡象反向升格成事實的句子；必須用「不一定；本盤只能證實/不能證實……」格式，例如：「不一定；本盤只能看出有注意或好感跡象，但不能證實明確暗戀。」或「不一定；本盤連明確好感跡象都不足，不能證實有人暗戀。」';

  function isCrushNegativeQuestion(q) {
    var s = String(q || '');
    return isCrushQuestion(s) && /(都沒人|沒人|沒有人|沒有.*暗戀|沒有.*喜歡|都沒有|完全沒人|完全沒有|沒.*對我有意思)/.test(s);
  }

  var FRAG_CRUSH_NEGATIVE_LIMIT =
    '・暗戀否定問句硬限制：本題若問「都沒人暗戀我嗎／是不是沒人喜歡我」這類否定句，第一句不可為了回答「不是」而寫成「不是都沒人」「有人暗戀你」。那會把「注意/好感跡象」誤升格成已證實事實。第一句必須採保守裁決格式：「不一定；本盤只能看出X，不能證實Y。」其中 X 只能是牌面能支持的注意/好感/情緒性關注，Y 是明確暗戀、指定人物、主動追求。若連 X 都不足，要直接說「本盤連明確好感跡象都不足」。';
  ...
  var FRAG_RECENCY_TAROT =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 牌陣張數與位置讀對 □ 每個位置都已用於回答問題但未逐格報告 □ 只引用本盤合法牌名 □ 壞消息沒有包裝 □ 第一段已直接回答問題 □ 有說明原因、阻礙、時間、24小時行動、可驗證信號 □ 人物/年齡/外貌只在牌面足夠時才推 □ 第三方單身/有對象題已區分證實與推測，沒有把不透明說成一定有伴侶 □ 暗戀/好感題已區分注意/好感跡象與明確暗戀，沒有把單一宮廷/社交/情緒牌放大成確定暗戀 □ 暗戀否定問句沒有把「不是都沒人」寫成「已證實有人暗戀」 □ 不把現代牌陣說成古典原典 □ 不重複同一結論 □ 能量石只作品牌收尾且不冒充原典\n';
  ...
  function buildFocusLock(q, tool) {
    ...
    if (isThirdPartyRelationshipStatus(f.raw)) {
      L.push(FRAG_THIRD_PARTY_REL_STATUS);
      L.push('・這類問題不是問「對方愛不愛問卜者」，也不是問「你們能不能交往」；不要把感情失衡、曖昧、不透明擴寫成已有固定伴侶，除非牌面有明確互證。');
    }

    if (isCrushQuestion(f.raw)) {
      L.push(FRAG_CRUSH_LIMIT);
      if (isCrushNegativeQuestion(f.raw)) L.push(FRAG_CRUSH_NEGATIVE_LIMIT);
      L.push('・這類問題不是問「對方會不會告白」或「你們能不能在一起」；若牌面只有被看見、社交互動、情緒波動或群體場合，先判為注意/好感跡象，不可直接升格成確定暗戀。');
    }
