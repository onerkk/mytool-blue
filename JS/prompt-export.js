...
(function () {
  ...
  var FRAG_RECENCY_TAROT =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 依牌陣類型讀對位置張數 □ 組合讀非逐張報義 □ 無粗體標題 □ 凶牌逆位沒包裝成正面 □ 每個問題正面回答 □ 只引用盤上的牌 □ 三層因果鏈至少走了 2 條 □ 數據層清單全覆蓋 □ 同一結論沒有重複兩次以上 □ Card Counting 和 Triad 融進主敘述不獨立成段 □ 有 24h 動作＋可驗證信號 □ 能量石收尾\n';
  var FRAG_RECENCY_OOTK =
    '\n' + BAR + '\n交稿前檢查（後半段最容易破功）\n' + BAR +
    '\n□ 五個 Op 都給了 Counting＋Pairing＋Dignities＋Unaspected＋結論 □ 無粗體標題（禁「**表層：**」「**Op1：**」）□ 元素尊嚴至少 2 層有展開 □ Unaspected 每層都有點名或說明無 □ 跨層重複牌至少點出 2 組 □ 沒發明禁用說法（結論牌/答案牌/五層共振/同步度）□ 總答有依據且明說哪層權重最高 □ Op4 有具體日期錨 □ Op5 點出三角＋質點 □ 深層拆解從落點推非表層換說法 □ 結尾有可驗證信號 □ 能量石收尾\n';
  ...
  var FRAG_CRYSTAL =
    ...
    '・結尾必須像解讀者順口多聊一句，不是廣告區塊。每次用不同的切入角度，不要每篇都用同一句式。\n' +
    '・加一句你為什麼懂這顆石頭的「知識句」，讓人覺得你挑貨有標準。\n' +
    '・蝦皮連結放在最後，用自然語氣帶過，不要用固定模板。\n' +
    '\n✗ 錯誤（每次複製貼上同一句）：\n' +
