/*! prompt-export.js — 靜月之光 前端提示詞匯出引擎  [v80.4]
 *  v80.4（Lenormand 輸出口吻校正提示）：
 *    1) 補充註記：Lenormand 的來源等級／正統邊界只應作 AI 內部校驗。
 *    2) 占卜正文不得主動向問卜者說「這是現代實務牌陣」「不是 Game of Hope 原典」。
 *    3) 問卜者若問的是感情／財運／工作，只回答結果；正統性說明只在被追問時另答。
 *  v80.3（檔案責任＋雷諾曼官方邊界警示）：
 *    1) 再次明確：本檔不產生雷諾曼提示詞；雷諾曼主提示詞在 lenormand.js。
 *    2) Lenormand 沒有 Waite/Book T 式官方技術文件；若要純 Game of Hope 原始邊界，只能以36張大牌陣代表牌附近牌故事法為核心。
 *    3) 三張線／五張線／九宮格／房屋／騎士跳皆須標成現代實務，不得在任何檔案冒充官方原典。
 *
 *  v80.2（檔案責任釐清）：
 *    1) 本檔只負責塔羅快讀 tarot 與開鑰之法 ootk 的提示詞匯出。
 *    2) 雷諾曼 Lenormand 的提示詞主生成位置是 lenormand.js 的 buildPrompt()。
 *    3) 雷諾曼正統邊界、3×3／4×8＋4 真實相鄰資料、AI按鈕跳轉修正，請以 lenormand.js 為準。
