...
(function() {
  ...
  function _buildPaywallHTML(mode) {
    return '<div style="max-width:340px;width:90%;background:linear-gradient(145deg,#1a0a0a,#2a1515);border:1.5px solid rgba(212,175,55,.35);border-radius:18px;padding:2.2rem 1.5rem;text-align:center;box-shadow:0 24px 80px rgba(0,0,0,.6)">' +
      ...
      '<div style="display:flex;flex-direction:column;gap:.5rem;align-items:center">' +
        ...
        '<div style="font-size:.58rem;color:var(--c-text-muted,#7a7060);margin-top:-.1rem;opacity:.7">單次僅限信用卡</div>' +
        // ── 付款方式標籤 ──
  ...
  window._jyStartPayment = async function(mode, type) {
    ...
    try {
      ...
      if (payWin) {
        ...
        var labelText = type === 'opus_single' ? 'Opus深度解析 NT$' + opusDisplayPrice : (type === 'single' ? '單次解讀 NT$' + PRICE_SINGLE : '會員 NT$' + PRICE_SUB);
  ...
  window._jyCheckPaymentAndUnlock = async function() {
    ...
    try {
      ...
      if (data.paid) {
        ...
        // ★ v29：只有訂閱才存到期時間
        if (pending.type !== 'single' && pending.type !== 'opus_single') {
    ...
    var observer = new MutationObserver(function(mutations) {
      for (var i = 0; i < mutations.length; i++) {
        ...
        for (var j = 0; j < added.length; j++) {
          ...
          // ai-deep-wrap 裡的「今日解讀額度已用盡」或「今日免費次數已用完」
          if (txt.indexOf('今日解讀額度已用盡') >= 0 || txt.indexOf('今日免費次數已用完') >= 0) {
          ...
          // ★ v29：OOTK 429 — 攔截開鑰結果區的額度已用完
          if (txt.indexOf('今日免費次數已用完') >= 0) {
  ...
  function _injectPaidToken() {
    ...
    var clearObs = new MutationObserver(function() {
      ...
      if (hasResult) {
        ...
        if (_ptType !== 'opus_single' || window._jyOpusDepth) {
  ...
  function _checkPaymentReturn() {
    ...
    // ★ v29：從 pending 判斷是單次還是訂閱
    var pending = null;
    try { pending = JSON.parse(localStorage.getItem('_jy_pending_payment') || 'null'); } catch(_) {}
    var isSingle = (pending && (pending.type === 'single' || pending.type === 'opus_single'));
    ...
    banner.textContent = isSingle ? '✅ 付款成功！單次解讀已解鎖' : '✅ 付款成功！30 天會員已開通';
