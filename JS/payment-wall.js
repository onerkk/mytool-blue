...
(function() {
  ...
  function _buildPaywallHTML(mode) {
    ...
    return '<div style="max-width:340px;width:90%;background:linear-gradient(145deg,#1a0a0a,#2a1515);border:1.5px solid rgba(' + accent + ',.35);border-radius:18px;padding:2.2rem 1.5rem;text-align:center;box-shadow:0 24px 80px rgba(0,0,0,.6)">' +
      ...
      '<h3 style="color:var(--c-gold,#c9a84c);font-size:1.05rem;margin-bottom:.6rem;font-family:var(--f-display,serif)">' + (mode === 'ootk' ? '開鑰之法' : '今日免費額度已用完') + '</h3>' +
      ...
      '<div style="display:flex;flex-direction:column;gap:.5rem;align-items:center">' +
        '<button onclick="_jyStartPayment(\'' + mode + '\')" style="display:flex;align-items:center;justify-content:center;gap:6px;width:220px;padding:13px;border-radius:12px;background:linear-gradient(135deg,rgba(' + accent + ',.18),rgba(' + accent + ',.06));color:var(--c-gold,#c9a84c);font-size:.9rem;font-weight:700;border:1.5px solid rgba(' + accent + ',.45);cursor:pointer;font-family:inherit;box-shadow:0 4px 16px rgba(' + accent + ',.12)">' + icon + ' 解鎖' + label + ' NT$' + price + '</button>' +
