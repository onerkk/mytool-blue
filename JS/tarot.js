...
function showMH(r){
  ...
  // Keep the newly revealed hexagrams in normal reading order.
  // 起卦成功後鎖定所有起卦按鈕
  lockMhButtons();
}
...
function calcMhTime(){
  ...
  // v80.14 正統梅花時間起卦：必須使用農曆年月日；Lunar 未載入時停止，不用西曆硬代。
  let lunarY, lunarM, lunarD;
  try {
    if(typeof Lunar==='undefined'||!Lunar.Solar) throw new Error('Lunar.Solar 尚未載入');
    const solar=Lunar.Solar.fromYmd(y,mo,d);
    const lunar=solar.getLunar();
    lunarY=lunar.getYear(); lunarM=lunar.getMonth(); lunarD=lunar.getDay();
  } catch(e){
    alert('梅花時間起卦需要精準農曆轉換；請確認網路可載入 lunar-javascript 後重試。');
    console.error('[梅花時間起卦] 農曆轉換失敗，已停止:', e);
    return;
  }
...
var TAROT=[
  // ═══ 大阿爾克那 Major Arcana ═══
  {id:0,  suit:'major',rank:0,  num:0,  n:'愚者',    el:'風', astro:'天王星 | א Aleph / Path 11 (Kether→Chokmah) / Uranus / 元素風 / 母字 Aleph',
   kwUp:'自由·冒險·新旅程', kwRv:'魯莽·失根·逃避',
   up:'充滿無限可能，勇敢踏出第一步，新旅程即將展開。',
   rv:'魯莽行事，缺乏計畫，需要停下來想清楚再走。',
   loveUp:'新鮮的吸引與相遇機會正在醞釀；彼此是否想走近，仍看後續互動。',loveRv:'不願定義關係，逃避承諾，關係停在模糊曖昧而無法往前。',
   careerUp:'勇於嘗試新領域，跳槽或創業的衝動可能成真，打破框架反而打開機會。',careerRv:'衝動跳槽沒有規劃，頻繁換工作難以累積，缺乏職涯方向感。',
   wealthUp:'可以留意新的收入管道；在確認金額與風險前，先別把機會算成已入帳。',wealthRv:'衝動消費，財務缺乏規劃，容易因為隨興而破財。',
  ...
  {id:1,  suit:'major',rank:1,  num:1,  n:'魔術師',  el:'風',astro:'水星 | ב Beth / Path 12 (Kether→Binah) / Mercury / 雙字 Beth',
   ...
   wealthUp:'牌面偏向善用技能與資訊管理資源；投資表現仍要看實際數據與風險。',wealthRv:'需要核對交易資訊與承諾，別讓話術代替查證。',
  ...
  {id:2,  suit:'major',rank:2,  num:2,  n:'女祭司',  el:'水', astro:'月亮 | ג Gimel / Path 13 (Kether→Tiphareth) / Moon / 雙字 Gimel',
   kwUp:'直覺·秘密·等待', kwRv:'封閉·隱瞞·迴避',
   up:'傾聽直覺，靜待時機，答案在你內心深處。',
   rv:'過於封閉，忽視內心聲音，或刻意迴避要面對的事。',
   loveUp:'感情裡有尚未說清的部分，目前宜觀察互動；單靠這張牌還不能判定對方的好感。',loveRv:'感情資訊不夠清楚，先分開自己已知的事與尚待確認的猜測。',
   careerUp:'適合觀察和研究的時期，不要急著決定，靜待更多資訊浮現。',careerRv:'職場資訊不透明，先核對流程與消息來源，再判斷原因。',
   wealthUp:'保守觀望才是明智之舉，不是出手的時機，耐心等待。',wealthRv:'財務上有隱藏的風險尚未浮現，不要輕易相信表面數字。',
   healthUp:'心理和情緒健康需要重視，傾聽身體給你的細微訊號。',healthRv:'留意情緒與作息是否受影響；如有持續不適，按實際症狀尋求評估。',
  ...
  {id:3,  suit:'major',rank:3,  num:3,  n:'皇后',    el:'水', astro:'金星 | ד Daleth / Path 14 (Chokmah→Binah) / Venus / 雙字 Daleth',
   ...
   wealthUp:'牌面偏向滋養與資源累積；收入是否增加，仍要對照實際進帳。',wealthRv:'享受優先於儲蓄，花費超過收入，財務漸漸失衡。',
   healthUp:'這張牌提醒照顧身體與日常調養；健康狀態和婦科變化仍須以實際檢查為準。',healthRv:'檢視飲食與生活節奏是否失衡，別由牌面推斷體重或消化疾病。',
  ...
  {id:4,  suit:'major',rank:4,  num:4,  n:'皇帝',    el:'火', astro:'牡羊座 | ה Heh / Path 15 (Chokmah→Tiphareth) / Aries / 簡字 Heh',
  ...
  {id:5,  suit:'major',rank:5,  num:5,  n:'教皇',    el:'土', astro:'金牛座 | ו Vav / Path 16 (Chokmah→Chesed) / Taurus / 簡字 Vav',
   ...
   healthUp:'重視規律照護與可信賴的專業資訊；調養方式是否適合仍需依個人情況評估。',healthRv:'忽視身體警訊，或過度執著於某種健康教條。',
  ...
  {id:6,  suit:'major',rank:6,  num:6,  n:'戀人',    el:'風', astro:'雙子座 | ז Zayin / Path 17 (Binah→Tiphareth) / Gemini / 簡字 Zayin',
   kwUp:'選擇·真愛·連結', kwRv:'猶豫·衝突·三角關係',
   up:'重要的選擇時刻到來，不只是感情，是整個價值觀的抉擇。',
   rv:'猶豫不決，害怕選擇就意味著失去，導致兩頭落空。',
   loveUp:'感情的吸引與價值選擇成為主題；能否建立更深的關係，仍要看雙方互動與明確意願。',loveRv:'關係可能面臨選擇與價值分歧；先釐清實情，別直接推定有第三人。',
  ...
  {id:7,  suit:'major',rank:7,  num:7,  n:'戰車',    el:'水', astro:'巨蟹座 | ח Cheth / Path 18 (Binah→Geburah) / Cancer / 簡字 Cheth',
   kwUp:'意志·勝利·前進', kwRv:'失控·方向不明·強硬',
   up:'靠意志力和決心衝過難關，不是靠運氣而是靠行動力。',
   rv:'失去方向，意志力用在錯誤地方，需要重新定位目標。',
   loveUp:'有主動拉近距離的能量；是否真正推進關係，還要看對方回應。',loveRv:'感情中太強勢控制，或雙方方向不一致讓關係失衡。',
   careerUp:'勢如破竹地推進工作目標，執行力強，升遷或業績突破在望。',careerRv:'工作方向不明，忙碌卻沒有成果，需要重新校準目標。',
   wealthUp:'推動財務計畫的意願較強；先核對風險、資金安排與實際成果再決定。',wealthRv:'留意急於求成與資金風險；是否已有損失以帳目為準。',
   healthUp:'這張牌提醒循序建立活動與作息習慣；健康改善以身體反應為準。',healthRv:'過度勞累，用意志力透支身體，身體在罷工前你要先注意。',
  ...
  {id:8,  suit:'major',rank:8,  num:8,  n:'力量',    el:'火', astro:'獅子座 | ט Teth / Path 19 (Chesed→Geburah) / Leo / 簡字 Teth',
   ...
   wealthUp:'比起急進，更適合耐心檢查長期規畫的風險與成本；回報不能由牌面保證。',wealthRv:'因恐懼而不敢做出必要的財務決策，錯失機會。',
   healthUp:'牌面偏向耐心與自我照顧；真正的體能與恢復情況仍看實際狀態。',healthRv:'精神力量不足，容易被壓力擊垮，需要補充心理能量。',
  ...
  {id:9,  suit:'major',rank:9,  num:9,  n:'隱者',    el:'土', astro:'處女座 | י Yod / Path 20 (Chesed→Tiphareth) / Virgo / 簡字 Yod',
   ...
   healthUp:'可安排休息和減少消耗；需要哪種照護，仍要依症狀與專業評估決定。',healthRv:'孤獨導致情緒低落，長期封閉影響心理健康，需要尋求連結。',
  ...
  {id:10, suit:'major',rank:10, num:10, n:'命運之輪', el:'火', astro:'木星 | כ Kaph / Path 21 (Chesed→Netzach) / Jupiter / 雙字 Kaph',
   kwUp:'轉機·命運·週期', kwRv:'逆風·停滯·抗拒',
   up:'局勢有變動與轉機的象；能否把握，要看具體條件與行動。',
   rv:'逆風期，但輪子會轉，守住實力等待時機。',
   loveUp:'感情可能遇到變化或新的相遇；是否互相靠近，還需後續互動確認。',loveRv:'感情遇到循環性的障礙，相同問題反覆出現，需要打破模式。',
   careerUp:'事業有調整方向的可能；先核實機會、條件與決策進度。',careerRv:'事業低谷期，但這是週期性的，守住不要擴張，等輪子轉回來。',
   wealthUp:'財務條件可能出現變動，先核對收入、風險和標的資料，再決定是否投入。',wealthRv:'留意財務環境變化，檢查現金流與風險承受度後再調整。',
   healthUp:'健康管理可隨實際狀況調整；是否好轉，要以症狀與檢查結果確認。',healthRv:'身體狀態可能起伏，舊症是否復發須由實際症狀與醫療評估判定。',
   adviceUp:'把握可以核對的機會，先確認條件與風險，再決定下一步。',adviceRv:'低谷期低頭走，保存實力等翻盤，這段時間不要亂動。',
   spiritualUp:'業力循環的轉折點，前世種的因正在結果，順應比抗拒更有智慧。',spiritualRv:'抗拒命運的安排，同樣的課題反覆出現是因為你還沒學會。'},
  {id:11, suit:'major',rank:11, num:11, n:'正義',     el:'風', astro:'天秤座 | ל Lamed / Path 22 (Geburah→Tiphareth) / Libra / 簡字 Lamed',
   kwUp:'公正·因果·平衡', kwRv:'不公·偏見·逃避責任',
   up:'公正、責任與核對證據是此刻主題；結果仍要看實際程序。',
   rv:'不公正的狀況或者你自己在逃避應該承擔的後果。',
   loveUp:'感情需要公平對待彼此，公正的態度讓關係更穩固。',loveRv:'感情中有人在逃避責任或不公平地要求對方。',
   careerUp:'職場評價與規則成為重點；先檢查成果紀錄與評核標準。',careerRv:'職場有不公平的待遇，需要據理力爭，否則只會越來越吃虧。',
   wealthUp:'檢視合約、支出和回報的對應關係，不用牌面推定投資收益。',wealthRv:'財務糾紛或不合理的損失，需要積極處理，不能默默承受。',
   healthUp:'需要以檢查與專業判讀釐清症狀原因，再決定後續處理。',healthRv:'若對症狀或檢查結果有疑問，可再確認資訊；不能由牌面判斷是否誤診。',
  ...
  {id:12, suit:'major',rank:12, num:12, n:'吊人',     el:'水', astro:'海王星 | מ Mem / Path 23 (Geburah→Hod) / Neptune-Water / 母字 Mem(水)',
   ...
   wealthUp:'財務決定暫時宜重新檢視期限與流動性，等待本身不保證回報。',wealthRv:'金錢被凍住，流動性出問題，需要想辦法解套。',
   healthUp:'暫停高強度活動，充分休息是為了走更遠，這段時間好好養。',healthRv:'若有持續或加重的不適，請依實際症狀尋求醫療評估。',
  ...
  {id:13, suit:'major',rank:13, num:13, n:'死神',     el:'水', astro:'天蠍座 | נ Nun / Path 24 (Tiphareth→Netzach) / Scorpio / 簡字 Nun',
   ...
   wealthUp:'舊有收入或支出方式可能需要調整；新的資金來源須經實際確認。',wealthRv:'核對虧損與預算，避免只因捨不得就持續投入。',
   healthUp:'可以重新檢視長期生活習慣；是否改善健康應以實際追蹤為準。',healthRv:'檢查需要調整的習慣，不能單靠牌面斷定病情惡化。',
  ...
  {id:14, suit:'major',rank:14, num:14, n:'節制',     el:'火', astro:'射手座 | ס Samekh / Path 25 (Tiphareth→Yesod) / Sagittarius / 簡字 Samekh',
  ...
  {id:15, suit:'major',rank:15, num:15, n:'惡魔',     el:'土', astro:'摩羯座 | ע Ayin / Path 26 (Tiphareth→Hod) / Capricorn / 簡字 Ayin',
   ...
   healthUp:'留意可能不利身心的習慣；是否構成依賴或健康問題仍需實際評估。',healthRv:'這張牌偏向擺脫舊習慣；身心恢復情況仍需持續觀察。',
  ...
  {id:16, suit:'major',rank:16, num:16, n:'塔',       el:'火', astro:'火星 | פ Peh / Path 27 (Netzach→Hod) / Mars / 雙字 Peh',
   ...
   careerUp:'工作結構可能受到衝擊，先檢查組織與職位的實際消息，不直接推定裁員。',careerRv:'工作環境中的問題越積越嚴重，危機被延後但沒有消失。',
   wealthUp:'重大財務衝擊，但觸底後可以重建，先止血再重來。',wealthRv:'財務危機被暫時壓住，根本問題沒解決，遲早還是會爆。',
   healthUp:'留意任何突發不適，依症狀決定何時就醫；牌面不能診斷急症。',healthRv:'健康方面不要忽略持續不適，但是否存在警訊仍以具體症狀與檢查為準。',
  ...
  {id:17, suit:'major',rank:17, num:17, n:'星星',     el:'風', astro:'水瓶座 | צ Tzaddi / Path 28 (Netzach→Yesod) / Aquarius / 簡字 Tzaddi',
   ...
   wealthUp:'可重新檢視長期財務規畫；收益與改善需要看帳目，不由希望判定。',wealthRv:'財務希望暫時落空，需要重新規劃，但不要放棄。',
   healthUp:'適合保持可持續的照護與休息；是否好轉依實際狀況判斷。',healthRv:'若復原速度不如預期，記錄變化並向醫療專業人員確認做法。',
  ...
  {id:18, suit:'major',rank:18, num:18, n:'月亮',     el:'水', astro:'雙魚座 | ק Qoph / Path 29 (Netzach→Malkuth) / Pisces / 簡字 Qoph',
   ...
   healthUp:'如果有焦慮或睡眠困擾，先記錄具體狀況並尋求合適的協助。',healthRv:'牌面偏向釐清壓力來源；實際身心變化仍須觀察與核對。',
  ...
  {id:19, suit:'major',rank:19, num:19, n:'太陽',     el:'火', astro:'太陽 | ר Resh / Path 30 (Hod→Yesod) / Sun / 雙字 Resh',
   ...
   wealthUp:'牌面偏向清楚與活力；先核對實際收入與風險，再判斷財務成果。',wealthRv:'財運稍有延遲，但方向是對的，不要因為暫時的挫折放棄。',
   healthUp:'這張牌有活力與恢復的象徵；身體狀況仍須依實際症狀與檢查判斷。',healthRv:'需要多曬太陽多運動，精神上需要一點刺激和陽光。',
  ...
  {id:20, suit:'major',rank:20, num:20, n:'審判',     el:'火', astro:'冥王星 | ש Shin / Path 31 (Hod→Malkuth) / Pluto-Fire / 母字 Shin(火)',
  ...
  {id:21, suit:'major',rank:21, num:21, n:'世界',     el:'土', astro:'土星 | ת Tav / Path 32 (Yesod→Malkuth) / Saturn-Earth / 雙字 Tav',
  ...
  // ═══ 小阿爾克那 — 權杖 Wands ═══
  {id:22, suit:'wand', rank:'ace',    num:1,  n:'權杖王牌', el:'火', astro:'season:Summer / Root of Powers of Fire / Kether of Atziluth',
  ...
  {id:23, suit:'wand', rank:'2',      num:2,  n:'權杖二',   el:'火', astro:'Mars in Aries 1st decan / Mar 21-30 / Chokmah of Atziluth / Lord of Dominion',
  ...
  {id:24, suit:'wand', rank:'3',      num:3,  n:'權杖三',   el:'火', astro:'Sun in Aries 2nd decan / Mar 31-Apr 10 / Binah of Atziluth / Lord of Established Strength',
  ...
  {id:25, suit:'wand', rank:'4',      num:4,  n:'權杖四',   el:'火', astro:'Venus in Aries 3rd decan / Apr 11-19 / Chesed of Atziluth / Lord of Perfected Work',
  ...
  {id:26, suit:'wand', rank:'5',      num:5,  n:'權杖五',   el:'火', astro:'Saturn in Leo 1st decan / Jul 22-Aug 1 / Geburah of Atziluth / Lord of Strife',
  ...
  {id:27, suit:'wand', rank:'6',      num:6,  n:'權杖六',   el:'火', astro:'Jupiter in Leo 2nd decan / Aug 2-11 / Tiphareth of Atziluth / Lord of Victory',
  ...
  {id:28, suit:'wand', rank:'7',      num:7,  n:'權杖七',   el:'火', astro:'Mars in Leo 3rd decan / Aug 12-22 / Netzach of Atziluth / Lord of Valour',
  ...
  {id:29, suit:'wand', rank:'8',      num:8,  n:'權杖八',   el:'火', astro:'Mercury in Sagittarius 1st decan / Nov 23-Dec 2 / Hod of Atziluth / Lord of Swiftness',
  ...
  {id:30, suit:'wand', rank:'9',      num:9,  n:'權杖九',   el:'火', astro:'Moon in Sagittarius 2nd decan / Dec 3-12 / Yesod of Atziluth / Lord of Great Strength',
  ...
  {id:31, suit:'wand', rank:'10',     num:10, n:'權杖十',   el:'火', astro:'Saturn in Sagittarius 3rd decan / Dec 13-21 / Malkuth of Atziluth / Lord of Oppression',
  ...
  {id:32, suit:'wand', rank:'page',   num:11, n:'權杖侍者', el:'火', astro:'Earth of Fire / Princess of the Shining Flame / Quadrant: Aries-Leo-Sagittarius',
  ...
  {id:33, suit:'wand', rank:'knight', num:12, n:'權杖騎士', el:'火', astro:'Air of Fire / Prince of the Chariot of Fire / Cancer 3rd-Leo 1st-Leo 2nd decans',
  ...
  {id:34, suit:'wand', rank:'queen',  num:13, n:'權杖皇后', el:'火', astro:'Water of Fire / Queen of the Thrones of Flame / Pisces 3rd-Aries 1st-Aries 2nd decans',
  ...
  {id:35, suit:'wand', rank:'king',   num:14, n:'權杖國王', el:'火', astro:'Fire of Fire / Lord of the Flame and Lightning / Scorpio 3rd-Sagittarius 1st-2nd decans',
  ...
  // ═══ 小阿爾克那 — 聖杯 Cups ═══
  {id:36, suit:'cup',  rank:'ace',    num:1,  n:'聖杯王牌', el:'水', astro:'season:Autumn / Root of Powers of Water / Kether of Briah',
  ...
  {id:37, suit:'cup',  rank:'2',      num:2,  n:'聖杯二',   el:'水', astro:'Venus in Cancer 1st decan / Jun 21-Jul 1 / Chokmah of Briah / Lord of Love',
  ...
  {id:38, suit:'cup',  rank:'3',      num:3,  n:'聖杯三',   el:'水', astro:'Mercury in Cancer 2nd decan / Jul 2-11 / Binah of Briah / Lord of Abundance',
  ...
  {id:39, suit:'cup',  rank:'4',      num:4,  n:'聖杯四',   el:'水', astro:'Moon in Cancer 3rd decan / Jul 12-21 / Chesed of Briah / Lord of Blended Pleasure',
  ...
  {id:40, suit:'cup',  rank:'5',      num:5,  n:'聖杯五',   el:'水', astro:'Mars in Scorpio 1st decan / Oct 23-Nov 2 / Geburah of Briah / Lord of Loss in Pleasure',
  ...
  {id:41, suit:'cup',  rank:'6',      num:6,  n:'聖杯六',   el:'水', astro:'Sun in Scorpio 2nd decan / Nov 3-12 / Tiphareth of Briah / Lord of Pleasure',
  ...
  {id:42, suit:'cup',  rank:'7',      num:7,  n:'聖杯七',   el:'水', astro:'Venus in Scorpio 3rd decan / Nov 13-22 / Netzach of Briah / Lord of Illusionary Success',
  ...
  {id:43, suit:'cup',  rank:'8',      num:8,  n:'聖杯八',   el:'水', astro:'Saturn in Pisces 1st decan / Feb 19-28 / Hod of Briah / Lord of Abandoned Success',
  ...
  {id:44, suit:'cup',  rank:'9',      num:9,  n:'聖杯九',   el:'水', astro:'Jupiter in Pisces 2nd decan / Mar 1-10 / Yesod of Briah / Lord of Material Happiness',
   kwUp:'願望實現·滿足·豐盛', kwRv:'物質主義·空洞·不滿足',
   up:'願望實現、滿足、豐足；陰影面：自滿、把擁有的視為理所當然、因為夠舒服就不再用心經營。',
  ...
  {id:45, suit:'cup',  rank:'10',     num:10, n:'聖杯十',   el:'水', astro:'Mars in Pisces 3rd decan / Mar 11-20 / Malkuth of Briah / Lord of Perfected Success',
  ...
  {id:46, suit:'cup',  rank:'page',   num:11, n:'聖杯侍者', el:'水', astro:'Earth of Water / Princess of the Waters / Quadrant: Cancer-Scorpio-Pisces',
  ...
  {id:47, suit:'cup',  rank:'knight', num:12, n:'聖杯騎士', el:'水', astro:'Air of Water / Prince of the Chariot of Water / Libra 3rd-Scorpio 1st-2nd decans',
  ...
  {id:48, suit:'cup',  rank:'queen',  num:13, n:'聖杯皇后', el:'水', astro:'Water of Water / Queen of the Thrones of Waters / Gemini 3rd-Cancer 1st-2nd decans',
  ...
  {id:49, suit:'cup',  rank:'king',   num:14, n:'聖杯國王', el:'水', astro:'Fire of Water / Lord of the Waves and Waters / Aquarius 3rd-Pisces 1st-2nd decans',
  ...
  // ═══ 小阿爾克那 — 寶劍 Swords ═══
  {id:50, suit:'sword',rank:'ace',    num:1,  n:'寶劍王牌', el:'風', astro:'season:Spring / Root of Powers of Air / Kether of Yetzirah',
  ...
  {id:51, suit:'sword',rank:'2',      num:2,  n:'寶劍二',   el:'風', astro:'Moon in Libra 1st decan / Sep 23-Oct 2 / Chokmah of Yetzirah / Lord of Peace Restored',
  ...
  {id:52, suit:'sword',rank:'3',      num:3,  n:'寶劍三',   el:'風', astro:'Saturn in Libra 2nd decan / Oct 3-12 / Binah of Yetzirah / Lord of Sorrow',
  ...
  {id:53, suit:'sword',rank:'4',      num:4,  n:'寶劍四',   el:'風', astro:'Jupiter in Libra 3rd decan / Oct 13-22 / Chesed of Yetzirah / Lord of Rest from Strife',
  ...
  {id:54, suit:'sword',rank:'5',      num:5,  n:'寶劍五',   el:'風', astro:'Venus in Aquarius 1st decan / Jan 20-29 / Geburah of Yetzirah / Lord of Defeat',
  ...
  {id:55, suit:'sword',rank:'6',      num:6,  n:'寶劍六',   el:'風', astro:'Mercury in Aquarius 2nd decan / Jan 30-Feb 8 / Tiphareth of Yetzirah / Lord of Earned Success',
  ...
  {id:56, suit:'sword',rank:'7',      num:7,  n:'寶劍七',   el:'風', astro:'Moon in Aquarius 3rd decan / Feb 9-18 / Netzach of Yetzirah / Lord of Unstable Effort',
  ...
  {id:57, suit:'sword',rank:'8',      num:8,  n:'寶劍八',   el:'風', astro:'Jupiter in Gemini 1st decan / May 21-31 / Hod of Yetzirah / Lord of Shortened Force',
  ...
  {id:58, suit:'sword',rank:'9',      num:9,  n:'寶劍九',   el:'風', astro:'Mars in Gemini 2nd decan / Jun 1-10 / Yesod of Yetzirah / Lord of Despair and Cruelty',
  ...
  {id:59, suit:'sword',rank:'10',     num:10, n:'寶劍十',   el:'風', astro:'Sun in Gemini 3rd decan / Jun 11-20 / Malkuth of Yetzirah / Lord of Ruin',
   ...
   wealthUp:'有損失壓力時先核對帳目，制定可執行的重建步驟；不可認定已到最低點。',wealthRv:'重新核對債務與現金流；牌面不能保證最艱難的階段已過。',
   healthUp:'健康題請留意是否過度消耗，症狀持續或惡化時向醫療專業人員求助。',healthRv:'牌面偏向重新整理休養節奏；是否恢復應按實際健康狀況確認。',
   adviceUp:'觸底了，放下吧，保留你的能量用於重建，不要繼續在廢墟上耗。',adviceRv:'最難的過了，相信這個恢復的過程，你走在回來的路上。'},
  {id:60, suit:'sword',rank:'page',   num:11, n:'寶劍侍者', el:'風', astro:'Earth of Air / Princess of the Rushing Winds / Quadrant: Gemini-Libra-Aquarius',
  ...
  {id:61, suit:'sword',rank:'knight', num:12, n:'寶劍騎士', el:'風', astro:'Air of Air / Prince of the Chariot of Winds / Capricorn 3rd-Aquarius 1st-2nd decans',
  ...
  {id:62, suit:'sword',rank:'queen',  num:13, n:'寶劍皇后', el:'風', astro:'Water of Air / Queen of the Thrones of Air / Virgo 3rd-Libra 1st-2nd decans',
  ...
  {id:63, suit:'sword',rank:'king',   num:14, n:'寶劍國王', el:'風', astro:'Fire of Air / Lord of the Wind and Breezes / Taurus 3rd-Gemini 1st-2nd decans',
  ...
  // ═══ 小阿爾克那 — 金幣 Pentacles ═══
  {id:64, suit:'pent', rank:'ace',    num:1,  n:'金幣王牌', el:'土', astro:'season:Winter / Root of Powers of Earth / Kether of Assiah',
  ...
  {id:65, suit:'pent', rank:'2',      num:2,  n:'金幣二',   el:'土', astro:'Jupiter in Capricorn 1st decan / Dec 22-30 / Chokmah of Assiah / Lord of Harmonious Change',
  ...
  {id:66, suit:'pent', rank:'3',      num:3,  n:'金幣三',   el:'土', astro:'Mars in Capricorn 2nd decan / Dec 31-Jan 9 / Binah of Assiah / Lord of Material Works',
  ...
  {id:67, suit:'pent', rank:'4',      num:4,  n:'金幣四',   el:'土', astro:'Sun in Capricorn 3rd decan / Jan 10-19 / Chesed of Assiah / Lord of Earthly Power',
  ...
  {id:68, suit:'pent', rank:'5',      num:5,  n:'金幣五',   el:'土', astro:'Mercury in Taurus 1st decan / Apr 21-30 / Geburah of Assiah / Lord of Material Trouble',
   ...
   healthUp:'健康困難時期，缺乏資源和支持，需要主動尋求幫助。',healthRv:'偏向尋求外部支援；治療是否有效仍要依實際檢查與評估確認。',
   adviceUp:'困難時期開口求援不是弱點，主動尋求幫助是智慧。',adviceRv:'最難的已經過去，援助在來的路上，繼續撐住。'},
  {id:69, suit:'pent', rank:'6',      num:6,  n:'金幣六',   el:'土', astro:'Moon in Taurus 2nd decan / May 1-10 / Tiphareth of Assiah / Lord of Material Success',
  ...
  {id:70, suit:'pent', rank:'7',      num:7,  n:'金幣七',   el:'土', astro:'Saturn in Taurus 3rd decan / May 11-20 / Netzach of Assiah / Lord of Success Unfulfilled',
   ...
   healthUp:'健康調整需要時間；觀察實際反應，按需要修正做法。',healthRv:'健康改善缺乏耐心，急於看到結果讓你放棄正確的方法。',
   adviceUp:'繼續播種，繼續等待，你在做的事情是值得的，成果需要時間。',adviceRv:'你的等待還沒完，不要在快到了的時候放棄，再耐心一點。'},
  {id:71, suit:'pent', rank:'8',      num:8,  n:'金幣八',   el:'土', astro:'Sun in Virgo 1st decan / Aug 23-Sep 1 / Hod of Assiah / Lord of Prudence',
  ...
  {id:72, suit:'pent', rank:'9',      num:9,  n:'金幣九',   el:'土', astro:'Venus in Virgo 2nd decan / Sep 2-11 / Yesod of Assiah / Lord of Material Gain',
  ...
  {id:73, suit:'pent', rank:'10',     num:10, n:'金幣十',   el:'土', astro:'Mercury in Virgo 3rd decan / Sep 12-22 / Malkuth of Assiah / Lord of Wealth',
   ...
   healthUp:'長期穩定的健康管理有效，家族健康傳統值得繼承。',healthRv:'若擔心家族病史，可整理已知資訊，並向專業人員確認是否需要評估。',
   adviceUp:'珍惜這個長期積累的豐盛，用智慧守護和傳承下去。',adviceRv:'面對家族中積累已久的問題，長期拖延讓它越來越複雜。'},
  {id:74, suit:'pent', rank:'page',   num:11, n:'金幣侍者', el:'土', astro:'Earth of Earth / Princess of the Echoing Hills / Quadrant: Taurus-Virgo-Capricorn',
  ...
  {id:75, suit:'pent', rank:'knight', num:12, n:'金幣騎士', el:'土', astro:'Air of Earth / Prince of the Chariot of Earth / Aries 3rd-Taurus 1st-2nd decans',
  ...
  {id:76, suit:'pent', rank:'queen',  num:13, n:'金幣皇后', el:'土', astro:'Water of Earth / Queen of the Thrones of Earth / Sagittarius 3rd-Capricorn 1st-2nd decans',
  ...
  {id:77, suit:'pent', rank:'king',   num:14, n:'金幣國王', el:'土', astro:'Fire of Earth / Lord of the Wide and Fertile Land / Leo 3rd-Virgo 1st-2nd decans',
   ...
   wealthUp:'財務管理需要負責任的判斷；投資成果仍須核對真實帳目和風險。',wealthRv:'貪婪讓財務決策走偏，過度控制財務讓資金無法流動。',
...
TAROT_DEEP[1] = { // 魔術師
  ...
  wealthUp:'善用已有技能核對收入與風險，不能把能力當作收益保證',wealthRv:'核實交易來源與風險，避免受未經查證的承諾影響',
...
TAROT_DEEP[2] = { // 女祭司
  ...
  loveUp:'有尚未說清楚的感情訊息；對方是否有好感仍要看實際互動',loveRv:'有不明朗的部分，先核對實際訊息，不能斷言對方在隱瞞',
...
TAROT_DEEP[3] = { // 皇后
  ...
  wealthUp:'關注資源累積與付出回報；收入變化仍依實際帳目',wealthRv:'花費過度，享受先於儲蓄',
  healthUp:'適合照顧生活與作息；備孕或身體狀態須依專業評估',healthRv:'飲食不節制，需注意體重和消化',
...
TAROT_DEEP[10] = { // 命運之輪
  ...
  loveUp:'感情可能出現變化或新相遇，是否互相靠近仍看雙方選擇',loveRv:'感情遇到瓶頸，需要耐心等待轉機',
  careerUp:'事業轉折點，把握機會就上去了',careerRv:'事業低谷，但不會永遠',
  wealthUp:'財務條件可能變化；核實收益、現金流與風險後才作決定',wealthRv:'財運下滑，守住不要擴張',
  healthUp:'重新檢視保養習慣，是否好轉依實際症狀與檢查確認',healthRv:'留意體能變化，是否為舊症復發需要實際評估',
...
TAROT_DEEP[11] = { // 正義
  ...
  wealthUp:'檢查實際投入和回報，不能由公正之象推定收益',wealthRv:'財務糾紛或不合理的損失',
  healthUp:'查明症狀仍需醫療評估，牌面只提示核對資訊',healthRv:'有疑問時再核對檢查結果，不能由牌面推斷誤診',
...
TAROT_DEEP[19] = { // 太陽
  ...
  wealthUp:'財務變得較清楚，實際收益仍需核對帳目',wealthRv:'財運稍有延遲但方向正確',
  healthUp:'活力之象，身體狀況仍以實際檢查為準',healthRv:'需要多曬太陽多運動，別窩著',
...
// ═══════════════════════════════════════════════════════════════
var _deck3dRAF=null, _deck3dPaused=false, _deck3dCleanup=null;
window.JYTarotDeckBrowse={move:function(){}};
...
function initTarotDeck(){
  ...
  // One pointer controller owns both touch and mouse. No duplicate touchend
  // handler: a dragged card must never become an accidental selection.
...
function _update3DCards(rowEl){
  ...
  for(var i=0,len=rowEl.children.length;i<len;i++){
    ...
    if(cR.right<sR.left-80||cR.left>sR.right+80){
      card.style.visibility='hidden';card.tabIndex=-1;continue;
    }
    card.style.visibility='';card.tabIndex=0;
