...
function initTarotDeck(){
  ...
  // 管理員不受鎖定限制
  var _maxC=(S.tarot&&S.tarot.spreadDef&&S.tarot.spreadDef.count)||10; if(drawnCards.length>=_maxC && !S._isAdmin){
    showTarotLocked();
    return;
  }
  if(drawnCards.length>=_maxC && S._isAdmin){
    drawnCards=[];
    console.log('[管理員] 重置塔羅牌，允許重新抽牌');
  }
  // Shuffle all 78 cards — seeded if form data available (admin uses true random)
  if(S._isAdmin){
    deckShuffled=[...TAROT].sort(()=>Math.random()-0.5);
    console.log('[管理員] 使用真隨機洗牌');
...
function autoDraw(){
  ...
  var _tgt=(S.tarot&&S.tarot.spreadDef&&S.tarot.spreadDef.count)||10; if(drawnCards.length>=_tgt) return;

  const deckEl=document.getElementById('t-deck');
  const cards=deckEl.querySelectorAll('.tarot-deck-card:not(.picked)');
  if(!cards.length) return;

  // 取前10張未被選取的牌，依序快速抽出
  var _tgt2=(S.tarot&&S.tarot.spreadDef&&S.tarot.spreadDef.count)||10; const remaining=_tgt2-drawnCards.length;
  const toPick=Array.from(cards).slice(0, remaining);
  ...
  toPick.forEach(function(cardEl, i){
    ...
    delay+=650; // 每張間隔650ms，讓飛牌動畫跑完
  });
}

function pickCard(deckIdx,deckEl){
  if(pickAnimating||(drawnCards.length>=((S.tarot&&S.tarot.spreadDef&&S.tarot.spreadDef.count)||10))||deckEl.classList.contains('picked'))return;
  pickAnimating=true;
  ...
  setTimeout(()=>{
    ...
    const isCelticCross2 = (S.tarot && S.tarot.spreadType === 'celtic_cross');
    const isCard2=(slotIdx===1 && isCelticCross2); // Card 2 is横放 only in Celtic Cross
    ...
    if (drawnCards.length >= 3 && _btnAna.disabled) {
      _btnAna.disabled = false;
    }
