...
(function () {
  ...
  function buildBaziPrompt(question, b, meta) {
    ...
    L.push('收尾 2–3 句，自然推薦一種適合隨身的水晶／能量石（實務輔助、非八字原理），語氣平實不推銷，可輕附蝦皮去處，並以連結收尾：' + SHOPEE + ' ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。選石鐵則：以【用神／喜神】五行為主；若某五行能調候卻是日主的官殺或忌神（如身弱補水＝補七殺），不可選它當主石；最佳是選「同時服務扶抑用神＋調候」的那個五行（即上方調候欄標『與用神同向』者）。只講一種、只在最後。');
