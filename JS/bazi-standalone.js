/*! bazi-standalone.js — 靜月之光 八字命理獨立流程  [v80.34]
...
(function () {
  ...
  function buildBaziPrompt(question, b, meta) {
    ...
    L.push('□ 空亡落到用神／流年流月時有點出虛象、且同組空亡前後一致　□ 官殺混雜等格局病有翻成人事具象、不只報名詞　□ 有定格局、且格局通則與用神方向不衝突（衝突時以用神為準、不閃避格局）　□ 調候／病藥／通關標註一致：沒把「已飽和／已過旺／官殺／忌神」的五行當喜用補（不與「已足／不缺」自相矛盾）　□ 有點名下一步大運順背與翻身窗口　□ 能量石只寫一種、且取「與用神同向＋調候」那行五行');
    L.push('');

    // 輸出格式 + 水晶收尾
    L.push('【輸出格式】');
    L.push('繁體中文，像命理師當面講話，不是分析報告。第一句給結論，之後一段段推進，每段帶新資訊、同一結論只講一次。不要列表格、不要粗體小標、不要逐柱報告。');
    L.push('收尾 2–3 句，自然推薦一種適合隨身的水晶／能量石（實務輔助、非八字原理），語氣平實不推銷，可輕附蝦皮去處，並以連結收尾：' + SHOPEE + ' ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。選石鐵則：以【用神／喜神】五行為主；若某五行能調候卻是日主的官殺或忌神（如身弱補水＝補七殺），不可選它當主石；最佳是選「同時服務扶抑用神＋調候」的那個五行（即上方調候欄標『與用神同向』者）。主石只准寫出「一種」石頭的名字——嚴禁「或」「這類」「例如A、B」式並列第二種；領頭那顆必須是該五行之石，不可改用其他喜神行代打。只講一種、只在最後。');
