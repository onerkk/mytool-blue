...
(function () {
  ...
  function buildBaziPrompt(question, b, meta) {
    ...
    if (b.tiaohou && b.tiaohou.need && b.tiaohou.need.length) {
      // v80.45：調候五行逐一對照本命佔比／忌神，標「真正要補」或「已足／已過旺」，
      //   消除「需水卻又忌多水（水多木漂）」這類自相矛盾，讓 AI 不會把已過旺的五行當喜用。
      ...
      var _needTxt = b.tiaohou.need.map(function (el) {
        ...
        if (_unfav.indexOf(el) >= 0) return el + '（本命已過旺、已列忌神→調候之' + el + '已足，不可再補，重點補另一個）';
        if (pct != null && pct >= 22) return el + '（本命已充足約' + pct + '%→不需再補，調候重點在另一個）';
        if (_fav.indexOf(el) >= 0) return el + '（真正要補，且與用神同向）';
        return el + '（真正要補）';
      });
      ...
      if (b.tiaohou.priority) L.push('調候優先級：' + b.tiaohou.priority + (/第一|輔助/.test(b.tiaohou.priority) ? '（寒燥命調候優先，用神再強也要先暖／先潤；但上面標「已足／已過旺」的五行視為已滿足，不再補）' : '') + '。');
    } else {
    ...
    L.push('③調候疊加：寒燥命（冬生需火暖、夏生需水潤等）調候為第一優先，用神再強也要先解寒燥。已附《窮通寶鑑》調候。但要看上方調候欄的標註——標「已足／已過旺」的五行不可再當喜用補進去（例如水多木漂卻說要補水、火炎土燥卻說要補火，都是錯），只補標「真正要補」的那個。');
    ...
    L.push('□ 空亡落到用神／流年流月時有點出虛象、且同組空亡前後一致　□ 官殺混雜等格局病有翻成人事具象、不只報名詞　□ 調候沒把「已過旺」的五行當喜用');
    L.push('');
