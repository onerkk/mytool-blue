...
(function () {
  ...
  function _getWrap() {
    ...
    css.textContent = [
      ...
      '.bzx-sheet-foot{flex:0 0 auto;display:grid;grid-template-columns:1fr 1.2fr;gap:.5rem;margin-top:.45rem;padding:.7rem 0 .1rem;background:linear-gradient(180deg,rgba(13,13,19,0),#0d0d13 30%)}',
  ...
  function buildBaziPrompt(question, b, meta) {
    ...
    L.push('');
    L=L.concat(spec.answerContractLines('single'));
    L.push('');
    L=L.concat(spec.brandTailLines());
    return L.join('\n');
  }
  ...
  function _ensureEngine(cb) {
    ...
    if (typeof calcTrueSolarTime !== 'function') need.push('JS/solar-location.js?v=20260922bridge1');
    if (typeof computeBazi !== 'function') need.push('JS/bazi.js?v=20260922bridge1');
    if (typeof enhanceBazi !== 'function') need.push('JS/bazi_upgrade.js?v=20260922rules1');
  ...
  function _dpRenderDay(initialBody){
    ...
    else { _setBody(h); _setSub('點日期，或點上方年月快速跳轉'); }
  }
  ...
  window._bzxOpenCity=function(){
    ...
    for(var gi=0;gi<CITY_GROUPS.length;gi++){ var grp=CITY_GROUPS[gi];
      ...
      for(var j=0;j<grp.idx.length;j++){ var ix=grp.idx[j];
        h+='<button type="button" class="bzx-loc-chip'+((''+ix)===(''+_selCity)?' sel':'')+'" aria-pressed="'+((''+ix)===(''+_selCity))+'" onclick="_bzxPickCity('+ix+')">'+_cityShort(ix)+'</button>'; }
      h+='</div></div>';
    }
