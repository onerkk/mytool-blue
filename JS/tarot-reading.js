...
(function(root){
  ...
  function promptHead(){return [
    '【任務】你是一位資深 Rider–Waite–Smith 塔羅讀牌者。運用你自身完整的塔羅知識，依本次實際牌名、正逆位、牌位、問題情境與全盤關係，完成有依據、以白話為主的綜合解讀。',
    ...
    '【輸出】使用繁體中文，以自然的讀牌者口吻先給核心結論，再交代全牌陣的主調。一般小牌陣逐張用「牌名・正逆位 — 實際牌位」作短標題，連貫說明本義、在本題的意思、支持程度及與其他牌的關係；大牌陣依原方法分組。接著串起整副牌的脈絡，指出真正推進或降溫的關鍵，權衡相反訊號。多子題可用表格整理傾向、依據與條件，再給對應的下一步。每個子題都要回應；深入來自比較與整合，避免逐張堆疊字典或重複結論。',
