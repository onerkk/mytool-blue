...
function baziDetectZhengGe(bazi) {
  ...
  var dm = bazi.dm; // 日干
  var monthZhi = bazi.pillars[1][1]; // 月支
  var monthGan = bazi.pillars[1][0]; // 月干
  ...
  // 找月令透出的十神 → 定格局
  // 先看月干是否透出月令藏干
  var yearGan = bazi.pillars[0][0];
  var hourGan = bazi.pillars[3][0];
...
function baziHiddenInteractions(bazi) {
  ...
  var interactions = [];
  var allZhi = [bazi.pillars[0][1], bazi.pillars[1][1], bazi.pillars[2][1], bazi.pillars[3][1]];
...
function baziTenGodCombinations(bazi) {
  ...
  var gods = bazi.gods || {};
  var dm = bazi.dm;
  var allGan = [bazi.pillars[0][0], bazi.pillars[1][0], bazi.pillars[2][0], bazi.pillars[3][0]];
