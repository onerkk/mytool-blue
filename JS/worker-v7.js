...
不是給用戶看的，是給另一位命理師看的——他要根據你的筆記跟用戶面對面聊天。

所以你的筆記必須：
1. 保留具體細節（哪個宮、哪張牌在哪個位置、哪個星在哪個相位）
2. 不要壓縮成「感情有阻礙」這種空話——要寫清楚「為什麼」有阻礙
3. 針對用戶的問題，從每套系統挑出最相關的 1-2 個數據點深入分析
4. 明確標出哪些結論是多套系統同時支持的（高信心）
5. 明確標出系統之間的矛盾（矛盾往往是最有價值的線索）

═══ 每套系統你要做的事 ═══

不需要七套都寫，哪些跟問題最相關就寫哪些（通常 4-5 套就夠）。
每套系統的分析要包含：
- 你看到什麼（具體數據）
- 這代表什麼（你的解讀）
- 跟用戶的問題有什麼關係
...
寫清楚：什麼時候是轉折點、什麼時候適合行動、什麼時候該等。

═══ 必須包含的三個判斷 ═══

1. core_tension：這個人內心最大的矛盾——不要公式化，要像你看穿他一樣。
2. hidden_truth：他最不想面對的事實。一句話，要具體。
3. person_profile：從命盤看出這個人的性格——他的盲點、他處理問題的慣性。

═══ 輸出格式 ═══

直接寫分析筆記，不需要 JSON 格式。用自然語言，分段落寫。
...
## 重要發現
（3-5 個發現，每個都要有具體數據 + 解讀。標出信心等級和多少系統支持。）

## 時間線
（具體到月份的時間判斷，標出依據）

## 這個人的性格盲點
（person_profile：他怎麼對待這個問題？他的慣性是什麼？）
...
重要：每個發現都要保留原始數據的引用（例如「紫微夫妻宮天機化忌」「塔羅聖杯騎士逆位在第二張對方位」），這樣下一位命理師才能引用這些細節跟用戶說話。`;
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // ═══ Step 1: 提取重疊象徵 ═══
      const step1 = await callAI(env, STEP1_PROMPT, rawDataMessage, 4500, 0.15);
      // Step 1 now returns analysis notes (plain text, not JSON)
...
寫作指南：
1. 先選一個比喻框架（跟這個人的問題貼合的意象），用它串起整篇。
2. 開場用筆記裡的 core_tension 或 hidden_truth——第一句就讓對方愣住。
3. 筆記裡每個「重要發現」都帶有具體的命盤數據引用——你不要直接說術語，但要用那些細節來構建場景。比如筆記說「紫微夫妻宮天機化忌」，你就寫成「她自己現在也不知道自己要什麼」。
4. 筆記裡的 person_profile 告訴你這個人的盲點——用它來點出他自己看不到的地方。
5. 時間判斷直接融入敘事，不要另開一段。
6. 至少有一段讓人不想聽但知道你說的對的真話。
7. 收尾用比喻呼應開場，形成閉環。
8. 寫 1200-2000 字。挑 2-3 個最重要的發現深入展開，不要面面俱到。`;
...
function buildUserMessage(p) {
  ...
  // 直接傳文本（不包 JSON.stringify），讓 AI 更容易閱讀
  const r = p.readings || {};
  if (r.bazi) { lines.push('【八字命盤】'); lines.push(r.bazi); lines.push(''); }
  if (r.ziwei) { lines.push('【紫微斗數】'); lines.push(r.ziwei); lines.push(''); }
  if (r.meihua) { lines.push('【梅花易數】'); lines.push(r.meihua); lines.push(''); }
  if (r.tarot) { lines.push('【塔羅牌陣】'); lines.push(r.tarot); lines.push(''); }
  if (r.natal) { lines.push('【西洋占星】'); lines.push(r.natal); lines.push(''); }
  if (r.vedic) { lines.push('【吠陀占星】'); lines.push(r.vedic); lines.push(''); }
  if (r.name) { lines.push('【姓名學】'); lines.push(r.name); lines.push(''); }
  ...
  if (p.verdict) { lines.push('【七維度綜合結論】' + p.verdict + (p.compositeScore ? ' 綜合分:' + p.compositeScore + '/100' : '')); lines.push(''); }
  if (p.crossSignals && p.crossSignals.length) {
    lines.push('【多系統交叉信號】');
    p.crossSignals.forEach(function(s) { lines.push(s); });
    lines.push('');
  }
  if (p.resonance) {
    lines.push('【七維度共振分析】');
  ...
  if (p.conflicts && p.conflicts.length) {
    lines.push('【系統間矛盾信號】');
    p.conflicts.forEach(function(c) { lines.push(c); });
    lines.push('');
  }
