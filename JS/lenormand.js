// ═══════════════════════════════════════
// 靜月之光 — 雷諾曼牌 Lenormand v2.0
// 正統 Petit Lenormand 36 張・組合義讀法
// ═══════════════════════════════════════
(function () {
'use strict';
console.log('[Lenormand] 靜月之光 雷諾曼牌 v2.0 loaded — 正統組合義/線讀/九宮格/大牌陣提示詞修正');
...
var SPREADS = {
  ...
  nine: { id:'nine', name:'九宮格', en:'Nine-Card Box (3×3)', count:9,
    desc:'3×3 方陣（正統 Portrait）。中心第5張是核心主題。橫排：上=想法、中=現實、下=根基；直列：左=過去、中=現在、右=未來；對角線看原因與結果。',
...
function buildPrompt(question, drawn, spreadId, sigGender) {
  ...
  lines.push('');
  lines.push('【正統性邊界】');
  lines.push('雷諾曼不是塔羅：不用逆位、不讀圖像心理投射、不以元素或牌陣位置單張定義答案。線讀、九宮格與大牌陣皆以組合義、距離、方向、主題牌與相鄰牌為主。能量石收尾是品牌實務輔助，不屬雷諾曼原典。');
  lines.push('');
  ...
  if (spreadId === 'three') {
    ...
    lines.push('  [7] [8] [9]');
    lines.push('');
    lines.push('第5張是核心。正統 Portrait（Box）讀法層次：');
  ...
  lines.push('2. 讀組合義，不是一張一張單獨解釋。正文可以自然說「鑰匙跟戒指一起出來，承諾很明確」，不要寫成公式表。');
