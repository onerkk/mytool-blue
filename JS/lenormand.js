...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  lines.push('結尾自然帶入一句「靜月之光蝦皮有挑過的」之類的話，然後以連結收尾：https://shopee.tw/a50h95648d?tab=shop ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。'); // v2.6 連結＝全文末字，根治尾端黏字
  lines.push('');
