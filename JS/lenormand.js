// ═══════════════════════════════════════
// 靜月之光 — 雷諾曼牌 Lenormand v4.0（保守正統版）
...
//   ⑧能量石／賣場文案與雷諾曼核心提示詞分離，避免商業附加層污染讀牌。
...
(function () {
'use strict';
console.log('[Lenormand] 靜月之光 雷諾曼牌 v4.0 loaded — conservative method / structure-based spread routing');
...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  lines.push('7. 正文只能提到本次實際抽出的牌名。不要拿未抽到的牌舉例，也不要用缺牌作否定證據。');
  lines.push('');
  ...
  lines.push('只回答問句實際要求的面向；不要固定追加「何時」或「怎麼辦」。');
  lines.push('');
  ...
  lines.push('4. 不補問卜者未問的財務、工作、感情、人物年齡、外貌或私生活。牌面不足就明說不足。');
  lines.push('5. 不做能量石推薦、商品推銷或賣場連結；那是獨立商業附加層，不屬雷諾曼解讀。');
  lines.push('6. 結論應短而完整：答案、必要條件、最主要風險；只有問句有要求時才補原因、時間或行動。');
  lines.push('');
  lines.push('【本盤可在正文使用的牌名】' + legalNames.join('、'));
  lines.push('現在開始解讀。');
