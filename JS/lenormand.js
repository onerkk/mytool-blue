...
function buildPrompt(question, drawn, spreadId, sigGender) {
  ...
  lines.push('・「橫排」「直列」「對角線」「四角」「鏡像配對」「核心牌」「Portrait」——這些是你分析的工具，不是要說出來的。');
  ...
  lines.push('2. 九宮格的橫排、直列、對角線、四角、鏡像配對都是你分析的工具，不是要逐一報告的清單。所有訊息都要拿來回答這個問題，但禁止寫成「橫排顯示X、直列顯示Y、對角線顯示Z」這種逐層報告。把所有線索串成一個針對問題的故事。');
  ...
  lines.push('3. 同一張牌在不同問題裡意義不同。問感情時，魚是「對方的吸引力或感情的流動」；問財時才是「錢」。務必把牌義扭向問卜者問的那件事。');
