...
function buildPrompt(question, drawn, spreadId, sigGender) {
  ...
  lines.push('2. 九宮格的十字線、四角、對角線、鏡像配對都是你分析的工具，不是要逐一報告的清單。所有訊息都要拿來回答這個問題，但禁止寫成「十字線顯示X、四角顯示Y、對角線顯示Z」這種逐層報告。把所有線索串成一個針對問題的故事。');
  lines.push('   ✗ 錯誤（逐層報告）：「十字線上方星星代表希望，下方鸛代表改變；四角錨與信代表…；對角線淑女到鳥代表…」');
