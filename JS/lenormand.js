...
// v3.8(2026/6/15・index v86_31)：主題偵測全面根治（補 v3.7 只修「工作」一條的不完整）——
...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  // ════ 特殊題型（按問題形狀才載入，避免無關規則稀釋）════
  var _qSp = String(question || '');
  var _spLines = [];
  if (/還是|或是|或者|哪個|哪一個|要不要|該不該|二選一|二擇一|選哪|to be/i.test(_qSp)) {
  ...
  if (/想法|想什麼|在想|心裡|意圖|打算|隱瞞|瞞|可信|可不可信|信不信|喜不喜歡|喜歡我|在乎|在意|暗中|背著|偷|誠實|真心|是不是真|有沒有人/.test(_qSp)) {
  ...
  if (isGT && drawn.length === 36) {
    ...
    try {
      var _tThemes = [
        // 感情/關係：含口語與「對方對我…」問法；注入 心24/戒指25 ＋ 人物牌28/29（雙代表牌連線必需，關係題一律備齊格位）
        [/感情|戀愛|戀情|談戀愛|喜歡|好感|心動|曖昧|告白|表白|交往|在一起|復合|挽回|分手|劈腿|外遇|出軌|對象|另一半|伴侶|男友|女友|老公|老婆|配偶|結婚|婚姻|離婚|正緣|緣分|姻緣|追求|暗戀|喜不喜歡|愛不愛|在不在乎|對我有(意思|感覺|好感)/, [24, 25, 28, 29]],
        [/工作|職務|任務|案子|職涯|事業|使命|天職|天賦|天命|才華|專長|志業|熱情|發光|發熱|人生方向|該做什麼|適合做什麼|做什麼工作|轉職|換工作|離職|找工作|求職|面試|錄取|工作機會/, [14, 35]],
        [/升遷|升職|加薪|調職|調動|主管|老闆|上司|位階|職位|職等|權威|體制|考核|重用|出頭/, [15, 19]],
        [/財|錢|金錢|收入|薪水|薪資|營收|生意|買賣|投資|理財|賺|獲利|進帳|債|貸款|存款|發財/, [34, 15]],
        [/健康|身體|生病|病情|疾病|就醫|看醫生|檢查|開刀|手術|復原|康復|體檢|養生|疼痛|受傷/, [5]],
        [/溝通|訊息|消息|聯絡|聯繫|回覆|回訊|已讀|回應|對話|傳話/, [12, 27]],
        [/旅行|旅遊|出國|出差|遠行|搬家|移居|移民|搬遷/, [3]],
        [/運勢|運氣|流年|整體運|近況|最近(如何|怎樣)/, [2, 16]]
      ];
  ...
  // v3.5：把「問卜者的問題」與最常被忽略的鐵律複述在最末端（query-at-end／近因效應）。
  //   依據：Anthropic 官方長文指引「查詢放結尾，複雜輸入品質可提升約三成」＋ Lost-in-the-Middle
  //   （開頭/結尾最強、中段最弱）。原本問題在前段、後面壓一大牆規則，正落在最弱位置。
  lines.push('');
  lines.push('══════════════════════════════════════════');
