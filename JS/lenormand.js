...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  lines.push('・人物牌歸屬（抽出紳士28或淑女29時，凡涉及人物先做這個判斷）：①問卜者已聲明性別（見上方【問卜者性別】）或已選 28/29 當指示牌＝性別已知——抽出的那張若就是指示牌本身＝問卜者本人；另一張人物牌＝對象或他人。②問卜者沒有以 28/29 自指（性別未知）——抽出的人物牌歸屬未定：正文必須先講明「這張可能是你本人、也可能是你身邊的某位女性／男性，牌面沒有給定」，之後的解讀兩種可能都要照顧到；嚴禁未經聲明直接斷成「另一個女人／另一個男人」。');
  lines.push('');
  ...
  lines.push('結尾自然帶入一句「靜月之光蝦皮有挑過的」之類的話。最後兩行固定照此收束：倒數第二行只放蝦皮網址本身 https://shopee.tw/a50h95648d?tab=shop （該行不得有網址以外的任何文字、標點、引號、符號或空白）；最後一行固定輸出「願你諸事順遂。」六個字作結，之後不再輸出任何內容。兩行缺一不可、順序不可顛倒（網址行被任何字元黏到就點不動，所以網址行上下都要乾淨）。'); // v3.4 犧牲行：全站收束統一（末字版實測仍黏不可見字元）
  lines.push('');
