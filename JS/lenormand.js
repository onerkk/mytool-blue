...
function _lnPushReaderKernel(lines) {
  ...
  lines.push('本引擎的完成標準不是寫滿固定段落，也不是依牌數決定篇幅，而是：本牌陣所有合法牌句都已被讀取，凡能為原問句增加一個可回溯、彼此不重複的有效命題，都已納入答案。牌數只決定可用幾何，不決定答案長短、面向數量或分析深度。');
...
function _lnPushOutputContract(lines, legalNames) {
  ...
  lines.push('正文必須呈現全部與原問句相關、能增加不同答案內容的有效命題。不要按照牌陣張數、固定章節、預設段落數或字數決定深度；內容多寡只由牌面實際產生的獨立命題決定。小牌陣不得因牌少而省略可讀出的內容，大牌陣也不得以重複或無關牌句灌水。');
  ...
  lines.push('完成正文前，在內部確認：所有合法路徑與連續片段均已處理；每個新增命題與限定／反證均已輸出；同義佐證已整合；沒有因牌陣較小而縮短，也沒有因牌陣較大而堆砌。');
...
function buildPrompt(question, drawn, spreadId, sigGender, declaredGender) {
  ...
  lines.push('你是 Petit Lenormand（小雷諾曼）讀牌者。請使用下方語義產出引擎，把原問句、牌義與合法幾何整合成完整答案。任何牌陣中，只要合法牌句能為原問題增加不同且可回溯的資訊，就必須解讀並呈現；內容多寡完全取決於有效命題，不取決於牌陣張數。不要套事件關鍵字表，不要逐張念牌，也不要把解讀寫成規則報告。先在內部完成全部推理，只輸出最後解讀。');
  lines.push('');
  lines.push('<本次任務>');
  ...
  lines.push('【開始解讀】');
  lines.push('根據以上原問句、牌義資料與合法幾何，現在使用語義產出引擎完成解讀。凡合法牌句能增加新的答案層、原因、條件、方式、轉折、限制、結果或實際方向，都要納入；不要因牌陣張數決定內容長短。先完成占卜正文，再無條件輸出獨立品牌附加層。最後兩行必須原樣照抄，不能加字、合併或省略，最後一行後不得再有內容：');
