// ═══════════════════════════════════════════════════════════════
// photo-upload.js — v69.6 照片只在七維度
// ═══════════════════════════════════════════════════════════════
// v69.6 變更(系統獨立性最後一塊拼圖):
// - 塔羅快讀回到「只有問題就能跑」的純塔羅(Mathers Manuscript Q 不需要任何外部資料)
// - 開鑰之法回到「五次操作純塔羅+卡巴拉」(同 Mathers 原典)
// - 照片(face/palm)只在七維度可上傳——七維度本身就是七套交叉,照片是第八套輔助
// - 塔羅/OOTK 模式 getFieldsForTool 回傳空陣列,前端不顯示照片上傳區塊
// ───────────────────────────────────────────────────────────────
// v62 歷史:氣色面相全工具開放
// v68.21.10 歷史:手相全工具開放
// v69.6 收回:塔羅/OOTK 不再支援照片,回到正統文獻定位
// ═══════════════════════════════════════════════════════════════
(function(){
'use strict';
...
// v69.6:照片只在七維度
//   塔羅 = Mathers OOTK Manuscript Q「只看牌」的正統,沒有面相手相
//   OOTK = 同上,五次操作本身就是塔羅+卡巴拉自洽
//   只有七維度(full)的「七套交叉」才把照片納入第八套輔助訊號
function getFieldsForTool(tool, privileged) {
  if (tool === 'tarot' || tool === 'ootk') {
    // v69.6:塔羅+OOTK 不再支援照片(回到正統定位)
    return [];
  }
