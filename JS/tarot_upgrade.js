...
(function() {
  ...
  function _runOOTKSequence(significatorId) {
    ...
    function advancePhase() {
      ...
      ritualDiv.style.cssText = 'text-align:center;padding:1.5rem 1rem;opacity:0;transition:opacity .4s';
      var _ritualIcons = ['🜂','🏠','♈','🔮','🌳'];
      ritualDiv.innerHTML = '<div style="font-size:1.8rem;margin-bottom:.5rem;filter:drop-shadow(0 0 8px rgba(201,168,76,.3))">' + (_ritualIcons[currentPhase] || '✦') + '</div>' +
        '<div style="font-size:.88rem;color:var(--c-gold);font-weight:600">' + _ritualMsgs[currentPhase] + '</div>' +
        '<div style="width:60px;height:2px;margin:.6rem auto;background:rgba(201,168,76,.15);border-radius:99px;overflow:hidden"><div style="width:0%;height:100%;background:var(--c-gold);border-radius:99px;transition:width 1.2s ease-out" id="ootk-ritual-bar"></div></div>';
      phasesEl.appendChild(ritualDiv);
      requestAnimationFrame(function() { ritualDiv.style.opacity = '1'; var bar = document.getElementById('ootk-ritual-bar'); if (bar) bar.style.width = '100%'; });
  ...
  async function _triggerOOTKAI(results) {
    ...
    try {
      ...
      // ★ v21：OOTK 本身就是深度解讀，不顯示七維度升級 CTA
    ...
    wrap.innerHTML = '<div style="text-align:center;padding:2rem 1.2rem 2.3rem">' +
      ...
      '<div style="width:min(280px,82%);height:3px;border-radius:999px;background:rgba(201,168,76,.08);overflow:hidden;margin:.95rem auto .4rem">' +
        '<div id="ootk-loading-bar" style="width:6%;height:100%;border-radius:999px;background:linear-gradient(90deg,rgba(201,168,76,.6),rgba(201,168,76,.95),rgba(255,236,184,.9));animation:jyBarSlow 55s cubic-bezier(.3,.1,.2,1) forwards"></div>' +
      '</div>' +
    ...
    var phaseTimer = setInterval(function() {
      ...
      // tag 逐一亮燈（7 phases 分配給 5 tags）
      var tagWrap = document.getElementById('ootk-loading-tags');
      if (tagWrap) {
        var tags = tagWrap.querySelectorAll('span');
        var lightIdx = Math.min(Math.floor(phaseIdx * 5 / phases.length), tags.length - 1);
