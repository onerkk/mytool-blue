    ...
    if (def && def.deckFilter === 'minor_only' && typeof deckShuffled !== 'undefined') {
      ...
      if (deckWrap) {
        ...
        for (var fi = 0; fi < half; fi++) {
          ...
          var bgCss = imgUrl ? 'background-image:url(' + imgUrl + ');background-size:cover;background-position:center' : '';
          topHtml += '<div class="tarot-deck-card" data-idx="' + fi + '" style="--float-delay:' + d + 's"><div class="tarot-deck-card-inner" style="' + bgCss + '"></div></div>';
        }
        for (var fi = half; fi < deckShuffled.length; fi++) {
          var d = ((fi - half) * 0.13).toFixed(2);
          var imgUrl = (typeof getTarotCardImage === 'function') ? getTarotCardImage(deckShuffled[fi]) : '';
          var bgCss = imgUrl ? 'background-image:url(' + imgUrl + ');background-size:cover;background-position:center' : '';
          botHtml += '<div class="tarot-deck-card" data-idx="' + fi + '" style="--float-delay:' + d + 's"><div class="tarot-deck-card-inner" style="' + bgCss + '"></div></div>';
        }
...
(function() {
  ...
  function _runOOTKSequence(significatorId) {
    ...
    function advancePhase() {
      ...
      _advanceLock = true;
      setTimeout(function(){ _advanceLock = false; }, 800);
      try {
      currentPhase++;
      if (currentPhase >= 5) {
        // 所有階段完成 → 顯示 AI 分析按鈕
  ...
  function _renderKeyCards(keyCards) {
    ...
    var h = '<div style="margin-top:.5rem;display:flex;flex-wrap:wrap;gap:.4rem;justify-content:center">';
    keyCards.slice(0, 8).forEach(function(kc) {
      ...
      var dColor = dignity === 'strengthen' ? 'rgba(34,197,94,.4)' : dignity === 'weaken' ? 'rgba(239,68,68,.4)' : 'rgba(201,168,76,.2)';
      h += '<div style="text-align:center;width:52px">';
      if (imgSrc) {
        h += '<img src="' + imgSrc + '" style="width:44px;height:66px;border-radius:4px;object-fit:cover;border:1.5px solid ' + dColor + ';box-shadow:0 2px 6px rgba(0,0,0,.3)">';
      }
      h += '<div style="font-size:.52rem;color:var(--c-gold);margin-top:.2rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + name + '</div>';
  ...
  async function _triggerOOTKAI(results) {
    ...
    var phaseTimer = setInterval(function() {
      ...
      // tag 逐一亮燈（直接對應 phase：四元素→宮位→星座→旬→生命之樹）
