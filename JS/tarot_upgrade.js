...
var SPREAD_DEFS = {
  ...
  tree_of_life: {
    id: 'tree_of_life', zh: 'Tree of Life（生命之樹）', count: 10,
  ...
  ootk: {
    id: 'ootk', zh: 'Opening of the Key（開鑰之法）', count: 0,
    en: 'Opening of the Key',
...
function detectSpreadType(question, type) {
  ...
  // 3.7. 開鑰之法 — 只在用戶明確要求或選擇時觸發
  if (/開鑰|opening.*key|ootk|最深入|最完整.*占卜|金色黎明.*占卜/.test(q)) {
    return 'ootk';
  }
...
(function() {
  ...
  if (!document.getElementById('jy-spread-css')) {
    ...
    st.textContent = [
      ...
      '#t-chosen .jy-lbl{font-size:.65rem;color:var(--c-gold,#d4af37);font-weight:600}',
      ...
      '#t-chosen .jy-wrap .tarot-chosen-slot{position:relative!important;width:62px!important;height:92px!important;border:1.5px dashed rgba(212,175,55,.25)!important;border-radius:8px!important;display:flex!important;align-items:center!important;justify-content:center!important;flex-direction:column!important;flex-shrink:0!important;background:rgba(212,175,55,.03)!important}',
      '#t-chosen .jy-wrap .tarot-chosen-slot .slot-label{position:absolute;bottom:-14px;font-size:.48rem;color:var(--c-gold,#d4af37);white-space:nowrap;text-align:center;width:80px;left:50%;transform:translateX(-50%);opacity:.6}',
      '#t-chosen .jy-wrap .tarot-chosen-slot .slot-num{font-size:.55rem;opacity:.4;color:var(--c-gold,#d4af37)}'
    ].join('\n');
  ...
  function buildSlotLayout(spreadId, def) {
    ...
    if (spreadId === 'celtic_cross') {
      ...
      // ── 黃道十二宮：圓形 12 宮 + 中心總結牌 ──
      h += '<style>#t-chosen .jy-zodiac{position:relative;width:280px;height:280px;margin:0 auto}';
  ...
  window.initTarotDeck = function() {
    ...
    if (def && def.deckFilter === 'minor_only' && typeof deckShuffled !== 'undefined') {
      ...
      // 重新渲染牌組 UI（只顯示 56 張小阿卡那）
      var deckWrap = document.querySelector('.tarot-deck');
      if (deckWrap) {
        deckWrap.innerHTML = '';
        var showCount = deckShuffled.length;
        for (var fi = 0; fi < showCount; fi++) {
          var card = deckShuffled[fi];
          var el = document.createElement('div');
          el.className = 'tarot-card-back';
          el.dataset.idx = fi;
          el.onclick = (function(idx, elem) { return function() { pickCard(idx, elem); }; })(fi, el);
    ...
    try {
      ...
      if (pickHint) pickHint.textContent = '← 滑動瀏覽，選出 ' + targetCount + ' 張牌 →';
    } catch(e) {}
  };
...
// ══════════════════════════════════════════════════════════════════════
// 9. 開鑰之法 (Opening of the Key) — 核心計算引擎
// ══════════════════════════════════════════════════════════════════════
...
(function() {
  ...
  var TRUMP_COUNT = {
    ...
    6:12,   // 戀人(雙子=星座)
    7:12,   // 戰車(巨蟹=星座)
    8:9,    // 力量(獅子→但GD原版是 Strength=Leo=星座…保留12)
    9:12,   // 隱者(處女=星座) — 修正：GD 隱者=處女=星座
    10:9,   // 命運之輪(木星=行星)
    11:12,  // 正義(天秤=星座)
    12:3,   // 吊人(水=元素)
    13:12,  // 死神(天蠍=星座)
  ...
  function getCountValue(card) {
    if (!card) return 1;
    var s = card.suit || '';
    var r = card.rank != null ? card.rank : -1;
    // 大阿爾克那
    if (s === 'major') return TRUMP_COUNT[card.id] || 3;
    // 小阿爾克那
    if (r === 0) return 5; // Ace → 5 (Crowley簡化版)
    if (r >= 2 && r <= 10) return r; // 數字牌 → 面值
    // 宮廷牌
    var n = (card.n || card.name || '').toLowerCase();
    if (/侍者|侍從|公主|princess|page/.test(n)) return 7;
    return 4; // 騎士/王子/皇后/國王 → 4
  }
  ...
  function getCardElement(card) {
    ...
    if (!el) {
      var suit = card.suit || '';
      if (suit === 'wands' || suit === '火') return '火';
      if (suit === 'cups' || suit === '水') return '水';
      if (suit === 'swords' || suit === '風') return '風';
      if (suit === 'pentacles' || suit === '土') return '土';
    }
    // 元素名標準化
    if (/火|wands|fire/.test(el)) return '火';
    if (/水|cups|water/.test(el)) return '水';
    if (/風|swords|air/.test(el)) return '風';
    if (/土|pentacles|earth/.test(el)) return '土';
    return el;
  }
  ...
  // ── Significator 自動選擇 ──
  // 金色黎明：根據太陽星座元素 + 性別/年齡 → 宮廷牌
  var SIGN_ELEMENT = {
    '牡羊':'火','金牛':'土','雙子':'風','巨蟹':'水',
    '獅子':'火','處女':'土','天秤':'風','天蠍':'水',
    '射手':'火','摩羯':'土','水瓶':'風','雙魚':'水'
  };
  // 月份→星座（簡化：不用精確日期，用中間值）
  ...
  // 元素→宮廷牌花色
  var EL_SUIT = {'火':'wands','水':'cups','風':'swords','土':'pentacles'};
  // 宮廷牌 rank: 國王=63/49/35/21 系列（在 TAROT 陣列裡的 id）
  // 簡化：直接從 TAROT 找
  function findCourtCard(element, gender, age) {
    if (typeof TAROT === 'undefined') return null;
    var suitTarget = EL_SUIT[element] || 'cups';
    // 根據性別+年齡選 rank
    var rankTarget;
    if (gender === '女' || gender === 'female' || gender === 'F') {
      rankTarget = (age && age < 30) ? 'page' : 'queen';
    } else {
      rankTarget = (age && age < 30) ? 'knight' : 'king';
    }
    // 在 TAROT 裡找
    for (var i = 0; i < TAROT.length; i++) {
      ...
      var matchSuit = false;
      if (suitTarget === 'wands' && /權杖/.test(c.n)) matchSuit = true;
      if (suitTarget === 'cups' && /聖杯/.test(c.n)) matchSuit = true;
      if (suitTarget === 'swords' && /寶劍/.test(c.n)) matchSuit = true;
      if (suitTarget === 'pentacles' && /錢幣/.test(c.n)) matchSuit = true;
      if (!matchSuit) continue;
      var matchRank = false;
      if (rankTarget === 'king' && /國王/.test(c.n)) matchRank = true;
      if (rankTarget === 'queen' && /皇后/.test(c.n)) matchRank = true;
      if (rankTarget === 'knight' && /騎士/.test(c.n)) matchRank = true;
      if (rankTarget === 'page' && /侍者|侍從/.test(c.n)) matchRank = true;
      if (matchRank) return c;
    }
    return null;
  }
  ...
  function ootkOp1(deck, significatorId) {
    // 1. 把 78 張牌分成 4 堆（模擬切牌：每第 n 張分到第 n%4 堆）
    var piles = { fire: [], water: [], air: [], earth: [] };
    var pileKeys = ['fire', 'water', 'air', 'earth']; // YHVH
    deck.forEach(function(card, i) {
      piles[pileKeys[i % 4]].push(card);
    });

    // 2. 找 Significator 在哪堆
    var activePile = '';
    for (var pk in piles) {
  ...
  function ootkOp2(deck, significatorId) {
    ...
    deck.forEach(function(card, i) { houses[i % 12].push(card); });
    ...
    var houseMeanings = [
      '自我、身體、外貌','財務、價值觀、資源','溝通、學習、兄弟',
      '家庭、根基、父親','創造、戀愛、子女、快樂','健康、工作、日常',
      '伴侶、合夥、公開敵人','轉化、共同資產、深層親密','遠方、信仰、高等教育',
      '事業、地位、社會形象','社群、理想、朋友','隱藏、業力、潛意識'
    ];
  ...
  function ootkOp3(deck, significatorId) {
    ...
    deck.forEach(function(card, i) { signs[i % 12].push(card); });
  ...
  // ── Operation 4：三十六旬 ──
  // 36 旬：每堆約 2 張。簡化版用 i%36 分堆。
  var DECAN_MAP = [
    {sign:'牡羊',range:'1°-10°',card:'權杖二'},{sign:'牡羊',range:'11°-20°',card:'權杖三'},{sign:'牡羊',range:'21°-30°',card:'權杖四'},
    {sign:'金牛',range:'1°-10°',card:'錢幣五'},{sign:'金牛',range:'11°-20°',card:'錢幣六'},{sign:'金牛',range:'21°-30°',card:'錢幣七'},
    {sign:'雙子',range:'1°-10°',card:'寶劍八'},{sign:'雙子',range:'11°-20°',card:'寶劍九'},{sign:'雙子',range:'21°-30°',card:'寶劍十'},
    {sign:'巨蟹',range:'1°-10°',card:'聖杯二'},{sign:'巨蟹',range:'11°-20°',card:'聖杯三'},{sign:'巨蟹',range:'21°-30°',card:'聖杯四'},
    {sign:'獅子',range:'1°-10°',card:'權杖五'},{sign:'獅子',range:'11°-20°',card:'權杖六'},{sign:'獅子',range:'21°-30°',card:'權杖七'},
    {sign:'處女',range:'1°-10°',card:'錢幣八'},{sign:'處女',range:'11°-20°',card:'錢幣九'},{sign:'處女',range:'21°-30°',card:'錢幣十'},
    {sign:'天秤',range:'1°-10°',card:'寶劍二'},{sign:'天秤',range:'11°-20°',card:'寶劍三'},{sign:'天秤',range:'21°-30°',card:'寶劍四'},
    {sign:'天蠍',range:'1°-10°',card:'聖杯五'},{sign:'天蠍',range:'11°-20°',card:'聖杯六'},{sign:'天蠍',range:'21°-30°',card:'聖杯七'},
    {sign:'射手',range:'1°-10°',card:'權杖八'},{sign:'射手',range:'11°-20°',card:'權杖九'},{sign:'射手',range:'21°-30°',card:'權杖十'},
    {sign:'摩羯',range:'1°-10°',card:'錢幣二'},{sign:'摩羯',range:'11°-20°',card:'錢幣三'},{sign:'摩羯',range:'21°-30°',card:'錢幣四'},
    {sign:'水瓶',range:'1°-10°',card:'寶劍五'},{sign:'水瓶',range:'11°-20°',card:'寶劍六'},{sign:'水瓶',range:'21°-30°',card:'寶劍七'},
    {sign:'雙魚',range:'1°-10°',card:'聖杯八'},{sign:'雙魚',range:'11°-20°',card:'聖杯九'},{sign:'雙魚',range:'21°-30°',card:'聖杯十'}
  ];

  function ootkOp4(deck, significatorId) {
    var decans = [];
    for (var d = 0; d < 36; d++) decans.push([]);
    deck.forEach(function(card, i) { decans[i % 36].push(card); });
  ...
  // ── Operation 5：生命之樹 ──
  var SEPHIROTH = [
    {name:'Kether',zh:'王冠',meaning:'精神目標、最高指引'},
    {name:'Chokmah',zh:'智慧',meaning:'創造衝動、原始動力'},
    {name:'Binah',zh:'理解',meaning:'結構限制、必須面對的現實'},
    {name:'Chesed',zh:'慈悲',meaning:'擴展、機會、恩典'},
    {name:'Geburah',zh:'力量',meaning:'收縮、割捨、紀律'},
    {name:'Tiphereth',zh:'美',meaning:'核心自我、和諧、平衡'},
    {name:'Netzach',zh:'勝利',meaning:'情感、慾望、愛'},
    {name:'Hod',zh:'榮耀',meaning:'思維、溝通、理性'},
    {name:'Yesod',zh:'基礎',meaning:'潛意識、想像、基礎'},
    {name:'Malkuth',zh:'王國',meaning:'物質現實、具體結果'}
  ];

  function ootkOp5(deck, significatorId) {
    var sephirot = [];
    for (var s = 0; s < 10; s++) sephirot.push([]);
    deck.forEach(function(card, i) { sephirot[i % 10].push(card); });
  ...
  function ootkCounting(cards, startIdx) {
    ...
    for (var step = 0; step < maxSteps; step++) {
      ...
      path.push({ cardId: card.id, cardName: card.n || card.name, position: idx, countValue: count });
      // 向右移動（環繞）
  ...
  function ootkPairing(cards, sigIdx) {
    ...
    while (left >= 0 && right < cards.length) {
      pairs.push({ left: cards[left], right: cards[right] });
      left--;
      right++;
    }
    return pairs;
  ...
  function runFullOOTK(significatorId) {
    ...
    function shuffleDeck() {
      var deck = TAROT.map(function(c, i) {
        return Object.assign({}, c, { isUp: Math.random() > 0.35 });
      });
    ...
    results.significator = sigCard ? { id: sigCard.id, name: sigCard.n, element: getCardElement(sigCard) } : null;

    results.op1 = ootkOp1(shuffleDeck(), significatorId);
    results.op2 = ootkOp2(shuffleDeck(), significatorId);
    results.op3 = ootkOp3(shuffleDeck(), significatorId);
    results.op4 = ootkOp4(shuffleDeck(), significatorId);
    results.op5 = ootkOp5(shuffleDeck(), significatorId);
    ...
    for (var cid in allKeyCardIds) {
      if (allKeyCardIds[cid].length >= 2) {
        ...
        recurring.push(cn + '（出現在' + allKeyCardIds[cid].join('+') + '）');
      }
    }
  ...
  console.log('[OOTK] Opening of the Key 引擎已載入');
})();
...
(function() {
  ...
  var PILE_COLOR = { fire: '#ef4444', water: '#3b82f6', air: '#a78bfa', earth: '#22c55e' };
  ...
  function _injectOOTKStyles() {
    ...
    s.textContent = [
      ...
      '.ootk-sig-card{width:80px;height:128px;border-radius:8px;object-fit:cover;border:2px solid rgba(212,175,55,.5);transition:all .3s;cursor:pointer}',
      '.ootk-sig-card:hover,.ootk-sig-card.active{border-color:var(--c-gold);box-shadow:0 0 20px rgba(212,175,55,.4);transform:scale(1.06)}',
      '.ootk-phase{opacity:0;transform:translateY(20px);transition:opacity .6s ease,transform .6s ease}',
      '.ootk-phase.visible{opacity:1;transform:translateY(0)}',
      ...
      '.ootk-dot.done{background:rgba(212,175,55,.7)}',
      '.ootk-dot.current{background:var(--c-gold);box-shadow:0 0 8px rgba(212,175,55,.5)}',
      '.ootk-result-card{background:rgba(255,255,255,.04);border:1px solid rgba(212,175,55,.12);border-radius:12px;padding:.8rem;margin:.4rem 0}',
      '.ootk-pile-bar{height:6px;border-radius:99px;transition:width .8s ease-out}',
      '.ootk-key-card{display:inline-flex;align-items:center;gap:.3rem;padding:.2rem .5rem;border-radius:6px;background:rgba(212,175,55,.08);border:1px solid rgba(212,175,55,.15);font-size:.72rem;color:var(--c-gold);margin:.15rem}',
      ...
      '@keyframes ootkGlow{0%,100%{box-shadow:0 0 15px rgba(212,175,55,.1)}50%{box-shadow:0 0 30px rgba(212,175,55,.25)}}',
  ...
  function _showSignificatorSelection(onSelect) {
    ...
    html += '<div style="font-size:.78rem;color:var(--c-text-dim);margin-bottom:.6rem">Opening of the Key — 金色黎明最高階占卜</div>';
    html += '<div style="font-size:.82rem;color:var(--c-text,#e0d8c8);line-height:1.7;margin-bottom:1rem;padding:0 .5rem">五個階段，從四元素到生命之樹，逐層深入你的問題核心。<br>首先，選擇代表你的「代表牌」(Significator)。</div>';
    ...
    if (autoCard) {
      ...
      if (imgSrc) html += '<img src="' + imgSrc + '" style="width:64px;height:102px;border-radius:6px;object-fit:cover;border:1.5px solid rgba(212,175,55,.4)">';
      html += '<div style="text-align:left">';
      html += '<div style="font-size:.95rem;font-weight:700;color:var(--c-gold-pale,#f5e6b8)">' + (autoCard.n || '') + '</div>';
      html += '<div style="font-size:.72rem;color:var(--c-text-dim);margin-top:.2rem">' + (auto.element || '') + '元素宮廷牌</div>';
      html += '</div></div>';
      html += '<button id="ootk-use-auto" style="margin-top:.6rem;padding:.5rem 1.5rem;border-radius:20px;background:linear-gradient(135deg,rgba(212,175,55,.2),rgba(212,175,55,.1));border:1.5px solid rgba(212,175,55,.4);color:var(--c-gold);font-weight:700;font-size:.85rem;cursor:pointer;font-family:inherit">用這張，開始占卜</button>';
      html += '</div>';
    }

    // 手動選擇
    html += '<div style="font-size:.75rem;color:var(--c-text-muted);margin-bottom:.5rem">或者手動選擇：</div>';
    html += '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.4rem;max-height:280px;overflow-y:auto;padding:.3rem">';
    courtCards.forEach(function(cc) {
      var img = (typeof getTarotCardImage === 'function') ? getTarotCardImage(cc) : '';
      var isAuto = (cc.id === autoId);
      html += '<div class="ootk-manual-sig" data-id="' + cc.id + '" style="text-align:center;cursor:pointer;padding:.3rem;border-radius:8px;border:1px solid ' + (isAuto ? 'rgba(212,175,55,.4)' : 'rgba(255,255,255,.06)') + ';transition:all .2s">';
      if (img) html += '<img src="' + img + '" style="width:48px;height:76px;border-radius:4px;object-fit:cover">';
      html += '<div style="font-size:.6rem;color:var(--c-text-dim);margin-top:.15rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + (cc.n || '') + '</div>';
      html += '</div>';
    });
    html += '</div>';

    // 關閉按鈕
    html += '<div style="margin-top:.8rem"><button id="ootk-cancel" style="padding:.4rem 1rem;border-radius:16px;background:transparent;border:1px solid rgba(255,255,255,.15);color:var(--c-text-dim);font-size:.75rem;cursor:pointer;font-family:inherit">取消</button></div>';
    html += '</div>';
    ...
    // 事件綁定
    var autoBtn = document.getElementById('ootk-use-auto');
    if (autoBtn) autoBtn.addEventListener('click', function() {
      overlay.remove();
      onSelect(autoId);
    });
    document.getElementById('ootk-cancel').addEventListener('click', function() { overlay.remove(); });
  ...
  function _runOOTKSequence(significatorId) {
    ...
    // 跑五階段計算
    var results = window.ootkRunFull ? window.ootkRunFull(significatorId) : null;
    ...
    S.tarot.spreadDef = { id: 'ootk', zh: 'Opening of the Key（開鑰之法）' };
    ...
    html += '<button id="ootk-next" style="padding:.55rem 1.5rem;border-radius:20px;background:linear-gradient(135deg,rgba(212,175,55,.2),rgba(212,175,55,.1));border:1.5px solid rgba(212,175,55,.4);color:var(--c-gold);font-weight:700;font-size:.85rem;cursor:pointer;font-family:inherit">開始第一階段 →</button>';
    html += '</div>';
    ...
    function advancePhase() {
      ...
      var phaseHtml = _renderPhase(currentPhase, label, opData, results);
      ...
      requestAnimationFrame(function() {
        requestAnimationFrame(function() { phaseDiv.classList.add('visible'); });
      });

      // 滾到這個階段
      setTimeout(function() { phaseDiv.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 200);

      // 更新按鈕
      if (currentPhase < 4) {
        nextBtn.textContent = OP_LABELS[currentPhase + 1].zh + ' →';
      } else {
        nextBtn.textContent = '查看全部結果 →';
      }
    }
  ...
  function _renderPhase(phaseIdx, label, opData, allResults) {
    var h = '';
    h += '<div class="ootk-result-card ootk-reveal" style="animation-delay:' + (phaseIdx * 0.1) + 's">';

    // 階段頭
    h += '<div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem">';
    h += '<span style="font-size:1.3rem">' + label.icon + '</span>';
    h += '<div><div style="font-size:.9rem;font-weight:700;color:var(--c-gold-pale,#f5e6b8)">' + label.zh + '</div>';
    h += '<div style="font-size:.68rem;color:var(--c-text-dim)">' + label.desc + '</div></div>';
    h += '</div>';
  ...
  function _renderOp1(op) {
    ...
    ['fire','water','air','earth'].forEach(function(k) {
      ...
      h += '<div style="display:flex;align-items:center;gap:.4rem;margin:.25rem 0">';
      h += '<span style="font-size:.7rem;width:70px;color:' + (isActive ? PILE_COLOR[k] : 'var(--c-text-dim)') + ';font-weight:' + (isActive ? '700' : '400') + '">' + PILE_ZH[k] + '</span>';
      h += '<div style="flex:1;height:6px;border-radius:99px;background:rgba(255,255,255,.06);overflow:hidden">';
      h += '<div class="ootk-pile-bar" style="width:' + pct + '%;background:' + PILE_COLOR[k] + ';opacity:' + (isActive ? '1' : '.35') + '"></div>';
      h += '</div>';
      h += '<span style="font-size:.65rem;color:var(--c-text-muted);width:28px;text-align:right">' + n + '張</span>';
      h += '</div>';
    });
    h += '</div>';

    // 結果
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.1);margin-top:.3rem">';
    h += '<div style="font-size:.82rem;color:var(--c-gold);font-weight:600">你的代表牌落入 ' + (PILE_ZH[op.activePile] || op.activePile) + '</div>';
    h += '<div style="font-size:.75rem;color:var(--c-text-dim);margin-top:.2rem">問題核心：' + (op.meaning || '') + '</div>';
    h += '</div>';
  ...
  function _renderOp2(op) {
    ...
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.1)">';
    h += '<div style="font-size:.82rem;color:var(--c-gold);font-weight:600">代表牌落在 第' + ((op.activeHouse || 0) + 1) + '宮</div>';
    h += '<div style="font-size:.75rem;color:var(--c-text-dim);margin-top:.2rem">' + (HOUSE_ZH[op.activeHouse] || '') + '</div>';
    h += '</div>';

    // 宮位分佈圓形指示
    h += '<div style="display:flex;flex-wrap:wrap;gap:.2rem;margin-top:.4rem">';
    var houseCounts = op.houseCounts || [];
    for (var i = 0; i < 12; i++) {
      var cnt = houseCounts[i] || (op.activeHouse === i ? '★' : '·');
      var isActive = (op.activeHouse === i);
      h += '<span style="width:calc(25% - .15rem);text-align:center;padding:.2rem 0;border-radius:4px;font-size:.6rem;' +
        (isActive ? 'background:rgba(212,175,55,.12);color:var(--c-gold);font-weight:700;border:1px solid rgba(212,175,55,.2)' : 'color:var(--c-text-muted);background:rgba(255,255,255,.03)') + '">' +
        (i+1) + '宮' + (typeof cnt === 'number' ? '(' + cnt + ')' : '') + '</span>';
    }
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    return h;
  ...
  function _renderOp3(op) {
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.1)">';
    h += '<div style="font-size:.82rem;color:var(--c-gold);font-weight:600">代表牌落在 ' + (op.activeSign || '?') + '</div>';
    if (op.rulingMajor) {
      h += '<div style="font-size:.75rem;color:var(--c-text-dim);margin-top:.2rem">主牌：' + (op.rulingMajor || '') + '</div>';
    }
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    return h;
  ...
  function _renderOp4(op) {
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(212,175,55,.06);border:1px solid rgba(212,175,55,.1)">';
    h += '<div style="font-size:.82rem;color:var(--c-gold);font-weight:600">' + (op.decanSign || '') + ' ' + (op.decanRange || '') + '</div>';
    if (op.decanRuler) {
      h += '<div style="font-size:.75rem;color:var(--c-text-dim);margin-top:.2rem">旬主：' + op.decanRuler + '</div>';
    }
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    return h;
  ...
  function _renderOp5(op, allResults) {
    var SEPH_ZH = ['Kether 王冠','Chokmah 智慧','Binah 理解','Chesed 慈悲','Geburah 嚴厲','Tiphareth 美','Netzach 勝利','Hod 榮耀','Yesod 基礎','Malkuth 王國'];
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(139,92,246,.06);border:1px solid rgba(139,92,246,.12)">';
    h += '<div style="font-size:.82rem;color:#a78bfa;font-weight:600">代表牌落在 ' + (SEPH_ZH[op.activeSephirah] || ('第' + ((op.activeSephirah||0)+1) + '球體')) + '</div>';
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    ...
    if (allResults && allResults.crossAnalysis) {
      ...
      h += '<div style="margin-top:.5rem;padding:.6rem;border-radius:8px;border:1px solid rgba(212,175,55,.15);background:linear-gradient(135deg,rgba(212,175,55,.04),rgba(139,92,246,.04))">';
      h += '<div style="font-size:.78rem;font-weight:700;color:var(--c-gold);margin-bottom:.3rem">五階段能量流動</div>';
      h += '<div style="font-size:.72rem;color:var(--c-text-dim);line-height:1.7">' + (ca.elementProgression || '') + '</div>';
      if (ca.recurringCards && ca.recurringCards.length) {
        h += '<div style="margin-top:.3rem;font-size:.72rem;color:var(--c-gold-pale,#f5e6b8)">重複出現的牌：' + ca.recurringCards.join('、') + '</div>';
      }
      h += '</div>';
    }
    return h;
  ...
  function _renderKeyCards(keyCards) {
    ...
    var h = '<div style="margin-top:.35rem;display:flex;flex-wrap:wrap;gap:.15rem">';
    keyCards.slice(0, 8).forEach(function(kc) {
      var name = kc.card ? (kc.card.n || kc.card.name || '') : '';
      var dignity = kc.dignity || '';
      var dColor = dignity === 'strengthen' ? '#22c55e' : dignity === 'weaken' ? '#ef4444' : 'var(--c-text-muted)';
      h += '<span class="ootk-key-card" style="border-color:' + dColor + '">' + name;
      if (kc.step) h += ' <span style="opacity:.6">(' + kc.step + '步)</span>';
      h += '</span>';
    });
    h += '</div>';
    return h;
  ...
  // ── 觸發 OOTK AI 分析 ──
  function _triggerOOTKAI(results) {
    // 把 OOTK 結果存到全域，讓 _buildOOTKPayload 取得
    window._ootkResults = results;

    // 顯示結果區
    var spreadSec = document.getElementById('t-spread-sec');
    if (spreadSec) spreadSec.classList.remove('hidden');

    var wrap = document.getElementById('tarot-ai-wrap');
    if (!wrap) {
      // 如果沒有 tarot-ai-wrap，嘗試在 t-spread-sec 後面建立
      wrap = document.createElement('div');
      wrap.id = 'tarot-ai-wrap';
      if (spreadSec) spreadSec.after(wrap);
      else document.body.appendChild(wrap);
    }

    // 觸發 AI（用既有的 _triggerTarotAI，它會偵測 ootk 模式）
    if (typeof _triggerTarotAI === 'function') {
      _triggerTarotAI();
    } else {
      wrap.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--c-text-dim)">AI 解讀功能未載入</div>';
    }
  }
