...
(function() {
  ...
  if (!document.getElementById('jy-spread-css')) {
    ...
    st.textContent = [
      ...
      '#t-chosen .jy-wrap .tarot-chosen-slot{position:relative!important;width:62px!important;height:92px!important;border:1px dashed rgba(255,255,255,.08)!important;border-radius:8px!important;display:flex!important;align-items:center!important;justify-content:center!important;flex-direction:column!important;flex-shrink:0!important;background:rgba(255,255,255,.02)!important}',
      '#t-chosen .jy-wrap .tarot-chosen-slot .slot-label{position:absolute;bottom:-14px;font-size:.48rem;color:rgba(255,255,255,.3);white-space:nowrap;text-align:center;width:80px;left:50%;transform:translateX(-50%);opacity:.6}',
      '#t-chosen .jy-wrap .tarot-chosen-slot .slot-num{font-size:.55rem;opacity:.3;color:rgba(255,255,255,.25)}'
    ].join('\n');
...
(function() {
  ...
  function ootkOp1(deck, significatorId) {
    // 模擬切四堆：3 個隨機切點，堆大小不均等
    var len = deck.length; // 78
    var cuts = [];
    while (cuts.length < 3) {
      var c = Math.floor(Math.random() * (len - 4)) + 1; // 確保每堆至少 1 張
      if (cuts.indexOf(c) === -1) cuts.push(c);
    }
    ...
    var pileMeaning = {
      fire: '工作、事業、行動、意志（Yod）',
      water: '愛情、婚姻、快樂、情感（Heh）',
      air: '煩惱、損失、衝突、思維（Vav）',
      earth: '金錢、物質、實際事務（Heh final）'
    };
  ...
  function ootkOp2(deck, significatorId) {
    ...
    deck.forEach(function(card) {
      var houseNum = getCardHouse(card); // 1-12
      houses[houseNum - 1].push(card);
    });
  ...
  function ootkCounting(cards, startIdx) {
    ...
    for (var step = 0; step < maxSteps; step++) {
      ...
      // 正位向右，逆位向左
      var direction = (card.isUp !== false) ? 1 : -1;
      path.push({
        cardId: card.id,
  ...
  function runFullOOTK(significatorId) {
    ...
    results.op2 = ootkOp2(deck, significatorId);
    results.op3 = ootkOp3(deck, significatorId);
    results.op4 = ootkOp4(deck, significatorId);
    results.op5 = ootkOp5(deck, significatorId);
    ...
    for (var cid in allKeyCardIds) {
      if (allKeyCardIds[cid].length >= 2) {
        ...
        recurring.push(cn + '（出現在' + allKeyCardIds[cid].map(function(k) { return opZh[k] || k; }).join('、') + '）');
      }
    }
...
(function() {
  ...
  function _runOOTKSequence(significatorId) {
    ...
    function advancePhase() {
      ...
      } catch(err) { console.error('[OOTK advancePhase] Error:', err); alert('階段載入失敗：' + err.message); }
    }
  ...
  function _renderOp1(op) {
    ...
    // 結果
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.1);margin-top:.3rem">';
  ...
  function _renderOp2(op) {
    ...
    var activeH = op.activeHouse || 1; // 1-indexed
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.1)">';
    h += '<div style="font-size:.82rem;color:var(--c-gold);font-weight:600">代表牌落在 第' + activeH + '宮</div>';
    h += '<div style="font-size:.75rem;color:var(--c-text-dim);margin-top:.2rem">' + (HOUSE_ZH[activeH - 1] || '') + '</div>';
    if (op.meaning) h += '<div style="font-size:.72rem;color:var(--c-text-muted);margin-top:.15rem">' + op.meaning + '</div>';
    h += '</div>';
    ...
    for (var i = 0; i < 12; i++) {
      ...
      h += '<span style="width:calc(25% - .15rem);text-align:center;padding:.2rem 0;border-radius:4px;font-size:.6rem;' +
        (isActive ? 'background:rgba(201,168,76,.12);color:var(--c-gold);font-weight:700;border:1px solid rgba(201,168,76,.2)' : 'color:var(--c-text-muted);background:rgba(255,255,255,.03)') + '">' +
        (i+1) + '宮' + (cnt > 0 ? '(' + cnt + ')' : '') + '</span>';
    }
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    return h;
  ...
  function _renderOp5(op, allResults) {
    ...
    var h = '';
    h += '<div style="padding:.5rem;border-radius:8px;background:rgba(201,168,76,.06);border:1px solid rgba(201,168,76,.12)">';
    h += '<div style="font-size:.82rem;color:#c9a84c;font-weight:600">代表牌落在 ' + sephLabel + '</div>';
    if (op.sephirahMeaning) h += '<div style="font-size:.7rem;color:var(--c-text-dim);margin-top:.2rem">' + op.sephirahMeaning + '</div>';
    h += '</div>';
    h += _renderKeyCards(op.keyCards);
    ...
    if (allResults && allResults.crossAnalysis) {
      ...
      h += '<div style="margin-top:.5rem;padding:.6rem;border-radius:8px;border:1px solid rgba(201,168,76,.15);background:linear-gradient(135deg,rgba(201,168,76,.04),rgba(201,168,76,.04))">';
  ...
  async function _triggerOOTKAI(results) {
    ...
    wrap.innerHTML = '<div style="text-align:center;padding:2rem 1.2rem 2.3rem">' +
      '<div style="position:relative;width:128px;height:128px;margin:0 auto .95rem">' +
        '<div style="position:absolute;inset:0;border-radius:50%;background:radial-gradient(circle,rgba(201,168,76,.16) 0%,rgba(22,163,74,.08) 26%,transparent 70%);animation:jyPulseGlow 3.4s ease-in-out infinite"></div>' +
        '<div style="position:absolute;left:50%;top:50%;width:104px;height:104px;transform:translate(-50%,-50%);border-radius:50%;border:1px solid rgba(201,168,76,.10);box-shadow:0 0 24px rgba(22,163,74,.08) inset"></div>' +
        ...
        '<div style="position:absolute;left:50%;top:50%;width:88px;height:88px;transform:translate(-50%,-50%);border-radius:50%;border:2px solid transparent;border-top-color:rgba(201,168,76,.7);border-right-color:rgba(22,163,74,.4);animation:jySpinGold 2s linear infinite"></div>' +
        '<div style="position:absolute;left:50%;top:50%;width:64px;height:64px;transform:translate(-50%,-50%);border-radius:50%;border:2px solid transparent;border-bottom-color:rgba(22,163,74,.6);border-left-color:rgba(201,168,76,.3);animation:jySpinPurple 3.1s linear infinite reverse"></div>' +
        '<div style="position:absolute;left:50%;top:50%;width:18px;height:18px;margin:-9px 0 0 -9px;border-radius:50%;background:radial-gradient(circle,rgba(201,168,76,.95) 0%,rgba(22,163,74,.25) 62%,transparent 100%);animation:jyBreath 2.4s ease-in-out infinite"></div>' +
      ...
      '<div style="width:min(280px,82%);height:3px;border-radius:999px;background:rgba(201,168,76,.08);overflow:hidden;margin:.7rem auto 0">' +
        '<div style="width:42%;height:100%;border-radius:999px;background:linear-gradient(90deg,rgba(201,168,76,0),rgba(201,168,76,.95),rgba(22,163,74,.9),rgba(201,168,76,0));background-size:180% 100%;animation:jyScan 1.8s ease-in-out infinite"></div>' +
      '</div>' +
      '<div style="margin-top:1.2rem;max-width:320px;margin-left:auto;margin-right:auto;padding:.7rem .85rem;border-radius:16px;border:1px solid rgba(201,168,76,.14);background:linear-gradient(180deg,rgba(201,168,76,.045),rgba(22,163,74,.035));box-shadow:0 10px 28px rgba(0,0,0,.18) inset,0 0 0 1px rgba(255,255,255,.02);position:relative;overflow:hidden">' +
        '<div style="position:absolute;inset:auto -30% 0 -30%;height:1px;background:linear-gradient(90deg,rgba(201,168,76,0),rgba(201,168,76,.26),rgba(22,163,74,.22),rgba(201,168,76,0));animation:jySweep 3.8s ease-in-out infinite"></div>' +
    ...
    var phaseTimer = setInterval(function() {
      ...
      var fill = document.getElementById('ootk-ai-progress');
    ...
    try {
      ...
      if (err.code === 'OOTK_PAYMENT_REQUIRED' || (err.status === 429 && !window._JY_ADMIN_TOKEN)) {
        // 需要付費 → 彈付費牆
        ...
        wrap.innerHTML = '<div style="text-align:center;padding:1.5rem">' +
          ...
          '<div style="font-size:.8rem;color:var(--c-text-dim);margin-bottom:.8rem">NT$40 · 五階段深度占卜</div>' +
