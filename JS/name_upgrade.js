...
function analyzeCharPhonetic(pinyin) {
  ...
  // ★ Bug #38 fix: ju/qu/xu/yu 的 u 實際上是 ü（漢語拼音規則：j/q/x/y 後面的 u 一律唸 ü）
  //   舊版會 match 到 'u' → '木'（錯，應為水）
  //   修法：偵測到 j/q/x/y 開頭時，把後面的 u/ue/un/uan 替換成 ü 系（v 系）
  if (['j','q','x','y'].indexOf(initial) >= 0) {
    if (finals.charAt(0) === 'u') {
      finals = 'v' + finals.substring(1); // u→v（即 ü）
    }
  }
...
function getShuliDetail(ge) {
  // ★ Bug #39 fix: 81 數理應該 mod 81 才對，舊版 mod 80 會把 81/161/241... 都壓成 1
  //   結果是「81 萬物回春」永遠出現不到。例如總格 81 → ((81-1)%80)+1 = 1（錯）
  //   修法後：((81-1)%81)+1 = 81（正確），((82-1)%81)+1 = 1（合理 wrap）
  var n = ((ge - 1) % 81 + 81) % 81 + 1;
  // 邊界保險：n 應該在 1-81 範圍內
  if (n < 1) n = 1;
  if (n > 81) n = ((n - 1) % 81) + 1;
...
function enhanceName(nameResult, bazi) {
  ...
  // 4. 音韻五行
  nameResult.phoneticNote = '音韻五行分析需提供各字拼音';

  // 5. 生肖姓名學
  if (nameResult.fullName && birthYear) {
    try { nameResult.zodiacName = analyzeZodiacNameCompat(nameResult.fullName, birthYear); }
    catch(e) { nameResult.zodiacName = null; }
  }

  return nameResult;
}

// ── 生肖姓名學：名字部首/結構 vs 生肖喜忌 ──
function analyzeZodiacNameCompat(fullName, birthYear) {
  if (!fullName || !birthYear) return null;
  var ZODIAC_CYCLE = ['鼠','牛','虎','兔','龍','蛇','馬','羊','猴','雞','狗','豬'];
  var zIdx = ((birthYear - 4) % 12 + 12) % 12;
  var zodiac = ZODIAC_CYCLE[zIdx];

  // 生肖喜忌部首表（簡化版，涵蓋主要部首）
  var ZODIAC_PREFS = {
    '鼠': { like: ['口','宀','米','豆','禾','田','艹','金','水','氵','王','夕'], dislike: ['日','火','灬','馬','午','羊','未','刀','力','石'], likeNote: '鼠喜洞穴（口、宀）、五穀（米禾豆）、夜行（夕）', dislikeNote: '鼠忌日光（日火）、天敵（馬羊）、刀兵' },
    '牛': { like: ['艹','禾','豆','米','田','宀','車','氵','水','木'], dislike: ['馬','午','羊','未','日','火','灬','心','忄','王'], likeNote: '牛喜草（艹禾）、穀倉（宀）、拉車（車）、水草', dislikeNote: '牛忌衝（馬午）、烈日、心（被宰）' },
    '虎': { like: ['山','木','林','王','君','氵','水','衣','衤','馬','午'], dislike: ['口','辶','廴','人','亻','日','蟲','虫','小'], likeNote: '虎喜山林（山木）、為王（王）、披彩衣（衣）', dislikeNote: '虎忌被關（口）、被馴（人）、蟲小' },
    '兔': { like: ['口','宀','艹','禾','木','月','犭','彡','衣','衤'], dislike: ['日','火','灬','金','刀','酉','雞','鷹','鳥'], likeNote: '兔喜洞穴（口宀）、草木（艹木）、月（玉兔）、彡（好看毛色）', dislikeNote: '兔忌日曬、金刀（被獵）、天敵（鷹鳥雞）' },
    '龍': { like: ['水','氵','雨','辰','日','月','星','王','大','天'], dislike: ['小','宀','口','田','土','蟲','虫','戈','弓'], likeNote: '龍喜水（雲行雨施）、日月星辰、大（稱王）', dislikeNote: '龍忌小（降格）、被困（宀口田）、蟲（降為蛇）' },
    '蛇': { like: ['口','宀','木','艹','田','衣','衤','彡','辶','廴'], dislike: ['日','火','灬','水','氵','人','亻','豕','虎'], likeNote: '蛇喜洞穴（口宀）、草叢（艹木）、蜿蜒（辶）、華服（衣彡）', dislikeNote: '蛇忌強光（日火）、水多（不利）、天敵（虎人豬）' },
    '馬': { like: ['艹','禾','木','巾','衣','衤','龍','辰','目','大'], dislike: ['田','口','山','水','氵','鼠','子','牛','丑'], likeNote: '馬喜草原（艹禾）、披鞍（巾衣）、龍馬配（辰）、大（奔馳）', dislikeNote: '馬忌被圍（田口山）、水深（溺）、沖（鼠牛）' },
    '羊': { like: ['艹','禾','豆','米','木','口','宀','足','金'], dislike: ['刀','力','皮','革','牛','丑','馬','午','犬','犭'], likeNote: '羊喜草穀（艹禾米豆）、棚（口宀）', dislikeNote: '羊忌刀刃（被宰）、犬（牧羊犬壓力）、衝（牛馬）' },
    '猴': { like: ['木','山','口','宀','王','君','人','亻','大','衣'], dislike: ['火','灬','金','刀','豕','虎','寅','水','氵'], likeNote: '猴喜樹林（木山）、洞穴（口宀）、稱王（王）、與人親近', dislikeNote: '猴忌火（火燒花果山）、金刀（被抓）、虎（天敵）' },
    '雞': { like: ['禾','米','豆','艹','山','丘','宀','彡','冠'], dislike: ['刀','力','犬','犭','兔','卯','水','氵','石'], likeNote: '雞喜五穀（禾米豆）、棲息（山宀）、華冠（彡冠）', dislikeNote: '雞忌刀刃（被宰）、犬（雞犬不寧）、卯沖' },
    '狗': { like: ['人','亻','口','宀','心','忄','月','小','少','士'], dislike: ['龍','辰','雞','酉','禾','米','豆','田','日'], likeNote: '狗喜人（忠犬）、洞穴（口宀）、肉食（心月）、小巧', dislikeNote: '狗忌辰（龍狗沖）、酉（雞犬不寧）、素食穀物' },
    '豬': { like: ['口','宀','門','豆','米','禾','艹','金','水','氵'], dislike: ['刀','力','皮','革','王','君','蛇','巳','猴','申'], likeNote: '豬喜洞穴豐食（口宀豆米）、金水', dislikeNote: '豬忌刀刃（被宰）、王（豬被獻祭）、巳申（沖害）' }
  };

  var pref = ZODIAC_PREFS[zodiac];
  if (!pref) return { zodiac: zodiac, note: '生肖喜忌資料不足' };

  // 簡易部首檢測（檢查名字每個字是否包含喜忌部首特徵）
  var chars = fullName.split('');
  var likeFound = [], dislikeFound = [];
  chars.forEach(function(ch, idx) {
    if (idx === 0) return; // 跳過姓
    pref.like.forEach(function(radical) {
      // 用 Unicode 範圍做粗略檢測（部首在字形裡）
      // 這是簡化版——完整版需要部首資料庫
      if (ch.indexOf(radical) >= 0 || _charContainsRadical(ch, radical)) {
        likeFound.push(ch + '含「' + radical + '」');
      }
    });
    pref.dislike.forEach(function(radical) {
      if (ch.indexOf(radical) >= 0 || _charContainsRadical(ch, radical)) {
        dislikeFound.push(ch + '含「' + radical + '」');
      }
    });
  });

  var score = likeFound.length * 2 - dislikeFound.length * 3;
  var level = score >= 3 ? '佳' : score >= 0 ? '中等' : '需留意';
  return {
    zodiac: zodiac,
    likeNote: pref.likeNote,
    dislikeNote: pref.dislikeNote,
    likeFound: likeFound.slice(0, 5),
    dislikeFound: dislikeFound.slice(0, 5),
    score: score,
    level: level,
    zh: '屬' + zodiac + '姓名學：' + level + (likeFound.length ? '（喜：' + likeFound.slice(0, 3).join('、') + '）' : '') + (dislikeFound.length ? '（忌：' + dislikeFound.slice(0, 3).join('、') + '）' : '')
  };
}

function _charContainsRadical(ch, radical) {
  // 簡易偏旁檢測：CJK Radical 範圍 + 常見部首對應
  // 這是近似檢測，完整版需要 Unihan 資料庫
  var RADICAL_MAP = {
    '氵': /[河海湖清淨溫潤澤漢濤淡渴流漂源深淺波洗洋浪池沙汪泉沖泊滄]/,
    '火': /[煙燒燃熱焰燈煥熙燦燎炎焚熔]/,
    '灬': /[然煮熟烈照熊點焦蒸煎熬]/,
    '艹': /[花草莊荷莉蘭芳菲華茂萌苗芸蓉菁蕾茹藝芝蓮萍蔓葉]/,
    '木': /[林森樹梅梓柏松柳桂楓橋櫻杏桃椿杉楠榮樂]/,
    '口': /[品哲唐嘉喬詠吟鳴含嗣呈]/,
    '宀': /[宇宙安宏宜宣富宗家容寧寶寒]/,
    '心': /[恩慧思慈惠悅恬怡愉懿忻]/,
    '忄': /[怡恆悠惟愷慎懷憶悟]/,
  ...
  var regex = RADICAL_MAP[radical];
  if (regex && regex.test(ch)) return true;
  return false;
}
