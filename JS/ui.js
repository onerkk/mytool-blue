            ...
            if (!_chkData.allowed) {
              ...
              if (typeof _jyStartPayment === 'function') {
                ...
                _pm.innerHTML = (typeof _buildPaywallHTML === 'function') ? _buildPaywallHTML('tarot_only') :
                  ...
                  '<div style="font-size:1rem;color:var(--c-gold);font-weight:700;margin-bottom:.5rem">今日免費次數已用完</div>' +
                  '<button onclick="_jyStartPayment(\'tarot_only\')" style="padding:.6rem 1.5rem;border-radius:10px;background:rgba(212,175,55,.1);color:var(--c-gold);border:1.5px solid rgba(212,175,55,.35);cursor:pointer;font-family:inherit;font-weight:700">🌙 開通會員 NT$399/月</button>' +
...
function _sendFeedbackToForms(rating,reasons,comment){
  ...
  const q=S.form?S.form.question:'';
  const type=S.form?S.form.type:'';
  const name=S.form?(S.form.name||'匿名'):'匿名';
  const reasonStr=Array.isArray(reasons)?reasons.join('、'):(reasons||'');

  // localStorage 備份
  try{
    const stored=JSON.parse(localStorage.getItem('jy_feedback')||'[]');
    stored.push({ts:new Date().toISOString(),rating,question:q,type,reasons:reasonStr,comment});
  ...
  try{
    fetch(WORKER_URL+'/feedback',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({rating,question:q,type,reasons:reasonStr,comment,name})
    })
    ...
    '<div class="jy-home">' +
      ...
      '<div class="jy-home-footer">' +
        ...
        (isAdmin ?
          ...
          '<div class="jy-home-quota" id="jy-home-quota-text">每日免費一次（任選）・ <strong style="color:var(--c-gold)">NT$399/月 每日 5 次</strong></div>'
        ) +
      '</div>' +
  ...
  (function _checkSubStatus() {
    ...
    if (subExpires > Date.now()) {
      ...
      if (el) {
        ...
        el.innerHTML = '🌙 <strong style="color:var(--c-gold)">會員啟用中</strong> ・ 每日 5 次 ・ 剩餘 ' + daysLeft + ' 天';
      }
    }
  })();
      ...
      if (!checkData.allowed) {
        ...
        if (inputScreen) {
          ...
          modal.innerHTML = '<div style="background:var(--c-bg-card,#1a1208);border:1px solid rgba(212,175,55,.25);border-radius:16px;padding:2rem 1.5rem;max-width:320px;text-align:center">' +
            ...
            '<div style="font-size:1rem;color:var(--c-gold);font-weight:700;margin-bottom:.4rem">今日免費次數已用完</div>' +
            '<div style="font-size:.82rem;color:var(--c-text-dim);line-height:1.7;margin-bottom:1rem">每天免費一次（任選一種工具或追問）<br>子時（00:00）重置，或開通會員每日 5 次</div>' +
            '<div style="display:flex;flex-direction:column;gap:.5rem;align-items:center">' +
