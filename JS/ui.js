            ...
            if (!_chkData.allowed) {
              ...
              if (typeof _jyStartPayment === 'function') {
                ...
                _pm.innerHTML = (typeof _buildPaywallHTML === 'function') ? _buildPaywallHTML('tarot_only') :
                  ...
                  '<button onclick="_jyStartPayment(\'tarot_only\')" style="padding:.6rem 1.5rem;border-radius:10px;background:rgba(212,175,55,.1);color:var(--c-gold);border:1.5px solid rgba(212,175,55,.35);cursor:pointer;font-family:inherit;font-weight:700">🌙 開通會員 NT$799/月</button>' +
...
function submitFeedbackDetail(){
  ...
  _sendFeedbackToForms(_fbRating||'bad',reasons,comment);
  document.getElementById('fb-reason-box').style.display='none';
  document.getElementById('fb-thanks').style.display='block';
}
...
function _sendFeedbackToForms(rating,reasons,comment){
  ...
  if (!aiClosing && !aiDirectAnswer) {
    try {
      ...
      if (prevResult) {
        aiClosing = (prevResult.closing || '').substring(0, 80);
        aiDirectAnswer = (prevResult.directAnswer || '').substring(0, 300);
        aiStory = (prevResult.story || '').substring(0, 800);
        if (prevResult.yesNo) aiYesNo = JSON.stringify(prevResult.yesNo).substring(0, 200);
      }
    } catch(_e) {}
  }
  if (!cards && !snap.cards) {
    try {
      if (tool === 'ootk' && window._ootkResults) {
        ...
        cards = ootkCards.join(' | ').substring(0, 400);
      } else if (tool === 'full') {
        ...
        if (prevR && prevR.layers) {
          ...
          cards = layerParts.join(' | ').substring(0, 400);
        }
      } else {
        var dc = (typeof drawnCards !== 'undefined') ? drawnCards : [];
        if (dc.length) cards = dc.map(function(c) { return (c.isUp ? '正' : '逆') + ' ' + (c.n || c.name || ''); }).join('、').substring(0, 400);
      }
    } catch(_e) {}
  }
  ...
  try{
    fetch(WORKER_URL+'/feedback',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({rating,question:q,type,reasons:reasonStr,comment,name,tool,cards,aiClosing,aiDirectAnswer,aiYesNo,aiStory,birth,birthTime,gender,birthLocation})
    })
...
function showFortuneResult(f){
  ...
  container.innerHTML = `
    <div class="fortune-result" style="animation:scaleIn 0.5s ease">
      ...
      <p class="text-xs text-muted mt-md">免費體驗次數有限 ✨ 開通會員暢用</p>
    </div>`;
    ...
    '<div class="jy-home">' +
      ...
      '<div class="jy-home-footer">' +
        ...
        (isAdmin ?
          ...
          '<div class="jy-home-quota" id="jy-home-quota-text">七維度・塔羅・開鑰 各免費體驗 1 次・ <strong style="color:var(--c-gold)">NT$799/月 會員無限暢用</strong></div>'
        ) +
      '</div>' +
        ...
        if (fs) {
          ...
          if (parts.length > 0) {
            el.innerHTML = '✨ 免費體驗剩餘：<strong style="color:var(--c-gold)">' + parts.join('、') + '</strong> ・ ' +
              '<span style="color:var(--c-gold);cursor:pointer" onclick="if(typeof _jyStartPayment===\'function\')_jyStartPayment(\'full\',\'subscription\')">開通會員無限暢用</span>';
          } else {
            el.innerHTML = '免費體驗已全部用完 ・ <strong style="color:var(--c-gold);cursor:pointer" onclick="if(typeof _jyStartPayment===\'function\')_jyStartPayment(\'full\',\'subscription\')">開通會員 NT$799/月</strong>';
          }
        } else {
      ...
      if (!checkData.allowed) {
        ...
        if (inputScreen) {
          ...
          modal.innerHTML = '<div style="background:var(--c-bg-card,#1a1208);border:1px solid rgba(212,175,55,.25);border-radius:16px;padding:2rem 1.5rem;max-width:320px;text-align:center">' +
            ...
            '<div style="font-size:.82rem;color:var(--c-text-dim);line-height:1.7;margin-bottom:1rem">三套工具各可免費體驗 1 次<br>開通會員暢用（塔羅每日3次＋七維度每月5次）</div>' +
            '<div style="display:flex;flex-direction:column;gap:.5rem;align-items:center">' +
