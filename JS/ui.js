...
function showFortuneResult(f){
  ...
  container.innerHTML = `
    <div class="fortune-result" style="animation:scaleIn 0.5s ease">
      ...
      <!-- 今日運勢焦點（白話文）-->
      <div style="background:linear-gradient(135deg,rgba(139,92,246,0.06),rgba(139,92,246,0.02));border-radius:var(--r-md);padding:var(--sp-md);margin:var(--sp-sm) 0;border-left:3px solid rgba(139,92,246,0.3)">
      ...
      <details style="margin:var(--sp-sm) 0">
        ...
        <div style="background:rgba(139,92,246,0.04);border-radius:var(--r-md);padding:var(--sp-sm) var(--sp-md);margin-top:0.3rem;font-size:0.82rem;color:var(--c-text-dim);line-height:1.8;border-left:2px solid rgba(139,92,246,.2)">
  ...
  try{
    var _extraHtml = '';
    if(f.dynNote) _extraHtml += '<div style="background:rgba(139,92,246,.08);border-radius:8px;padding:8px 12px;margin:8px 0;font-size:.85rem">'+f.dynNote+'</div>';
...
function showQuizResult(){
  ...
  if(typeof S !== 'undefined' && S.bazi && S.bazi.fav && S.bazi.fav.length){
    ...
    if(multiDimNote) baziNote += `<div style="margin-top:.5rem;padding:.5rem;background:rgba(139,92,246,.05);border-radius:6px">${multiDimNote}</div>`;
  }
...
// 小阿爾卡納花色
const SUIT_STYLES = {
  '權杖': {sym:'🔥', color:'#FF6347', bg:'#13151f', accent:'#FF4500'},
...
function generateLuckyInfo(){
  ...
  const colorsByEl = {
    '金': [{name:'白色',hex:'#f5f5f5',meaning:'金行正色，淨化氣場'},{name:'金色',hex:'var(--c-gold)',meaning:'強化權威與成就'},{name:'銀色',hex:'#94a3b8',meaning:'冷靜理性，金行守護'}],
  ...
  container.innerHTML = `
    <div class="lucky-card" style="animation:scaleIn 0.4s">
      ...
      <div style="margin-top:.6rem;padding:.5rem .8rem;background:${_isDayClash?'rgba(248,113,113,.08)':'rgba(139,92,246,.06)'};border-left:3px solid ${_isDayClash?'#f87171':'var(--c-gold)'};border-radius:0 6px 6px 0;font-size:.78rem;line-height:1.6">
      ...
      ${(function(){
        try{
          ...
          if(typeof S!=='undefined'&&S._uResult&&S._uResult.comb&&S._uResult.comb.topTags&&S._uResult.comb.topTags.length>0){
            ...
            return '<details style="margin-top:.5rem"><summary style="cursor:pointer;font-size:.8rem;color:var(--c-gold);padding:.3rem 0;font-weight:600">🔗 多系統象徵交集</summary><div style="font-size:.78rem;line-height:1.9;padding:.4rem .6rem;border-left:2px solid rgba(139,92,246,.2);margin-top:.2rem">'+_tagHtml+'</div></details>';
          }
...
function showAuraResult(){
  ...
  var elColors={'\u91D1':'var(--c-gold)','\u6728':'#22c55e','\u6C34':'#3b82f6','\u706B':'#ef4444','\u571F':'#eab308'};
...
(function upgradeVerdict(){
  ...
  const observer=new MutationObserver(()=>{
    ...
    else if(pct>=55) color='var(--c-gold)';
...
(function upgradeLoading(){
  ...
  if(!overlay.querySelector('.loading-bagua')){
    ...
    bagua.innerHTML='<svg viewBox="0 0 80 80" width="80" height="80"><circle cx="40" cy="40" r="38" fill="none" stroke="rgba(139,92,246,0.3)" stroke-width="1.5"/><path d="M40 2a38 38 0 0 1 0 76" fill="rgba(139,92,246,0.15)" stroke="rgba(139,92,246,0.5)" stroke-width="1.5"/><path d="M40 2a38 38 0 0 0 0 76" fill="rgba(109,40,217,0.15)" stroke="rgba(109,40,217,0.5)" stroke-width="1.5"/><circle cx="40" cy="21" r="6" fill="rgba(139,92,246,0.4)"/><circle cx="40" cy="59" r="6" fill="rgba(109,40,217,0.4)"/><circle cx="40" cy="21" r="2.5" fill="rgba(109,40,217,0.6)"/><circle cx="40" cy="59" r="2.5" fill="rgba(139,92,246,0.6)"/></svg>';
...
function _jd7BriefHTML(type, title){
  ...
  if(!syn) return '';
  var c = syn.finalDir==='pos'?'#4caf50':syn.finalDir==='neg'?'#e05c2f':'var(--c-gold)';
  return '<div style="margin-top:10px;padding:10px 12px;background:rgba(139,92,246,.05);border-left:3px solid '+c+';border-radius:0 8px 8px 0">'
...
generateLuckyInfo = function(){
  ...
  try{
    ...
    container.innerHTML += '<div style="margin-top:12px;padding:12px;border-radius:10px;background:rgba(139,92,246,.05);border-left:3px solid rgba(139,92,246,.45)">'
...
(function() {
  ...
  function _showUsedModal() {
    ...
    modal.innerHTML =
      '<div style="max-width:320px;width:88%;background:linear-gradient(145deg,#0e1018,#181a26);border:1.5px solid rgba(139,92,246,.35);border-radius:18px;padding:2.2rem 1.5rem;text-align:center;box-shadow:0 24px 80px rgba(0,0,0,.6)">' +
        '<div style="font-size:2.8rem;margin-bottom:1rem;filter:drop-shadow(0 0 12px rgba(139,92,246,.3))">🌙</div>' +
        '<h3 style="color:var(--c-gold,var(--c-gold));font-size:1.05rem;margin-bottom:.6rem;font-family:var(--f-display,serif)">今日的緣分已用盡</h3>' +
        '<p style="font-size:.85rem;color:var(--c-text-dim,rgba(232,234,240,0.6));line-height:1.7;margin-bottom:.3rem">命盤每天只能翻閱一次</p>' +
        '<p style="font-size:.78rem;color:var(--c-text-muted,rgba(232,234,240,0.35));margin-bottom:1.5rem">子時（00:00）重置，明天再來問</p>' +
        '<div style="display:flex;flex-direction:column;gap:.5rem;align-items:center">' +
          '<a href="https://tw.shp.ee/2n5Mo2w" target="_blank" rel="noopener" style="display:flex;align-items:center;justify-content:center;gap:6px;width:200px;padding:11px;border-radius:10px;background:linear-gradient(135deg,rgba(139,92,246,.15),rgba(139,92,246,.06));color:var(--c-gold,var(--c-gold));text-decoration:none;font-size:.85rem;font-weight:600;border:1px solid rgba(139,92,246,.3)"><i class="fas fa-gem"></i> 逛逛能量水晶</a>' +
          '<button onclick="document.getElementById(\'jy-used-modal\').remove()" style="width:200px;padding:11px;border-radius:10px;background:transparent;color:var(--c-text-dim,rgba(232,234,240,0.6));font-size:.82rem;border:1px solid rgba(255,255,255,.08);cursor:pointer;font-family:inherit">知道了</button>' +
        '</div>' +
      '</div>';
  ...
  function _redesignHomepage() {
    ...
    '<div style="position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;min-height:75vh;justify-content:center;padding:2rem 1rem">' +
      ...
      // 主標語
      '<h1 style="font-size:clamp(1.5rem,5vw,2rem);letter-spacing:.06em;text-align:center;margin-bottom:.6rem;font-family:var(--f-display,serif);color:var(--c-text,#e8eaf0)">' +
        '靜月之光' +
      '</h1>' +

      '<p style="font-size:.95rem;color:var(--c-text-dim,rgba(232,234,240,0.6));text-align:center;max-width:300px;line-height:1.9;margin-bottom:.4rem;font-family:var(--f-display,serif)">' +
        '你心裡的事，牌都知道' +
      '</p>' +
      '<p style="font-size:.76rem;color:var(--c-text-muted,rgba(232,234,240,0.35));text-align:center;max-width:320px;line-height:1.7;margin-bottom:2rem">' +
      ...
      '<div style="margin-top:1.5rem;text-align:center">' +
        (isAdmin ?
          ...
          '<span style="font-size:.73rem;color:var(--c-text-muted,rgba(232,234,240,0.35))">✦ 每日免費一次塔羅解讀</span>'
        ) +
      '</div>' +
    ...
    // 呼吸動畫
    '<style>@keyframes home-cta-breathe{0%,100%{box-shadow:0 0 30px rgba(139,92,246,.08),inset 0 0 20px rgba(139,92,246,.03);border-color:rgba(139,92,246,.4)}50%{box-shadow:0 0 45px rgba(139,92,246,.15),inset 0 0 30px rgba(139,92,246,.06);border-color:rgba(139,92,246,.6)}}</style>';
  }
  ...
  window._pickTypeNew = function(type) {
    ...
    event.currentTarget.style.borderColor = 'rgba(139,92,246,.5)';
    event.currentTarget.style.background = 'rgba(139,92,246,.08)';
    event.currentTarget.style.color = 'var(--c-gold,var(--c-gold))';
...
async function submitTarotQuick() {
    ...
    try {
      ...
      if (!checkData.allowed) {
        ...
        if (inputScreen) {
          ...
          modal.innerHTML = '<div style="background:var(--c-bg-card,#13151f);border:1px solid rgba(139,92,246,.25);border-radius:16px;padding:2rem 1.5rem;max-width:320px;text-align:center">' +
            ...
            '<button onclick="this.closest(\'#tarot-used-modal\').remove()" style="padding:.6rem 1.5rem;border-radius:10px;background:transparent;color:var(--c-gold);border:1.5px solid rgba(139,92,246,.4);font-size:.88rem;font-weight:600;cursor:pointer;font-family:inherit">知道了</button>' +
          '</div>';
  ...
  overlay.innerHTML =
    ...
    '@keyframes jyTarotGlow{0%,100%{box-shadow:0 0 20px rgba(139,92,246,.1),0 0 60px rgba(139,92,246,.05)}50%{box-shadow:0 0 40px rgba(139,92,246,.25),0 0 80px rgba(139,92,246,.12)}}' +
    ...
    '<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:100vh;gap:0;padding:2rem">' +
      ...
      '<div style="position:relative;width:180px;height:200px;margin-bottom:1.2rem">' +
        // 背景光暈
        '<div style="position:absolute;left:50%;top:50%;width:160px;height:160px;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(139,92,246,.12) 0%,rgba(139,92,246,.06) 40%,transparent 70%);animation:jyTarotGlow 3s ease-in-out infinite"></div>' +
        // 旋轉環
        '<div style="position:absolute;left:50%;top:50%;width:140px;height:140px;border-radius:50%;border:1.5px solid transparent;border-top-color:rgba(139,92,246,.3);border-right-color:rgba(139,92,246,.2);animation:jyTarotSpin 4s linear infinite;transform:translate(-50%,-50%)"></div>' +
        '<div style="position:absolute;left:50%;top:50%;width:110px;height:110px;border-radius:50%;border:1px solid transparent;border-bottom-color:rgba(139,92,246,.15);border-left-color:rgba(96,165,250,.15);animation:jyTarotSpin 3s linear infinite reverse;transform:translate(-50%,-50%)"></div>' +
        // 五張扇形卡牌
        '<div style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%)">' +
          '<div style="position:absolute;width:52px;height:78px;border-radius:6px;background:linear-gradient(135deg,#2a1f4e,#1a0f3e);border:1.5px solid rgba(139,92,246,.3);box-shadow:0 4px 15px rgba(0,0,0,.4);transform-origin:bottom center;transform:rotate(-24deg) translateY(-30px);animation:jyTarotFan .8s ease-out .2s both,jyTarotFloat 4s ease-in-out .2s infinite"><div style=\'text-align:center;padding-top:22px;font-size:1.2rem;opacity:.7\'>☽</div></div>' +
          ...
          '<div style="position:absolute;width:52px;height:78px;border-radius:6px;background:linear-gradient(135deg,#2a1f4e,#1a0f3e);border:1.5px solid rgba(139,92,246,.5);box-shadow:0 4px 20px rgba(139,92,246,.15);transform-origin:bottom center;transform:rotate(0deg) translateY(-30px);animation:jyTarotFan .8s ease-out .5s both,jyTarotFloat 3.5s ease-in-out 0s infinite"><div style=\'text-align:center;padding-top:18px;font-size:1.5rem\'>🃏</div></div>' +
        ...
        '</div>' +
        // 星星粒子
        '<div style="position:absolute;left:30%;top:20%;width:4px;height:4px;border-radius:50%;background:rgba(139,92,246,.8);animation:jyStarDrift 2.5s ease-out infinite"></div>' +
      ...
      // 狀態文字
      '<div id="ld-tarot-status" style="font-size:1.05rem;color:var(--c-gold,var(--c-gold));font-weight:700;letter-spacing:.02em;margin-bottom:.3rem;transition:opacity .35s">正在為你翻牌…</div>' +
      '<div id="ld-tarot-sub" style="font-size:.8rem;color:var(--c-text-dim,rgba(232,234,240,0.6));transition:opacity .35s">凝神感應你的問題</div>' +

      // 進度條
      '<div style="width:min(260px,75%);height:3px;border-radius:999px;background:rgba(139,92,246,.08);overflow:hidden;margin-top:1rem">' +
        '<div style="width:35%;height:100%;border-radius:999px;background:linear-gradient(90deg,rgba(139,92,246,0),rgba(139,92,246,.9),rgba(255,236,184,.8),rgba(139,92,246,0));background-size:180% 100%;animation:jyScan 1.8s ease-in-out infinite"></div>' +
      '</div>' +

      // 牌陣提示
      '<div style="margin-top:.8rem;display:flex;gap:.3rem;flex-wrap:wrap;justify-content:center">' +
        '<span style="padding:.18rem .45rem;border-radius:999px;font-size:.64rem;color:rgba(139,92,246,.7);border:1px solid rgba(139,92,246,.12);background:rgba(139,92,246,.04)">金色黎明</span>' +
...
function renderTarotSpreadDisplay() {
  ...
  (drawnCards || []).forEach(function(c, i) {
    ...
    if (typeR) h += '<p style="font-size:.76rem;color:var(--c-gold-light,var(--c-gold-light));margin-top:.2rem;line-height:1.5;border-top:1px solid rgba(139,92,246,.1);padding-top:.2rem">📌 ' + typeR + '</p>';
...
function switchResultTab(mode) {
  ...
  if (mode === 'tarot' && S._tarotQuickResult) {
    ...
    if (tabTarot) { tabTarot.classList.add('active'); tabTarot.style.background = 'rgba(139,92,246,.12)'; tabTarot.style.color = 'var(--c-gold)'; }
  } else {
    if (fullContent) fullContent.style.display = 'block';
    if (tarotContent) tarotContent.style.display = 'none';
    if (tabFull) { tabFull.classList.add('active'); tabFull.style.background = 'rgba(139,92,246,.12)'; tabFull.style.color = 'var(--c-gold)'; }
