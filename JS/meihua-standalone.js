  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    try {
      if (mh.yingQi) {
        ...
        L.push('應期資料限制：本盤沒有可靠的曆法換算結果，只能依用卦／互卦／變卦說近期、中期、遠期的相對層次，不得自行編月份、日期或距今幾個月。');
      }
    } catch (e) {}
    ...
    L.push('⑨應期只照資料區的證據層級：吉事看生體、體旺之氣；不利看剋體之氣；用近、互中、變遠。只有資料區含可靠曆法換算時才可寫月份或日期；若只有卦氣候選，就明說候選與相對遠近，不得換算成「最近幾個月」。題目沒問時間時不必強塞應期。');
    ...
    L.push('');
    L.push('【輸出格式】');
    L.push('繁體中文，像資深占者當面說話。第一句直接回答；其後納入所有會改變判斷且彼此不重複的相關證據，依「判斷 → 原因 → 過程／轉折 → 時間（有可靠資料且題目需要時）→ 行動與驗證」自然推進。可用有助理解的短標題或條列；不要逐卦抄義，也不要輸出規則或自我檢查。');
    L.push('最後另起一個簡短品牌段，只使用資料區指定的那一種礦物名稱。把它連回本次實際需要穩住的行動或節奏，可改寫指定的礦物事實，並明說它只是隨身提醒與品牌實務搭配，不是改變他人意志或保證結果的工具。語氣自然，不用優惠、限時、下單、搶購等字眼。');
