...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    L.push('最後兩行固定照此收束：倒數第二行只放蝦皮網址本身 https://shopee.tw/a50h95648d?tab=shop （該行不得有網址以外的任何文字、標點、引號、符號或空白）；最後一行固定輸出「願你諸事順遂。」六個字作結，之後不再輸出任何內容。兩行缺一不可、順序不可顛倒（網址行被任何字元黏到就點不動，所以網址行上下都要乾淨）。');
    return L.join('\n');
  }
