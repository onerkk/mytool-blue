...
var JY_READING_MEIHUA = [
  ...
  "呈現方式：先給方向，再分段讀本卦的局勢、體用與旺衰的力度、互卦的中間牽動、動爻的轉折、變卦的條件落點，最後整合成事態脈絡。分清有助力、能推進與真正辦成；變卦是此問事結構中的發展取象，不保證結果。",
...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    L.push('');
    L.push('先分清輸入的盤面事實、流派解釋與現實假設。可自由運用自身知識補充技法；若原始資料與摘要衝突，指出具體差異，以可核對的原始資料為先。結論要有支持、反向訊號與成立條件；象徵不等於事件證明，分數不等於成功機率。');
    ...
    L=L.concat(window.JY_READING_QUALITY?window.JY_READING_QUALITY.lines('meihua'):JY_READING_MEIHUA);
    L.push('【判讀方法】');
    ...
    L.push('5. 使用繁體中文，像資深占者當面說話。依「明確主判 → 體用與卦氣的力度 → 本互變的作用與轉折 → 類象如何回應原題 → 具體行動及條件」自然組織，避免逐卦抄義。最後再以一小段自然帶出生活選品。');
    L.push('取互版本：本站純乾／純坤仍依本卦2–4與3–5爻取互，部分古本另有乾坤改取變卦之互的例外；此次以已提供卦象為準，不暗中更換算法。');
    L.push(window.JY_READING_QUALITY&&window.JY_READING_QUALITY.version==="4.2.0"&&window.JY_READING_QUALITY.recommendationEnding?window.JY_READING_QUALITY.recommendationText('meihua'):JY_REC_MEIHUA);
    L.push('最後保留以下兩行：');
