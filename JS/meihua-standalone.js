/*! meihua-standalone.js — 靜月之光 梅花易數獨立流程  [v80.37]
...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    L.push('收尾 2–3 句：依體卦五行喜忌（體弱宜補體之五行、體被剋宜取化解之五行），自然推薦一種適合隨身的水晶／能量石幫他穩住氣場（實務輔助、非梅花原理），語氣平實不推銷。蝦皮連結獨立成行、放在全文最後一行：https://shopee.tw/a50h95648d?tab=shop ——該行只能有網址本身，網址前後不得有任何文字、標點、引號、符號或空白，網址後也不得再輸出任何內容、任何字元（黏上任何字元連結會點不動；輸出到網址最後一個字母就停筆）。只講一種、只在最後。主石只准寫出「一種」石頭的名字——嚴禁「或」「這類」「例如A、B」式並列第二種；石頭名只能用通用礦石名，不可使用卦象外的商品資訊。');
    return L.join('\n');
  }
