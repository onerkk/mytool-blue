/*! meihua-standalone.js — 靜月之光 梅花易數獨立流程  [v80.40]
...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    function _forceNote(relName, tw, yw) {
      ...
      if (relName==='體克用') {
        if (tiWeak && yoStrong) return '體弱剋旺用——掌控力最有限，成得最費勁、吉打最大折。';
        if (tiStrong && yoWeak) return '體旺剋衰用——壓得輕鬆、最易成。';
        if (tiStrong) return '體旺——你壓得住、可成，主動推進就行。';
        if (tiWeak) return '體弱想掌控——力道不足，成得很費勁。';
        return '掌控力中等——可成但需出力。';
      }
      if (relName==='用生體') {
    ...
    try {
      ...
      if (tiEl && _SM[tiEl]) {
        ...
        L.push('應期（正統斷法：事應於生體卦氣之時、敗於剋體卦氣之時；依節氣換月、前後誤差一兩天）：');
    ...
    L.push('②體用生剋是斷吉凶的核心，吉凶定性照《梅花易數·體用總訣》原文語彙，據實說、別只報關係名詞：');
    L.push('　・用生體＝有進益之喜（大吉）：外力、貴人、環境來幫你，事半功倍。');
    L.push('　・比和＝百事順遂（吉）：同氣相順，事順但突破有限。');
    L.push('　・體剋用＝諸事吉：可成、主動在我——剋出仍須出力，但原文定吉，不可講成只是小吉或困難重重。');
    L.push('　・體生用＝有耗失之患（洩耗）：你在付出、被牽著走，耗神耗財，當心得不償失。');
    L.push('　・用剋體＝諸事凶：受制、受阻、被壓——這是壞消息，講清楚不利在哪。');
    ...
    L.push('⑦應期要落地：照資料區「應期」行講——吉事大概應在生體、體旺之氣的時段，不利之事多在剋體之氣的時段顯形；近中遠以「用卦＝近期、互卦＝中期、變卦＝遠期」分層。給範圍並說出為什麼是那時候（哪個卦氣、哪個月），不要只說「快了」；不可自創其他應期算法（包括用「用卦五行對應季節」斷應期月份——那是事情節奏、不是應期）。若動爻／變卦顯示轉折，標出轉折大概落在哪段時間、是轉順還是轉逆。');
    L.push('⑧挑最強的訊號深挖：把「體用生剋＋動爻變化」這條主線講透——動爻要連同爻位層次一起讀（變化落在事情哪一層），不要十二類象蜻蜓點水。');
    L.push('⑨不要用粗體標題分類、不要「本卦：…互卦：…」逐段排版，像跟人講話一樣自然過渡。');
    L.push('⑩只依卦象與生剋推，不臆測他的心理動機、不反問他；卦上看不出來就說看不出來，不硬編。也嚴禁引用卦象之外你自以為知道的當事人資訊（職業、班別、副業、商品、生活細節等）——即使對話上下文或你的記憶裡有，一律不得寫進來；所有具象都必須從卦象與生剋推出。');
    ...
    L.push('□ 用類象推了可對照的人事物　□ 應期照資料區吉應／敗應講（範圍＋為什麼，有轉折標順逆，沒有自創算法）　□ 錯綜補看了另一面／對方視角（若有助主線）　□ 不利處直說並給可行的主動方向　□ 正文沒有出現任何指令字眼（「語氣平實」「只講一種」「鐵律」「清單」等）——這些指令是給你的，不是讀給客人聽的');
    L.push('');
    L.push('【輸出格式】');
    L.push('繁體中文，像占者當面講話，不是分析報告。第一句給結論，之後一段段推進，每段帶新資訊、同一結論只講一次。不要列表格、不要粗體小標、不要逐卦報告。');
    L.push('收尾 2–3 句：依體卦五行喜忌（體弱宜補體之五行、體被剋宜取化解之五行），自然推薦一種適合隨身的水晶／能量石幫他穩住氣場（實務輔助、非梅花原理），語氣平實不推銷。只講一種、只在最後。主石只准寫出「一種」石頭的名字——嚴禁「或」「這類」「例如A、B」式並列第二種；石頭名只能用通用礦石名，不可使用卦象外的商品資訊。');
    L.push('最後兩行固定照此收束：倒數第二行只放這一個 Markdown 連結、原樣照抄（含中括號與小括號）：[靜月之光蝦皮賣場](https://shopee.tw/a50h95648d?tab=shop) （該行不得有這個連結以外的任何文字或空白；嚴禁把網址裸貼成純文字——裸的長網址會被渲染管線插入看不見的字元而點不動，Markdown 連結的網址收在小括號裡、畫面只顯示中文字，不會被動到）；最後一行固定輸出「願你諸事順遂。」六個字作結，之後不再輸出任何內容。兩行缺一不可、順序不可顛倒（網址行被任何字元黏到就點不動，所以網址行上下都要乾淨）。');
    return L.join('\n');
  }
