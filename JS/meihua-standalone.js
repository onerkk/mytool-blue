...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    L.push('收尾 2–3 句：依體卦五行喜忌（體弱宜補體之五行、體被剋宜取化解之五行），自然推薦一種適合隨身的水晶／能量石幫他穩住氣場（實務輔助、非梅花原理），語氣平實不推銷，可輕附蝦皮去處，並以連結收尾：https://tw.shp.ee/9UHEJTp4 ——這個 URL 必須是整份輸出的最後一個字，網址後不接任何文字、標點、引號、符號或空白（黏上去連結會點不動）。只講一種、只在最後。');
    return L.join('\n');
  }
