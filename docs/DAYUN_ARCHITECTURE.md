...
- **十二節**（用於大運起運）：小寒、立春、驚蟄、清明、立夏、芒種、小暑、立秋、白露、寒露、立冬、大雪。
- **API**：
  - `getTermsForYear(year)` → `{ name, date, utc? }[]` 依時間排序；同年 **TERM_DB 優先**（本地），否則 JSON（UTC）。
...
- **起運歲數**（依架構**真實換算**，每人不同）：
  - 陽男陰女**順行**：順數至**下一個節**；陰男陽女**逆行**：逆數至**上一個節**（跨年見上）。
  - 換算：**天數 ÷ 3**，**1 日 = 4 月**；可選時辰 precise、節氣 UTC→真太陽時（見 opts）。
