...
- **手機版沒更新時**：多為瀏覽器快取。請 (1) 重新整理頁面（必要時「強制重新載入」／清除快取），或 (2) 部署後使用 `index.html` 內建的 `?v=x.x` 快取破壞參數（發版時改版號，例如目前為 `?v=2.22`，下次可改為 `?v=2.23`）讓手機載入最新 JS/CSS。
...
## 發版時讓手機同步（必做）
- 每次部署新程式後，請將 `index.html` 中**所有** `?v=x.x`（含 CSS、JS、manifest、sw、測試腳本）改為同一新版號（例如 `?v=2.20` → `?v=2.21`），否則手機可能繼續使用舊快取的 JS/CSS，導致「手機未同步」。
- 使用者若仍看到舊版：請在手機瀏覽器使用「強制重新載入」或清除該站快取後再開。
- **iOS 排版**：`css/ios-fixes.css` 處理劉海／Home Indicator 安全區、100dvh 視口、模態框 padding。修改結果區或浮動按鈕時請同步 `env(safe-area-inset-*)`。
