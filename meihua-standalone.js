...
(function () {
  ...
  function buildMeihuaPrompt(question, mh) {
    ...
    L.push('');
    if(!window.JY_READING_QUALITY)L.push("【白話優先】先用兩三句日常語言回答最在意的問題，接著說明現在能做什麼，再用一小段補上支持主判的關鍵依據。正文以白話為主，少用術語；首次使用術語時立刻翻成生活意思。分析要仔細，但不要把逐項推演、每宮每星或整套技法清單都寫給讀者；使用者要求詳細技法時再展開。每個實質子題仍須回應，不用縮短篇幅掩蓋資料不足。建議要具體、做得到，可附一句實際能說出口的話，以及何時依哪些現實訊號調整。語氣溫和直接，不責怪命主、不貼宿命標籤，不只說保持正向。");
    ...
    L.push('5. 使用繁體中文，像資深占者當面說話。依「白話結論 → 可行建議 → 簡短卦理依據與轉變條件」自然組織，避免逐卦抄義。最後再以一小段自然帶出生活選品。');
