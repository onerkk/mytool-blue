// Step 2: 加回防護盾（try-catch 包住）+ RC4 加密
...
const OBF_OPTS = {
  ...
  stringArray: true,
  stringArrayEncoding: ['rc4'],  // ★ 升級：base64 → RC4
...
console.log('');
console.log('🔒 混淆 Step 2: 防護盾 + RC4 加密');
console.log('════════════════════════════════');
...
for (const filePath of JS_FILES) {
  ...
  if (filePath === SHIELD_TARGET) { console.log('🛡️  注入防護盾 → ' + filePath); code = SHIELD_CODE + code; }
