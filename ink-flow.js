// ═══════════════════════════════════════════════════════════════════
// 靜月之光 墨流 ink-flow v1.2 (2026/6/11)
// Suminagashi（墨流し）墨暈層——v1.2 面板內掛載＋輪詢偵測：
//   v1.1 根因修正：四個工具頁是「建立一次、display:none/block 切換」的面板，
//   display 切換不會觸發 MutationObserver——改用已知容器 id 輕量輪詢（700ms，
//   閒置時才跑）：bzx-screen/mhx-screen/ln-screen/zw-input/zw-result，
//   另保留通用後備（可見的 fixed z≥2900 近全螢幕不透明容器）。
//   首頁不畫；面板打開→畫布掛進面板內部 z-index:-1 子層（面板底色上、表單下），
//   面板隱藏或拔除→自動收回、輪詢續行。
...
(function () {
  ...
  // ── 面板偵測 v1.2：已知容器 id 為主、通用判定為輔，輪詢驅動 ──
  ...
  function findOpenPanel() {
    ...
    if (best) return best;
    // 通用後備：未來新面板（可見 fixed z≥2900 近全螢幕不透明）
  ...
  function poll() {
    if (host) {
      // 面板被隱藏（返回首頁）或拔除→收回
      if (!host.isConnected || !isVisible(host)) unmount();
      return;
    }
    var p = findOpenPanel();
    if (p) mount(p);
  ...
  function boot() {
    ...
    setInterval(poll, 700);  // 閒置時 5 次 getElementById＋幾個樣式讀取，成本趨近零
    poll();                  // 開頁時面板可能已開著
