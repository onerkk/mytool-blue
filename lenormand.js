// ═══════════════════════════════════════
// 靜月之光 — 雷諾曼牌 Lenormand v80.7
// Petit Lenormand 36 張・傳統組合義讀法・反盤外牌名幻覺
// ═══════════════════════════════════════
(function () {
'use strict';
console.log('[Lenormand] 靜月之光 雷諾曼牌 v80.7 loaded — 真實相鄰/原典邊界/AI按鈕修正');
...
var SPREADS = {
  ...
  nine: { id:'nine', name:'九宮格', en:'Nine-Card Box (3×3)', count:9,
    desc:'3×3 現代實務牌陣。只按3×3實際相鄰與三張成句輔助解讀，不宣稱 Game of Hope 原始規則。',
...
function buildPrompt(question, drawn, spreadId, sigGender) {
  ...
  lines.push('你是精通 Petit Lenormand（小雷諾曼）的雷諾曼占卜師。以下是問卜者抽到的牌面與完整資料，請優先依「相鄰兩張／三張組合」解讀；若為大牌陣，優先依 Game of Hope 可查核心：36張、4排8張＋最後4張、紳士28／淑女29、附近牌與遠近。三張線、五張線、九宮格、房屋等只屬現代實務補充，必須降權使用。');
  lines.push('');
  ...
  lines.push('可查原始邊界採 Das Spiel der Hofnung / Das Spiel der Hoffnung / Game of Hope：完整36張牌；占卜排法為洗牌、切牌、排成4排8張加最後4張，女問卜者從29、男問卜者從28開始，依其附近牌說故事。');
  lines.push('British Museum 館藏 1896,0501.495 記錄 Das Spiel der Hofnung 為完整36張牌並附說明；Horniman Museum 1970.18 記錄36張 Lenormand 牌以 Hechtel 約1799年的 Game of Hope 為模型。');
  ...
  lines.push('3. 每個重要結論用破折號附本盤實際相鄰牌組出處；訊號不足就直接說不足。');
  lines.push('4. 可驗證信號必須來自牌面可觀察事件，例如訊息、對話、公開互動、阻礙解除；不要幻想不可驗證的內心劇情。');
  lines.push('5. 24小時行動建議是現代實務轉譯，不是原典；只能從本盤牌組推導，且要能實際操作。');
  lines.push('6. 收尾能量石屬品牌實務輔助，非雷諾曼原典，不得說成牌義、原典或必然效果。');
  lines.push('');
  ...
  if (topicRows.length) {
    ...
    lines.push('【問事相關牌定位】本盤沒有抽到足夠直接對應牌；正文必須說明訊號不足，不得引用盤外牌名反證。');
    lines.push('');
  }
